...
TEST_CASE( "Test Mergering Motifs into single structure " ) {
    ...
    SUBCASE("test adding two motifs in isolation") {
        ...
        CHECK(s->chains().size() == 4);
        
    }
    
    SUBCASE("test adding two motifs in connected") {
    ...
    SUBCASE("test adding helix and hairpin together for one chain") {
    ...
    SUBCASE("test adding hairpins together for one chain") {
    ...
    SUBCASE("catch updating motif that does not exist") {
        auto mm = motif_data_structure::MotifMerger();
        mm.add_motif(m1);
        mm.update_motif(m1);
    ...
    SUBCASE("catching motif with same id twice") {
        auto mm = motif_data_structure::MotifMerger();
        mm.add_motif(m1);
    ...
    SUBCASE("test simple") {
        ...
        CHECK(rna_struc->get_residue(m1->ends()[0]->res1()->uuid()) != nullptr);
        CHECK(rna_struc->get_residue(m1->ends()[1]->res1()->uuid()) == nullptr);

        auto ss = mm.secondary_structure();
        CHECK(ss->sequence() == "CCC&GGG");
        CHECK(ss->dot_bracket() == "(((&)))");

    }
    
    SUBCASE("test conserve sequence indentity with twoway") {
        ...
        CHECK(rna_struc->get_residue(m1->ends()[0]->res1()->uuid()) == nullptr);
        CHECK(rna_struc->get_residue(m2->ends()[1]->res1()->uuid()) != nullptr);
    }
    
    SUBCASE("test sequence indenity conflict") {
