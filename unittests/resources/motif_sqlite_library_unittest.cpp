...
TEST_CASE("Test Motif Sqlite3 Library") {
  ...
  SUBCASE("should return an error if not a valid library name") {
    REQUIRE_THROWS_AS(resources::MotifSqliteLibrary("fake"),
                      resources::SqliteLibraryException);
  }
  ...
  SUBCASE("test individual queries") {
    ...
    REQUIRE_THROWS_AS(mlib.get("", "TEST"), resources::SqliteLibraryException);
    REQUIRE_THROWS_AS(mlib.get("", "", "TEST"),
                      resources::SqliteLibraryException);
  }
  ...
  SUBCASE("test grabing more then one motif in the same query") {
    auto mlib = resources::MotifSqliteLibrary("new_twoways");
    mlib.load_all();
    ...
    for (auto const& m : mlib) {
      ...
      //   continue;
      // }
      f << m->name() << "/" << m->end_name(0) << "," << m->sequence()
        << "," << m->dot_bracket() << std::endl;
      seen[m->name()] = 1;
    }
    f.close();
