...
class DesignRNAScaffold : public base::Application {
 ...
 private:  // setup functions
  ...
  void _setup_from_pdb();

  motif_search::SearchOP _setup_search();

  motif_search::ProblemOP _setup_problem();

  motif_search::SolutionTopologyTemplateOP _setup_sol_template_from_path(
      String const &);

  motif_search::SolutionFilterOP _setup_sol_filter(String const &);

  void _check_bp(String const &, structure::RNAStructureOP const &,
                 String const &) const;

  void _setup_extra_pdbs();

 private:  // run functions
  motif_data_structure::MotifGraphOP _get_motif_graph_solution();

  void _get_graph_indexes_after_bp_steps(
      motif_data_structure::MotifGraph const &, GraphIndexes const &,
      GraphIndexes & /* return */);

  motif_data_structure::MotifGraphOP _perform_sequence_opt(
      motif_data_structure::MotifGraph const &, GraphIndexes const &);

  motif_data_structure::MotifGraphOP _perform_thermo_fluc_sim(
      motif_data_structure::MotifGraph &, GraphIndexes const &);

  void _record_solution(motif_data_structure::MotifGraph &);

  void _fix_flex_helices_mtype(motif_data_structure::MotifGraphOP);

  void _get_motif_names(motif_data_structure::MotifGraphOP);

  void _build_new_ensembles(String const &);

  void _get_mseg(motif_data_structure::MotifGraph &,
 ...
 private:
  struct SolutionInfo {
    ...
    static String col_names() {
      auto cols = String("");
      cols +=
          "design_num,design_score,design_sequence,design_structure,motifs_"
          "uses,opt_num,";
    ...
    friend std::ostream &operator<<(std::ostream &output,
                                    SolutionInfo const &si) {
      output << si.design_num << "," << si.design_score << ","
             << si.designable_sequence << ",";
      output << si.dot_bracket << "," << si.motif_names << ","
             << si.seqeunce_opt_num << ",";
      output << si.sequence << "," << si.sequence_opt_score << ","
             << si.thermo_fluc_best_score;
 ...
 public:
  CLI::App
      app_;  // added by CJ 08/20. has to be public to get the --help to work
