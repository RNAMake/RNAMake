...
void DesignRNAScaffold::parse_command_line(int argc, const char **argv) {}

void DesignRNAScaffold::run() {
  ...
  while (true) {
    ...
    _get_graph_indexes_after_bp_steps(*mg_w_sol, starting_indexes_,
                                      bp_step_indexes);
    ...
    for (int i = 0; i < parameters_.seq_opt.sequences_per_design; i++) {
      ...
      if (sol_info_.sequence_opt_score > parameters_.seq_opt.cutoff) {
        ...
        LOG_DEBUG << "seq opt did not a good enough solution: "
                  << sol_info_.sequence_opt_score << " redoing attempt "
                  << attempts;
        i -= 1;
        continue;
      }
  ...
  if (parameters_.core.designs <= sol_info_.design_num) {
    ...
    LOG_INFO << "found " << sol_info_.design_num
             << " designs; did not reach desired num of "
...
void DesignRNAScaffold::setup() {
  ...
  LOG_INFO << "out file that contains all build solutions -> " +
                  parameters_.io.out_file + " can be set with --out_file";
  LOG_INFO
      << "score file that contains summary information about solutions -> " +
  ...
  seq_optimizer_ =
      std::make_shared<sequence_optimization::SequenceOptimizer3D>();
  ...
  if (!parameters_.seq_opt.skip) {
    LOG_INFO << "sequence optimization cutoff -> "
             << seq_optimizer_->get_float_option("cutoff");
    LOG_INFO << "sequence optimization steps -> "
             << seq_optimizer_->get_int_option("steps");
  } else {
    LOG_INFO << "set --skip_sequence_optimization -> skipping sequence "
                "optimization step!";
  ...
  // thermo sim setup
  auto thermo_scorer =
      std::make_shared<thermo_fluctuation::graph::OldFrameScorer>();
  auto sterics =
      std::make_shared<thermo_fluctuation::graph::sterics::NoSterics>();
  thermo_sim_ = std::make_shared<thermo_fluctuation::graph::Simulation>(
      thermo_scorer, sterics);
}

void DesignRNAScaffold::_setup_from_pdb() {
  ...
  // TODO allow for construction of motif from RNA structure to allow for
  // changes in base pair types
  ...
  try {
    ...
    LOG_ERROR << "starting basepair: " << parameters_.core.start_bp
              << " was NOT FOUND!";
    exit(0);
  }
  ...
  try {
    ...
    LOG_ERROR << "end basepair: " << parameters_.core.end_bp
              << " was NOT FOUND!";
    exit(0);
  }
  ...
  if (!parameters_.search.no_sterics) {
    ...
    LOG_INFO << total_beads
             << " rna steric beads were found to block residue overlap with "
                "supplied structure";
    LOG_INFO << protein_beads
             << " protein steric beads were found to block residue overlap "
                "with supplied structure";
  } else {
  ...
  if (!parameters_.search.starting_helix.empty()) {
    ...
    msg_->add_state(h, starting_indexes_.start.node_index,
                    starting_indexes_.start.edge_index);
    starting_indexes_.start = data_structure::NodeIndexandEdge{1, 1};
  }
}

void DesignRNAScaffold::_setup_extra_pdbs() {
  auto pdb_files =
      base::split_str_by_delimiter(parameters_.core.extra_pdbs, ",");
  if (!pdb_files.empty()) {
    LOG_INFO << " loading pdbs from --extra_pdb flag: "
             << parameters_.core.extra_pdbs;
  }
...
void DesignRNAScaffold::_check_bp(String const &name,
  ...
  if (bps[0]->bp_type() != "cW-W" && parameters_.search.no_basepair_checks) {
    ...
    LOG_WARNING << "basepair " + name +
                       " is not a watson and crick bp, but forcing it to be, "
                       "this may be bad";
  }
...
motif_search::SearchOP DesignRNAScaffold::_setup_search() {
  ...
  if (parameters_.search.solution_filter == "NoFilter") {
    LOG_WARNING << "no solution filter is set, this will lead to duplicate "
                   "solutions under "
                << "certain conditions";
  }
  ...
  if (parameters_.search.type == "path_finding") {
    ...
    pf_search->set_option_value("max_node_level",
                                parameters_.search.max_motifs);
  ...
  if (parameters_.search.type == "exhaustive") {
    ...
    auto scorer =
        score_factory.get_scorer(parameters_.search.exhaustive_scorer);
    // TODO setup default sol topology if user does not specify motif_path
    auto sol_template =
        std::make_shared<motif_search::SolutionTopologyTemplate>();
    if (!parameters_.search.motif_path
             .empty() /* added by CJ 08/20 parameters_.motif_path != ""*/) {
      sol_template =
  ...
  if (parameters_.search.type == "mc") {
    ...
    if (parameters_.search.mc_scorer == "default") {
      ...
      scorer = std::make_shared<ScaledScorer>(
          parameters_.search.scaled_score_d, parameters_.search.scaled_score_r);
    }
  ...
  LOGI << "search max size (how many residues) -> " +
              std::to_string(parameters_.search.max_size);
  return search;
}

motif_search::ProblemOP DesignRNAScaffold::_setup_problem() {
  auto start = starting_indexes_.start;
  auto end = starting_indexes_.end;
  auto start_bp =
      msg_->get_node(start.node_index)->data()->get_end_state(start.edge_index);
  auto end_bp =
      msg_->get_node(end.node_index)->data()->get_end_state(end.edge_index);

  bool target_an_aligned_end = false;
  if (end.edge_index ==
      msg_->get_node(end.node_index)->data()->block_end_add()) {
    target_an_aligned_end = true;
  }
  return std::make_shared<motif_search::Problem>(start_bp, end_bp, lookup_,
                                                 target_an_aligned_end);
}

motif_search::SolutionTopologyTemplateOP
DesignRNAScaffold::_setup_sol_template_from_path(String const &motif_path) {
  auto sol_template =
      std::make_shared<motif_search::SolutionTopologyTemplate>();
  auto spl = base::split_str_by_delimiter(motif_path, ",");
  int i = 0;
  auto lib_names = resources::MotifStateSqliteLibrary::get_libnames();
  LOG_DEBUG << "building motif path from string: "
            << replace_char(motif_path, ',', ';');
  for (auto const &e : spl) {
    ...
    // found user specified ensemble
      ...
      try {
        ...
        LOG_ERROR << "unclear what " << e
                  << " is it is not recognized as a motif library,"
                  << " ensemble or supplied motif";
        exit(0);
      }

      auto states = motif::MotifStateOPs();
      auto floats = Floats();
      ...
      auto mse = std::make_shared<motif::MotifStateEnsemble>(states, floats);
...
motif_search::SolutionFilterOP DesignRNAScaffold::_setup_sol_filter(
...
DesignRNAScaffold::_get_motif_graph_solution() {
  ...
  if (sol_info_.design_num < 10) {
    LOG_INFO << "found a solution: " << sol_info_.motif_names
             << " with score: " << sol_info_.design_score;
  ...
  if (parameters_.search.only_tether_opt) {
    ...
    mg_w_sol->add_connection(
        mg_->last_node()->index(), end.node_index,
  ...
  if (parameters_.io.dump_pdbs && parameters_.seq_opt.skip) {
    mg_w_sol->to_pdb("design." + std::to_string(sol_info_.design_num) + ".pdb",
                     1, 1);
  }
...
void DesignRNAScaffold::_get_graph_indexes_after_bp_steps(
  ...
  auto last_m = mg.get_node(end.node_index)
                    ->connections()[end.edge_index]
                    ->partner(end.node_index);
  new_indexes.start = data_structure::NodeIndexandEdge{last_m->index(), 1};
  new_indexes.end = end;
}

motif_data_structure::MotifGraphOP DesignRNAScaffold::_perform_sequence_opt(
    motif_data_structure::MotifGraph const &mg_w_sol,
    GraphIndexes const &bp_step_indexes) {
  auto opt_seq_scorer =
      std::make_shared<sequence_optimization::InternalTargetScorer>(
...
motif_data_structure::MotifGraphOP DesignRNAScaffold::_perform_thermo_fluc_sim(
    motif_data_structure::MotifGraph &mg, GraphIndexes const &bp_step_indexes) {
  ...
  tf_indexes.start = data_structure::NodeIndexandEdge{
      index_hash[bp_step_indexes.start.node_index],
      bp_step_indexes.start.edge_index};
  tf_indexes.end = data_structure::NodeIndexandEdge{
      index_hash[bp_step_indexes.end.node_index],
  ...
  best_mg->add_connection(tf_indexes.start.node_index,
                          tf_indexes.end.node_index,
                          best_mg->get_node(tf_indexes.start.node_index)
                              ->data()
                              ->ends()[tf_indexes.start.edge_index]
                              ->name(),
                          best_mg->get_node(tf_indexes.end.node_index)
                              ->data()
                              ->ends()[tf_indexes.end.edge_index]
                              ->name());

  sol_info_.thermo_fluc_best_score = best;
  sol_info_.thermo_fluc_hits = count;
  LOG_DEBUG << "best dist score: " << best;
  LOG_DEBUG << "hits: " << count
            << "; %: = " << ((float)count / (float)1000000) * 100.0f;
  LOG_DEBUG << "*** finished thermo fluc sim ***";
  return best_mg;
}

void DesignRNAScaffold::_record_solution(motif_data_structure::MotifGraph &mg) {
  if (parameters_.io.dump_pdbs && !parameters_.seq_opt.skip) {
...
void DesignRNAScaffold::_fix_flex_helices_mtype(
    motif_data_structure::MotifGraphOP mg) {
  // Some versions of RNAMake have this issue, that flex helices arent set as
  // "helices" this can raise merger issues
  for (auto &n : *mg) {
...
void DesignRNAScaffold::_get_motif_names(
    motif_data_structure::MotifGraphOP mg) {
...
void DesignRNAScaffold::_build_new_ensembles(String const &new_ensemble_path) {
  ...
  for (auto const &l : lines) {
    ...
    new_motif_ensembles_[spl[0]] =
        std::make_shared<motif::MotifStateEnsemble>(all_ms, scores);
  }
}

void DesignRNAScaffold::_get_mseg(
