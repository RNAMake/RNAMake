...
PoseOP get_pose_from_pdb(String const &path, PDBParser &pdb_parser) {
  ...
  if (residues->has_RNA()) {
    ...
    if (end_indexes.size() == 0) {
      dot_bracket = std::make_shared<base::SimpleString>(
          generate_secondary_structure(*rna_structure, bps));
    } else {
      dot_bracket = std::make_shared<base::SimpleString>(
  ...
  LOGI << filename + " has " +
              std::to_string(protein_structure->get_num_residues()) +
              " protein residue(s) in " +
              std::to_string(protein_structure->get_num_chains()) + " chain(s)";
  LOGI << filename + " has " +
              std::to_string(small_molecules->get_num_residues()) +
              " small_molecules";
  LOGI << filename + " has " +
              std::to_string(rna_structure->get_num_residues()) +
              " RNA residue(s) in " +
              std::to_string(rna_structure->get_num_chains()) + " chain(s)";
  LOGI << filename + " has " + std::to_string(bps.size() + end_indexes.size()) +
              " basepairs with " + std::to_string(end_indexes.size()) +
              " basepair ends, sites to connect other structure too";
