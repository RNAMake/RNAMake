...
CIFParser::_get_entity_ids(String const &header_contents) {
  ...
  while (std::getline(ss, line, '\n')) {
    if (line[0] == '_' && line.find(".") != std::string::npos) {
      const auto key = base::string::trim(base::string::split(line, ".")[1]);
      headers[key] = index++;
  ...
  if (num_entries * headers.size() != tks.size()) {
    const auto msg = String{"header contents are mis-sized, mal-formed or the "
  ...
  for (auto index = 0; index < num_entries; ++index) {
    ...
    if (chain_name.find("polyribonucleotide") != std::string::npos ||
      ...
      allowed_ids.insert(std::stoi(tks[index * headers.size() + id_index]));
    }
  }
  return allowed_ids;
}
...
CIFParser::_get_atoms(String const &atom_info,
  ...
  // pretty dumb looking but need to identify each of the indices needed up
  // front
  ...
  for (auto &entry : atom_contents) {
    ...
    resname = base::string::trim(resname);
    ...
    if (allowed_chains.find(id) == allowed_chains.end()) {
      ...
      name = base::string::trim(name);
      residue_atoms[key].emplace_back(std::make_shared<structure::Atom>(
          name, math::Vector3(std::stod(tks[x_index]), std::stod(tks[y_index]),
...
ResidueOPs const &CIFParser::parse(String const &cif_file, int protein, int rna,
                                   int others) {
  ...
  // 5. Contstruct Residues from the atom groupings... TODO this part of the
  ...
  if (!infile.is_open()) {
    const auto msg =
        String{"Error opening file: " + cif_file + ". Are you sure it exists?"};
    throw CIFParseError(msg);
  }
  ...
  for (auto &kv : contents) {
    if (kv.second.empty()) {
      auto msg =
          String{"Required section " + kv.first + " not found in CIF file " +
                 cif_file +
                 ". This file is likely ill formed and/or a non-ASCII file."};
      throw CIFParseError(msg);
    }
  }
  ...
  for (auto &kv : residue_atoms) {
    const auto res_tokens = base::string::split(kv.first, "|");
    ResidueType rtype;
    ...
    if (!protein && rtype.set_type() == SetType::PROTEIN &&
        res_tokens[0].size() > 1) {
      continue;
    }
    if (!rna && rtype.set_type() == SetType::RNA) {
      continue;
    }

    residues_.emplace_back(std::make_shared<Residue>(
        rtype, res_tokens[0], std::stoi(res_tokens[1]), res_tokens[2],
        res_tokens[3]));
    (*residues_.rbegin())
        ->setup_atoms(kv.second); // that's uglyyyy, but it works :). TODO take
                                  // atoms at ctor
  }
  return residues_;
}
...
ResidueType CIFParser::_get_new_residue_type(String const &res_name,
