...
BasepairOPs structure::RNAStructure::get_basepair(util::Uuid const &bp_uuid) {
  BasepairOPs bps;
...
BasepairOPs structure::RNAStructure::get_basepair(ResidueOP const &res1,
...
BasepairOPs structure::RNAStructure::get_basepair(util::Uuid const &uuid1,
...
BasepairOPs structure::RNAStructure::get_basepair(String const &name) {
  Strings name_spl = base::string::split(name, "-");
...
util::Beads const &
structure::RNAStructure::get_beads(BasepairOPs const &excluded_ends) {
...
util::Beads const &
structure::RNAStructure::get_beads(BasepairOP const &excluded_end) {
...
int structure::RNAStructure::get_end_index(BasepairOP const &end) {
  if (std::find(ends_.begin(), ends_.end(), end) == ends_.end()) {
    throw structure::RNAStructureException(
        "cannot find end: " + end->get_name_str() + " in ends ");
  }
...
int structure::RNAStructure::get_end_index(String const &end_name) {
  for (int i = 0; i < ends_.size(); i++) {
    if (ends_[i]->get_name_str() == end_name) {
      return i;
    }
  }

  throw structure::RNAStructureException(
      "could not find a end basepair with name: " + end_name);
...
String const structure::RNAStructure::to_pdb_str(int renumber,
                                                 int close_chains) {
  return structure_->to_pdb_str(renumber);
}

void structure::RNAStructure::to_pdb(String const fname, int renumber,
                                     int close_chains, int conect_statements) {
...
std::shared_ptr<BasepairOPs> end_from_basepairs(StructureOP const &s,
  ...
  for (auto const &r : chain_ends) {
    res_map[r->get_uuid()] = r;
  }

  auto ends = std::make_shared<BasepairOPs>();
  for (auto const &bp : bps) {
    if (bp->get_bp_type() != "cW-W") {
      continue;
    }
...
subselect_basepairs_with_res(ResidueOPs const &res,
