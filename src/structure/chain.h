...
public:
  bool operator==(Chain const &c) const { return is_equal(c); }
...
public: // non const methods
  ...
  void transform(math::Matrix const &r, math::Vector const &t,
                 math::Point &dummy) {
...
public:
  ...
  inline String get_pdb_str(int acount = 1) const {
    return get_pdb_str(acount, get_first().get_num(),
                       get_first().get_chain_id());
  }

  void write_pdb(String const &f_name) const {
