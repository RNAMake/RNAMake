...
struct PDBParserResidues {
  ...
  inline bool has_protein() {
    if (protein_residues.size() > 0) {
      return true;
    } else {
      return false;
    }
  ...
  inline bool has_small_molecules() {
    if (small_molecule_residues.size() > 0) {
      return true;
    } else {
      return false;
    }
...
public:
  PDBParser(ResidueTypeSet const &rts)
      : _rts(rts), _atom_name_corrections(std::map<String, String>()),
...
public:
  PDBParserResiduesOP parse(String const &) const;

private:
  void _parse_atoms_from_pdb_file(String const &,
  ...
  bool _replace_missing_phosphate_backbone(std::vector<Atom const *> &,
                                           ResidueTypeCOP) const;
