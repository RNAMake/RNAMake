...
String Segment::get_pdb_str(int &acount, int &rnum, char &chain_id) {
  // TODO add both proteins and small molecules
  return structure_.get_pdb_str(acount, rnum, chain_id);
}

void Segment::write_pdb(String const &fname) const {
  std::ofstream out;
...
void Segment::write_steric_beads_to_pdb(String const &fname) {
