...
BasepairState str_to_basepairstate(String const &s) {

  // always d,r,sugars order most be kept!!

  Strings strs = base::string::split(s, ";");
  if (strs.size() < 3) {
    throw "cannot load BasepairState from String, not the right number of "
          "elements\n";
  }

  math::Vector3 d = math::Vector3(strs[0]);
  math::Matrix3x3 r = math::matrix_from_str(strs[1]);
  math::Vector3s sug = math::vectors_from_str(strs[2]);

  BasepairState bp(d, r, sug);
  return bp;
}

std::ostream &operator<<(std::ostream &stream, const BasepairState &bpstate) {
...
float get_bpstate_rotation_diff(BasepairState const &bp1,
...
int are_basepair_states_equal(BasepairState const &a, BasepairState const &b) {
  ...
  for (int i = 0; i < 2; i++) {
    if (!math::are_xyzVector_equal(a.sugars()[i], b.sugars()[i])) {
      return 0;
    }
  }
