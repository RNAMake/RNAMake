...
public:
  inline Structure() : ParentClass() {}

  inline Structure(Residues const &residues, Cutpoints const &cut_points)
      : ParentClass(residues, cut_points) {}
  inline Structure(Structure const &structure)
      : ParentClass(structure._residues, structure._cut_points) {}
  inline Structure(String const &s, ResidueTypeSet const &rts) : ParentClass() {

    auto spl_chain = base::string::split(s, ":");
    for (Index i = 0; i < spl_chain.size() - 1; i++) {
      if (i != 0) {
        _cut_points.push_back(_residues.size() + 1);
      }
      auto spl = base::string::split(spl_chain[i], ";");
    ...
    //          std::cout << "This the sql string in structure: " << s <<
    //          std::endl; for(Index i = 0; i < spl.size()-1; i++) {
...
public: // operators
  inline bool operator==(Structure const &s) const { return is_equal(s); }
...
public:
  bool is_equal(Structure const &s, bool check_uuid = true) const {
    ...
    for (int i = 0; i < _residues.size(); i++) {
      if (!(_residues[i].is_equal(s._residues[i], check_uuid))) {
        return false;
      }
    }
...
public: // non const
  ...
  inline void remove_residue_beads(util::Uuid const &r_uuid) {
...
public: // getters
  ...
  inline String get_pdb_str(int acount = 0) const {
...
int are_residues_connected_RNA(Residue const &, Residue const &);

int are_residues_connected_protein(Residue const &, Residue const &);

int are_residues_connected(Residue const &, Residue const &);

ResidueOP _get_first_residues_in_chain(ResidueOPs const &,
                                       std::map<ResidueOP, int> const &);

ResidueOP _get_next_residue(ResidueOPs const &,
                            std::map<ResidueOP, int> const &);

StructureOP get_structure_from_residues(ResidueOPs const &);

StructureOP get_structure_from_pdb(String const &, ResidueTypeSet const &,
                                   SetType);

base::VectorContainerOP<Basepair>
get_basepairs_from_x3dna(util::X3dna::X3Basepairs const &, Structure const &);
