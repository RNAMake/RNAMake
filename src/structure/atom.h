...
public:
  ...
  inline Atom(String const &s) {
    ...
    _name = spl[0];
    _coords = math::Vector3(std::stof(spl[1]), std::stof(spl[2]),
                          std::stof(spl[3]));
  }
...
public:
  ...
  /**
       ...
       * @endcode
       */
  inline void move(math::Point const &p) { _coords = _coords + p; }
  ...
  inline void set_coords(math::Point const &coords) { _coords = coords; }
...
public://accessors
  ...
  /**
       ...
       */
  inline math::Point const &get_coords() const { return _coords; }
