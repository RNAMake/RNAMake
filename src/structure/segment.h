...
class Segment
    : public primitives::Segment<Basepair, Structure, Chain, Residue> {
public:
...
public:
  Segment(Structure const &structure, Basepairs const &basepairs,
          Indexes const &end_indexes, base::SimpleStringCOPs const &end_ids,
          base::SimpleStringCOP name, Structure const &proteins,
          Structure const &small_molecules, base::SimpleStringCOP dot_bracket,
          util::SegmentType segment_type, Index aligned_end_index,
          util::Uuid const &uuid)
      : BaseClass(structure, basepairs, end_indexes, end_ids, name,
                  segment_type, aligned_end_index, uuid),
        proteins_(proteins), small_molecules_(small_molecules),
        dot_bracket_(dot_bracket) {}

  Segment(Segment const &seg)
      : BaseClass(seg.structure_, seg.basepairs_, seg.end_indexes_,
                  seg.end_ids_, seg.name_, seg.segment_type_,
                  seg.aligned_end_index_, seg.uuid_),
        proteins_(seg.proteins_), small_molecules_(seg.small_molecules_),
        dot_bracket_(seg.dot_bracket_) {}

  Segment(String const &s, ResidueTypeSet const &rts)
      : BaseClass(), proteins_(Structure()), small_molecules_(Structure()) {

    auto spl = base::string::split(s, "&");
    auto structure = std::make_shared<structure::Structure>(spl[5], rts);
    auto basepair_str = base::string::split(spl[6], "@");
    basepairs_ = Basepairs();
    end_indexes_ = Indexes();

    for (auto const &bp_str : basepair_str) {
      auto bp_spl = base::string::split(bp_str, ",");
      auto res_spl = base::string::split(bp_spl[0], "-");
      auto strs = base::string::split(bp_spl[1], ";");
      ...
      auto coords = base::string::split(bp_spl[1], ";");
      auto center = math::Vector3(coords[0]);
      auto ref = math::Matrix3x3(coords[1]);
      auto x3dna_type = util::get_x3dna_by_type(bp_spl[2]);
      auto bp = structure::Basepair(
          res1.get_uuid(), res2.get_uuid(), util::Uuid(),
          primitives::BasepairType::WC,
          std::make_shared<base::SimpleString const>(bp_spl[0]), x3dna_type,
          ref, center, sug);
      basepairs_.push_back(bp);
    }
    Strings end_indexes_strings_ = base::string::split(spl[7], " ");

    for (auto const &index : end_indexes_strings_) {
      end_indexes_.push_back(stoi(index));
    }

    Strings end_ids_strings_ = base::string::split(spl[8], " ");
  ...
  //          name_ = std::make_shared<base::SimpleString
  //          const>(j["name"].ToString()); dot_bracket_ =
  //          std::make_shared<base::SimpleString
  //          const>(j["dot_bracket"].ToString()); segment_type_ =
  //          static_cast<util::SegmentType>(j["segment_type"].ToInt());
  ...
  //              auto & r1 = get_residue(j_bps[i][1].ToInt(),
  //              (char)j_bps[i][2].ToInt(), (char)j_bps[i][3].ToInt()); auto &
  //              r2 = get_residue(j_bps[i][4].ToInt(),
  //              (char)j_bps[i][5].ToInt(), (char)j_bps[i][6].ToInt());
  //              basepairs_.push_back(Basepair(j_bps[i][0], r1.get_uuid(),
  //              r2.get_uuid(), util::Uuid()));
  //          }
  //
  ...
  //              end_ids_.push_back(std::make_shared<base::SimpleString
  //              const>(j_end_ids[i].ToString()));
  //          }
  //      }
...
public:
  ...
  inline bool is_protein_residue_start_of_chain(Residue const &r) const {
    return proteins_.is_residue_start_of_chain(r);
  }

  const_iterator small_molecules_begin() const {
    return small_molecules_.begin();
  }
  const_iterator small_molecules_end() const { return small_molecules_.end(); }

public:
  bool is_equal(Segment const &s, bool check_uuid = true) const {
    if (segment_type_ != s.segment_type_) {
      return false;
    }
    if (aligned_end_index_ != s.aligned_end_index_) {
      return false;
    }
    if (basepairs_.size() != s.basepairs_.size()) {
      return false;
    }
    if (end_indexes_.size() != s.end_indexes_.size()) {
      return false;
    }
    if (*name_ != *s.name_) {
      return false;
    }
    if (*dot_bracket_ != *s.dot_bracket_) {
      return false;
    }
    for (int i = 0; i < basepairs_.size(); i++) {
      if (!basepairs_[i].is_equal(s.basepairs_[i], check_uuid)) {
        return false;
      }
    }
    for (int i = 0; i < end_indexes_.size(); i++) {
      if (end_indexes_[i] != s.end_indexes_[i]) {
        return false;
      }
    }
    if (!structure_.is_equal(s.structure_, check_uuid)) {
      return false;
    }
    if (!proteins_.is_equal(s.proteins_, check_uuid)) {
      return false;
    }
    if (!small_molecules_.is_equal(s.small_molecules_, check_uuid)) {
      return false;
    }
    return true;
  }
...
public:
  bool steric_clash(Segment const &s) {
  ...
  int get_num_steric_clashes(Segment const &s) {
  ...
  secondary_structure::SegmentOP get_secondary_structure() const {
    auto dot_bracket_spl =
        base::string::split(dot_bracket_->get_str(), "&");
    ...
    for (auto const &r : structure_) {
      auto ss_r = secondary_structure::Residue(r.get_name(), dot_bracket_str[i],
                                               r.get_num(), r.get_chain_id(),
                                               r.get_i_code(), r.get_uuid());
    ...
    for (auto const &bp : basepairs_) {
      auto ss_bp = secondary_structure::Basepair(
          bp.get_res1_uuid(), bp.get_res2_uuid(), bp.get_uuid(),
          bp.get_bp_type(), bp.get_name());

      ss_bps.push_back(ss_bp);
    }

    return std::make_shared<secondary_structure::Segment>(
        ss_structure, ss_bps, end_indexes_, end_ids_, name_, segment_type_,
        aligned_end_index_, uuid_);
  }

public: // pdb functions
  String get_pdb_str(int &, int &, char &);

  auto get_js_str() const {
    ...
    str = "structure " + structure_.get_str() + "\n" + "basepairs " + bps_str +
          ...
          "\n" + "name " + name_->get_str() + "\n" + "proteins " +
          proteins_.get_str() + "\n" + "small_molecules " +
          small_molecules_.get_str() + "\n" + "dot_bracket " +
          dot_bracket_->get_str() + "\n" + "segment_type " +
          std::to_string((int)segment_type_) + "\n" + "aligned_end_index " +
          std::to_string(aligned_end_index_) + "\n";
  ...
  //          for(auto const & bp : basepairs_) {
  //              auto bp_res = get_bp_res(bp);
  //              j_bps.append(json::Array(bp.get_json(),
  //              bp_res->at(0).get_num(), bp_res->at(0).get_chain_id(),
  //                                       bp_res->at(0).get_i_code(),
  //                                       bp_res->at(1).get_num(),
  ...
  inline String get_pdb_str(int acount = 0) {
  ...
  void write_pdb(String const &) const;

  void write_steric_beads_to_pdb(String const &);

public:
  inline String get_dot_bracket() { return dot_bracket_->get_str(); }

  inline Basepair const &get_aligned_end() {
    return basepairs_[end_indexes_[aligned_end_index_]];
  }
