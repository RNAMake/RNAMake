...
SecondaryStructureChainGraphOP Parser::parse(String const &sequence,
  ...
  if (res.size() > 0) {
    _add_unpaired_residues_to_graph(g, res, 0);
  }

  return g;
}

void Parser::_add_unpaired_residues_to_graph(SecondaryStructureChainGraphOP &g,
...
void Parser::_add_paired_res_to_graph(SecondaryStructureChainGraphOP &g,
                                      ResidueOP const &r, int is_start_res) {
...
MotifOPs Parser::parse_to_motifs(String const &sequence,
                                 String const &dot_bracket) {
...
MotifOP Parser::parse_to_motif(String const &sequence,
                               String const &dot_bracket) {
...
SSNodeOP Parser::_walk_nodes(SSNodeOP const &n) {
...
MotifOP Parser::_build_motif(StructureOP const &struc) {
