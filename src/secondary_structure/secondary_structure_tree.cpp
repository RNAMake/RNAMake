...
struct TreePreNode {
  ...
  inline TreePreNode(MotifOP const &nm, int nparent_index,
                     int nparent_end_index)
      : m(nm), parent_index(nparent_index),
        parent_end_index(nparent_end_index) {}
};

SecondaryStructureTreeOP tree_from_pose(PoseOP const &p) {
  ...
  if (start_m == nullptr) {
    std::runtime_error("could not find start motif to build tree in "
                       "secondary_structure::tree_from_pose");
  }
  ...
  while (!open_nodes.empty()) {
    ...
    int pos = sst->add_motif(current.m, current.parent_index,
