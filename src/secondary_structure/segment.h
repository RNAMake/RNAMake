...
public:
  inline String get_dot_bracket() { return structure_.get_dot_bracket(); }

  inline void set_sequence(String const &sequence) {
...
public: // overrided
  base::SimpleStringCOP get_end_id(Index index) const {
    ...
    return primitives::Segment<Basepair, Structure, Chain, Residue>::get_end_id(
        index);
  }

private:
  void _update_end_ids() const {
    int i = -1;
    for (auto const &ei : this->end_indexes_) {
      i++;
      auto end_id = generate_end_id(this->structure_, this->basepairs_,
                                    this->basepairs_[ei]);
