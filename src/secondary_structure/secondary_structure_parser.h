...
public:
  Parser()
      : seen_(std::map<SSNodeOP, int>()), structure_(StructureOP()),
        residues_(ResidueOPs()), pairs_(BasepairOPs()) {}
...
private:
  ...
  ResidueOP _previous_res(ResidueOP const &r) {
    ...
    int i = (int)(std::find(residues_.begin(), residues_.end(), r) -
                  residues_.begin());
    if (i == 0) {
      return nullptr;
    } else {
      return residues_[i - 1];
    }
  }
