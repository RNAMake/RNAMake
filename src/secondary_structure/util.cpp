...
String assign_end_id(RNAStructureOP const &ss, BasepairOP const &end) {
  ...
  if (!all_chains.empty() && !open_chains.empty()) {
    all_chains.erase(
        std::remove(all_chains.begin(), all_chains.end(), open_chains.front()),
        all_chains.end());
  }
  ...
  while (!open_chains.empty()) {
    ...
    all_chains.erase(
        std::remove(all_chains.begin(), all_chains.end(), best_chain),
        all_chains.end());
  ...
  for (auto const &id_chain : id_chains) {
    ...
    for (auto const &e : id_chain.second) {
      if (e == '(') {
        ...
        throw Exception("unexpected symbol in dot bracket notation: " +
                        std::to_string(e));
      }
    }
    if (i != id_chains.size() - 1) {
      ss_id += "_";
    }
    i++;
  }
