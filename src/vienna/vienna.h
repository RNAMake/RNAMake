...
struct model_detailsT {
  int dangles;    /**<  \brief  Specifies the dangle model used in any energy
                   evaluation (0,1,2 or 3)    \note   Some function do not implement
                   all dangle model but only a subset of    (0,1,2,3). Read the
                   documentaion of the particular recurrences or    energy evaluation
...
struct paramT {
  ...
  model_detailsT
      model_details; /**<  \brief  Model details to be used in the recursions */
};
...
class Vienna {
 public:
  inline Vienna() {
    ...
    double sfact = 1.07;
    double kT =
        (betaScale * ((temperature + K0) * GASCONST)) / 1000.; /* in Kcal */
 ...
 private:  // Energy calculations
  ...
  inline int E_IntLoop(int n1, int n2, int type, int type_2, int si1, int sj1,
                       int sp1, int sq1) {
    ...
    if (ul == 0) { /* stack */
      ...
      if (us == 0) { /* bulge */
        ...
        if (ul == 2) { /* 2x1 loop */
          ...
          z = pf.expinternal[ul + us] * pf.expmismatch1nI[type][si1][sj1] *
        ...
        if (ul == 2) /* 2x2 loop */
          ...
          z = pf.expinternal[5] * pf.expmismatch23I[type][si1][sj1] *
              pf.expmismatch23I[type2][sq1][sp1];
          return z * pf.expninio[2][1];
        }
      }
      /* generic interior loop (no else here!)*/
      z = pf.expinternal[ul + us] * pf.expmismatchI[type][si1][sj1] *
