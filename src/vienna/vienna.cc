      ...
      for (i = 1; i <= n; i++) {
        ...
        for (j = i + TURN + 1; j <= n; j++) {
          ...
          if (type && (qb[ij] > 0.)) {
            ...
            tmp2 =
                exp_E_Hairpin(u, rt, S1[j + 1], S1[i - 1], loopseq) * scale[u];
            ...
            if ((i > TURN + 2) && (j < n - TURN - 1))
              tmp2 += qm[my_iindx[1] - i + 1] * qm[my_iindx[j + 1] - n] *
                      expMLclosing * exp_E_MLstem(type, S1[i - 1], S1[j + 1]);

            /* 1.3.2 Left part                      */
            for (k = TURN + 2; k < i - TURN - 2; k++)
              tmp2 += qm[my_iindx[1] - k] * qm1[jindx[i - 1] + k + 1] *
                      expMLbase[n - j] * expMLclosing *
                      exp_E_MLstem(type, S1[i - 1], S1[j + 1]);

            /* 1.3.3 Right part                      */
            for (k = j + TURN + 2; k < n - TURN - 1; k++)
              tmp2 += qm[my_iindx[j + 1] - k] * qm1[jindx[n] + k + 1] *
                      expMLbase[i - 1] * expMLclosing *
                      exp_E_MLstem(type, S1[i - 1], S1[j + 1]);
      ...
      for (i = 1; i <= n; i++) {
        ...
        for (j = i + TURN + 1; j <= n; j++) {
          ...
          if (type && (qb[ij] > 0.)) {
            ...
            probs[ij] *= exp_E_ExtLoop(type, (i > 1) ? S1[i - 1] : -1,
                                       (j < n) ? S1[j + 1] : -1);
          } else
            probs[ij] = 0.;
        }
      ...
      for (k = 1; k < l - TURN; k++) {
        ...
        for (i = MAX2(1, k - MAXLOOP - 1); i <= k - 1; i++)
          for (j = l + 1; j <= MIN2(l + MAXLOOP - k + i + 2, n); j++) {
            ...
            if (type && (probs[ij] > 0.)) {
              /* add *scale[u1+u2+2] */
              tmp2 += probs[ij] * (scale[k - i + j - l] *
                                   exp_E_IntLoop(k - i - 1, j - l - 1, type,
                                                 type_2, S1[i + 1], S1[j - 1],
                                                 S1[k - 1], S1[l + 1]));
            }
          }
        probs[kl] += tmp2;
      ...
      if (l < n)
        for (k = 2; k < l - TURN; k++) {
          ...
          for (j = l + 2; j <= n; j++) {
            ...
            tt = rtype[tt];
            if (tt)
              prmt += probs[ii - j] * exp_E_MLstem(tt, S1[j - 1], S1[i + 1]) *
                      qm[ll - (j - 1)];
          }
          kl = my_iindx[k] - l;
          tt = ptype[kl];
    ...
    for (i = j - TURN - 1; i >= 1; i--) {
      ...
      if (type != 0) {
        ...
        if (((type == 3) || (type == 4)) && noGUclosure)
          ...
          qbt1 = exp_E_Hairpin(u, type, S1[i + 1], S1[j - 1], s + i - 1) *
                 scale[u + 2];
        }
          ...
          for (l = MAX2(k + TURN + 1, j - 1 - MAXLOOP + u1); l < j; l++) {
            ...
            if (type_2) {
              ...
              qbt1 += qb[my_iindx[k] - l] *
                      (scale[u1 + j - l + 1] *
                       exp_E_IntLoop(u1, j - l - 1, type, type_2, S1[i + 1],
                                     S1[j - 1], S1[k - 1], S1[l + 1]));
            }
          }
        }
      ...
      if (type) {
        qbt1 =
            qb[ij] * exp_E_MLstem(type, ((i > 1) || circular) ? S1[i - 1] : -1,
      ...
      if (type) {
        qbt1 += qb[ij];
        qbt1 *= exp_E_ExtLoop(type, ((i > 1) || circular) ? S1[i - 1] : -1,
                              ((j < n) || circular) ? S1[j + 1] : -1);
      }
    ...
    if (ml == 0) { /* backtrack in f5 */
      ...
      if ((dangle_model == 3) && (k > j - 2 - TURN)) { /* must be coax stack */
        ...
        for (k = i + 1 + TURN; k <= j - 2 - TURN; k++) {
          ...
          if (type && type_2)
            if (fij == c[indx[k] + i] + c[indx[j] + k + 1] +
                           params.stack[type][type_2] + 2 * params.MLintern[1])
              break;
        }
      }
      sector[++s].i = i;
...
void Vienna::parenthesis_structure(int length) {
  ...
  for (k = 1; k <= base_pair2[0].i; k++) {
    ...
    // std::endl;
    if (base_pair2[k].i ==
        base_pair2[k].j) { /* Gquad bonds are marked as bp[i].i == bp[i].j */
