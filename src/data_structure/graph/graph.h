...
template <typename DataType> class GraphIterator;
...
public:
  ...
  inline GraphNodeOP<DataType> const &get_node(int index) const {
  ...
  inline GraphNodeOP<DataType> oldest_node() {
    auto node = last_node_;
    assert(node != nullptr &&
           "attemped to call oldest_node but there are no nodes");
...
public: // getters
  inline GraphNodeOPs<DataType> const &nodes() const { return nodes_; }

  inline GraphConnectionOPs<DataType> const &connections() const {
    return connections_;
  }

  inline GraphNodeOP<DataType> const &last_node() { return last_node_; }
...
public: // setters
  inline void index(int nindex) { index_ = nindex; }
...
private:
  ...
  GraphIterator(GraphNodeOPs<DataType> const &nodes) : nodes_(nodes) {
    if (nodes_.size() == 0) {
      current_ = nullptr;
      return;
    }
  ...
  GraphIterator(GraphNodeOP<DataType> const &node)
      : nodes_(GraphNodeOPs<DataType>()), queue_(GraphNodeQueue<DataType>()),
        seen_(std::map<GraphNodeOP<DataType>, int>()),
        leafs_(GraphNodeOPs<DataType>()), current_(node) {
    seen_[current_] = 1;
  }
};

template <typename DataType>
typename Graph<DataType>::iterator Graph<DataType>::begin() {
  return iterator(nodes_);
}

template <typename DataType>
typename Graph<DataType>::iterator Graph<DataType>::end() {
  return iterator(empty_);
}

template <typename DataType>
typename Graph<DataType>::iterator
Graph<DataType>::transverse(GraphNodeOP<DataType> const &node) const {
  return iterator(node);
}

template <typename DataType>
typename Graph<DataType>::const_iterator Graph<DataType>::begin() const {
  return iterator(nodes_);
}

template <typename DataType>
typename Graph<DataType>::const_iterator Graph<DataType>::end() const {
  return iterator(empty_);
}

template <typename DataType>
GraphNodeOP<DataType> &GraphIterator<DataType>::operator*() {
  return current_;
}

template <typename DataType>
GraphIterator<DataType> &GraphIterator<DataType>::operator++() {
...
template <typename DataType>
bool GraphIterator<DataType>::operator==(
...
template <typename DataType> class GraphDynamic : public Graph<DataType> {
public:
...
public:
  inline int add_data(DataType const &data, int parent_index = -1,
                      int orphan = 0) {

    GraphNodeOP<DataType> parent = this->last_node_;
    if (parent_index != -1) {
      parent = this->get_node(parent_index);
    }

    auto n = std::make_shared<GraphNodeDynamic<DataType>>(data, this->index_,
                                                          this->level_);
...
template <typename DataType> class GraphStatic : public Graph<DataType> {
public:
  inline GraphStatic() : Graph<DataType>() {}

  inline GraphStatic(GraphStatic<DataType> const &g) {
...
public:
  inline int add_data(DataType const &data, int parent_index = -1,
                      int parent_pos = -1, int child_pos = -1,
                      int n_children = 0, int orphan = 0, int index = -1) {
    ...
    auto n = std::make_shared<GraphNodeStatic<DataType>>(
        data, given_index, this->level_, n_children);
    ...
    if (parent != nullptr) {
      ...
      auto c = std::make_shared<GraphConnection<DataType>>(
          parent, n, parent_pos, child_pos);
  ...
  inline void connect(int i, int j, int i_pos, int j_pos) {
  ...
  inline int check_pos_is_valid(GraphNodeOP<DataType> const &n, int &pos) {

    if (pos == -1) {
      Indexes avail_pos = n->available_children_pos();
      if (avail_pos.size() == 0) {
        throw GraphException(
            "cannot add connection to node, has not available ends");
      }
      return avail_pos[0];
    } else {
  ...
  inline Indexes get_available_pos(GraphNodeOP<DataType> const &n, int &pos) {
  ...
  inline void remove_node(int pos) {
    ...
    for (auto c : n->connections()) {
      ...
      this->connections_.erase(
          std::remove(this->connections_.begin(), this->connections_.end(), c));
      c->disconnect();
    }

    this->nodes_.erase(std::remove(this->nodes_.begin(), this->nodes_.end(), n),
                       this->nodes_.end());

    if (this->nodes_.size() != 0) {
      this->last_node_ = this->nodes_.back();
    } else {
      this->last_node_ = nullptr;
    }
  }

  inline void remove_level(int level) {
