...
template <typename DataType, typename AdjacencyListType> class IterList {
public:
  struct VisitedNode {
...
public:
  IterList()
      : iter_list_(std::vector<Node<DataType> *>()), open_(std::queue<Index>()),
        seen_(std::map<Index, int>()) {
    iter_list_.reserve(100);
  }

public:
  typedef typename std::vector<Node<DataType> const *>::const_iterator
      const_iterator;
...
public:
  virtual void transversal(AdjacencyListType &adj_list, Index start_n) {
    iter_list_.resize(0);
  ...
  virtual void path_transversal(AdjacencyListType &adj_list, Index start_n,
                                Index end_n) {
    iter_list_.resize(0);
...
protected:
  virtual void get_neighbors(Index ni, AdjacencyListType &adj_list,
    ...
    for (auto const &e : edges) {
      ...
      if (seen_.find(e->partner(ni)) != seen_.end()) {
        continue;
      }
  ...
  virtual void get_neighbors_path(VisitedNodeOP vn, AdjacencyListType &adj_list,
                                  std::vector<Index> &neighbors) {
    neighbors.resize(0);
    ...
    for (auto const &e : edges) {
      ...
      if (vn->index_in_path(e->partner(vn->index))) {
        continue;
      }
...
public:
  virtual void transversal(AdjacencyListType &adj_list, Index start_n) {
    ...
    while (found) {
      found = 0;
      for (auto const &kv : adj_list) {
        if (this->seen_.find(kv.first) != this->seen_.end()) {
          continue;
        }
  ...
  void sub_graph_transversal(AdjacencyListType &adj_list, Index start_n,
                             Index end_n) {
...
protected:
  virtual void get_neighbors(Index ni, AdjacencyListType &adj_list,
    ...
    for (auto const &e : edges) {
      ...
      if (adj_list.has_parent(partner) &&
          adj_list.get_parent_index(partner) == ni) {
        if (this->seen_.find(e->partner(ni)) != this->seen_.end()) {
          continue;
        }
        neighbors.push_back(e->partner(ni));
  ...
  virtual void get_neighbors_path(VisitedNodeOP vn, AdjacencyListType &adj_list,
    ...
    for (auto const &e : edges) {
      ...
      if (adj_list.has_parent(partner) &&
          adj_list.get_parent_index(partner) == vn->index) {
        if (vn->index_in_path(e->partner(vn->index))) {
          continue;
        }
        neighbors.push_back(e->partner(vn->index));
