...
public:
    AdjacencyList() :
            ...
            nodes_(std::map<Index, Node < DataType>>()),
            index_(0) {}
    ...
    AdjacencyList(
        ...
        for (auto const & kv : nodes_) {
            edges_[kv.first] = std::vector < Edge const * > (abj_list.get_node_edges(kv.first).size());
        }
        for (auto const & kv : abj_list.edges_) {
            for (auto const & e : kv.second) {
                if (e == nullptr) { continue; }
                if (edge_between_nodes(e->node_i, e->node_j)) { continue; }
                add_edge(NodeIndexandEdge{e->node_i, e->edge_i}, NodeIndexandEdge{e->node_j, e->edge_j});
            }
        }
    }
...
public:
    ...
    add_edge(
        ...
        if (!edge_index_empty(nie1.node_index, nie1.edge_index)) {
            throw GraphException(
                    "cannot add edge, " + nie1.to_str() + " already is connected to -> " +
                    get_connected_node_info(nie1).to_str());
        }
        if (!edge_index_empty(nie2.node_index, nie2.edge_index)) {
            throw GraphException(
                    "cannot add edge, " + nie2.to_str() + " already is connected to -> " +
                    get_connected_node_info(nie2).to_str());
        }

        edges_[nie1.node_index][nie1.edge_index] = new Edge(nie1.node_index, nie2.node_index,
                                                            nie1.edge_index, nie2.edge_index);
        edges_[nie2.node_index][nie2.edge_index] = new Edge(nie1.node_index, nie2.node_index,
                                                            nie1.edge_index, nie2.edge_index);
    }
    ...
    remove_edge(
        ...
        if (edge_index_empty(nie1.node_index, nie1.edge_index)) {
            throw GraphException("cannot remove edge, " + nie1.to_str() + " it is not filled");
        }
        if (edge_index_empty(nie2.node_index, nie2.edge_index)) {
            throw GraphException("cannot remove edge, " + nie2.to_str() + " it is not filled");
        }
...
public:
    ...
    get_connected_node_info(
        ...
        expects<GraphException>(
                edges_.find(nei.node_index) != edges_.end() ||
                edges_.find(nei.node_index)->second.size() > nei.edge_index - 1,
...
public:
    ...
    edge_index_empty(
        ...
        if (edges_.find(ni)->second[ei] == nullptr) { return true; }
        else { return false; }
    }

protected:
    void
    _update_dyanmic_edges(
            NodeIndexandEdge const & nie) {
        expects<GraphException>(
                edges_.find(nie.node_index) != edges_.end(),
                "node with index: " + std::to_string(nie.node_index) + " does not exist!");
    ...
    _update_next_index() {
        auto largest = 0;
        for (auto const & kv : nodes_) {
            if (kv.first > largest) { largest = kv.first; }
        }
        index_ = largest + 1;
    }
...
public:
    ...
    add_node(
        ...
        expects<GraphException>(
                ...
                "cannot add node to graph, parent with index: " + std::to_string(pie.node_index) +
                " does not exist");
