...
public:
  inline TreeNodeOP<DataType> const &get_node(int index) {
  ...
  inline void remove_node(TreeNodeOP<DataType> const &n) {
    if (n->parent() != nullptr) {
  ...
  inline void remove_node(int pos) { return remove_node(get_node(pos)); }
...
public: // getters
  ...
  inline TreeNodeOP<DataType> const &last_node() { return last_node_; }
...
template <typename DataType> class TreeDynamic : public Tree<DataType> {
public:
...
public:
  inline int add_data(DataType const &data, int parent_index = -1) {
    ...
    auto n = std::make_shared<TreeNodeDynamic<DataType>>(data, this->index_,
                                                         this->level_);
...
template <typename DataType> class TreeStatic : public Tree<DataType> {
public:
  TreeStatic() : Tree<DataType>() {}

  TreeStatic(TreeStatic<DataType> const &t) {

    for (auto const &n : t) {
      auto n_new = std::make_shared<TreeNodeStatic<DataType>>(
          n->data(), n->index(), n->level(), n->children().size());
      if (n->parent() != nullptr) {
...
public:
  inline int add_data(DataType const &data, int n_children = 1,
                      int parent_index = -1, int parent_child_index = -1) {

    auto parent = this->last_node_;
    if (parent_index != -1) {
      parent = this->get_node(parent_index);
    }
    auto n = std::make_shared<TreeNodeStatic<DataType>>(
        data, this->index_, this->level_, n_children);
    ...
    if (parent != nullptr) {
      if (parent_child_index == -1) {
        ...
        if (all_pos.size() == 0) {
          throw TreeException(
              "cannot add child to this parent it has no open spots");
        }
  ...
  inline Indexes get_available_pos(TreeNodeOP<DataType> const &n,
                                   int pos = -1) {
...
public:
  inline void index(int n_index) { this->index_ = n_index; }
};
