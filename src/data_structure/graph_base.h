...
public:
  inline Node(DataType const &data, Index const index)
...
struct NodeIndexandEdge {
  ...
  String to_str() const {
    return "node_index: " + std::to_string(node_index) +
           " edge_index: " + std::to_string(edge_index);
  }
};

struct NodeIndexandEdgeCompare {
  bool operator()(data_structure::NodeIndexandEdge const &nie1,
...
struct Edge {
  ...
  inline Edge(Index nnode_i, Index nnode_j, Index nedge_i, Index nedge_j)
      : node_i(nnode_i), node_j(nnode_j), edge_i(nedge_i), edge_j(nedge_j) {}
  ...
  Index partner(Index index) const {
    expects<GraphException>(index == node_i || index == node_j,
  ...
  Index end_index(Index index) const {
    expects<GraphException>(index == node_i || index == node_j,
