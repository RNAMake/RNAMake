...
template <typename SegmentType> struct SegmentMergerResult {
  inline SegmentMergerResult(
      std::shared_ptr<SegmentType> nsegment,
      std::map<util::Uuid, util::Uuid> const &nres_uuid_map)
      : segment(nsegment), res_uuid_map(nres_uuid_map) {}
...
public:
  ...
  //
  SegmentMergerException(String const &message) : std::runtime_error(message) {}
};

template <typename SegmentType, typename ChainType, typename ResType,
          typename BasepairType, typename AlignerType>
class SegmentMerger {
public:
  struct ChainNodeData {
    inline ChainNodeData(ChainType const &nchain, util::Uuid const &nm_uuid,
                         bool nprime5_override, bool nprime3_override)
        : chain(std::move(nchain)), m_uuid(nm_uuid),
          prime5_override(nprime5_override), prime3_override(nprime3_override) {
    }
...
public:
  ...
  public:
    SegmentMerger(resources::ResourceManager const &rm)
        : rm_(rm), end_chain_ids_1_(Indexes(2)), end_chain_ids_2_(Indexes(2)),
          res_uuid_map_(std::map<util::Uuid, util::Uuid>()) {}

    ~SegmentMerger() {}

  public:
    SegmentMergerResultOP merge(SegmentGraph<SegmentType, AlignerType> const &g,
  ...
  protected:
    void _add_segment(SegmentType const &sg) {
    ...
    void _add_segment(SegmentType const &sg, BasepairType const &sg_end,
    ...
    void _link_segments(SegmentType const &sg, BasepairType const &sg_end,
                        SegmentType const &parent,
                        BasepairType const &parent_end) {
    ...
    void _get_end_chains(BasepairType const &end, Indexes &end_chain_ids) {
      end_chain_ids[0] = -1;
      end_chain_ids[1] = -1;

      for (auto const &n : chain_graph_) {
        if (n->data().chain.get_first().get_uuid() == end.get_res1_uuid() &&
            end_chain_ids[0] == -1) {
          end_chain_ids[0] = n->index();
        } else if (n->data().chain.get_first().get_uuid() ==
                   end.get_res1_uuid()) {
          throw SegmentMergerException(
              "end res1 is mapped to two different chains!");
        }

        if (n->data().chain.get_last().get_uuid() == end.get_res2_uuid() &&
            end_chain_ids[1] == -1) {
          end_chain_ids[1] = n->index();
        } else if (n->data().chain.get_last().get_uuid() ==
                   end.get_res2_uuid()) {
          throw SegmentMergerException(
              "end res2 is mapped to two different chains!");
        }
      }
    ...
    MergerType _assign_merger_type(SegmentType const &sg) {
      ...
      // ideal helices
      if (sg.get_name_str().length() > 10 &&
          sg.get_name_str().substr(0, 11) == "HELIX.IDEAL") {
        return MergerType::NON_SPECIFIC_SEQUENCE;
      }
      // flex helices
      if (sg.get_name_str().length() > 9 &&
          sg.get_name_str().substr(0, 10) == "HELIX.FLEX") {
        return MergerType::NON_SPECIFIC_SEQUENCE;
      }

      // bp step
      if (sg.get_name_str().length() > 2 &&
          sg.get_name_str().substr(0, 2) == "BP") {
        return MergerType::NON_SPECIFIC_SEQUENCE;
      }
    ...
    void _link_chains(Indexes dominant_indexes, Indexes auxiliary_indexes,
                      MergerType sg_type_1, MergerType sg_type_2) {
      ...
      if (dominant_indexes[0] == dominant_indexes[1]) {
        ...
        _connect_chains(dominant_indexes[0], auxiliary_indexes[0], 1, 0,
                        sg_type_1, sg_type_2);
        _connect_chains(dominant_indexes[0], auxiliary_indexes[1], 0, 1,
                        sg_type_1, sg_type_2);
        ...
        _connect_chains(dominant_indexes[1], auxiliary_indexes[0], 1, 0,
                        sg_type_1, sg_type_2);
        _connect_chains(dominant_indexes[0], auxiliary_indexes[0], 0, 1,
                        sg_type_1, sg_type_2);
      }
    ...
    void _connect_chains(Index d_index, Index a_index, Index d_end_index,
                         Index a_end_index, MergerType sg_type_1,
                         MergerType sg_type_2) {

      // auxiliary chain(a_index) first or
      //     last residue will be overritten by the dominant chain during the
      //     final
      //         merger
      if (a_end_index == 0) {
        chain_graph_.get_node_data(a_index).prime5_override = 1;
        res_uuid_map_
            [chain_graph_.get_node_data(a_index).chain.get_first().get_uuid()] =
                chain_graph_.get_node_data(d_index).chain.get_last().get_uuid();
      } else {
        chain_graph_.get_node_data(a_index).prime3_override = 1;
        res_uuid_map_[chain_graph_.get_node_data(a_index)
                          .chain.get_last()
                          .get_uuid()] =
            chain_graph_.get_node_data(d_index).chain.get_first().get_uuid();
      }

      chain_graph_.add_edge(
          data_structure::NodeIndexandEdge{d_index, d_end_index},
    ...
    void _log_overriding_specific_squence_warning() {
      LOGW << "Merging two chains with specific sequences this will remove
              either the 5 ' or 3' of "; LOGW << " residue of a specified
                  segments.This likely is not what you wanted to do !!!";
    }

    void _get_rna_residues_and_cutpoints(std::vector<ResType> &res,
                                         Cutpoints &cutpoints) {
      ...
      for (auto const &si : start_indexes) {
        ...
        while (true) {
          ...
          if (cur->data().prime5_override) {
            start = 1;
          }
          if (cur->data().prime3_override) {
            end -= 1;
          }
          ...
          if (chain_graph_.edge_index_empty(cur->index(), 1)) {
            break;
          }
...
typedef segment_data_structure::SegmentMerger<Segment, Chain, Residue, Basepair,
                                              Aligner>
    _SegmentMerger;
...
public:
  SegmentMerger(resources::ResourceManager const &rm) : BaseClass(rm) {}

  SegmentOP _build_structure(SegmentGraph const &g, String const &merged_name) {
    ...
    for (auto bp : all_bps_) {
      ...
      if (res_uuids.find(bp->get_res2_uuid()) == res_uuids.end()) {
        continue;
      }
    ...
    auto seg =
        std::make_shared<Segment>(s, basepairs, end_indexes, end_ids, name,
...
public:
  SegmentOP _build_structure(SegmentGraph const &g, String const &merged_name) {
    ...
    for (auto const &n : g) {
      for (auto it = n->data().protein_begin(); it != n->data().protein_end();
           it += 1) {
      ...
      if (protein_cutpoints.size() != 0 &&
          protein_cutpoints.back() != protein_res.size()) {
        protein_cutpoints.push_back(protein_res.size());
      }

      for (auto it = n->data().small_molecules_begin();
           it != n->data().small_molecules_end(); it += 1) {
    ...
    return rm_.segment_from_components(
        merged_name, s, basepairs, Structure(protein_res, protein_cutpoints),
