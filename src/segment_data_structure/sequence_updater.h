...
public:
  SequenceUpdater(resources::ResourceManager const &rm)
      : sm_(SegmentMerger(rm)) {}

  ~SequenceUpdater() {}

public:
  SegmentGraphOP get_updated_graph(SegmentGraph const &sg, String const &seq) {
  ...
  void update_graph(SegmentGraph &sg, String const &seq) {
    auto smr = sm_.merge(sg, "merged_graph");
    merged_seg_ = smr->segment;
    merged_seg_->set_sequence(seq);
...
private:
  void _update_graph_from_merged_segment(
      SegmentGraph &sg, SegmentOP seg,
...
public:
  SequenceUpdater(resources::ResourceManager const &rm)
      : _rm(rm), _ss_su(secondary_structure::SequenceUpdater(rm)) {}
...
public:
  SegmentGraphOP get_updated_graph(SegmentGraph const &sg, String const &seq) {
    ...
    _ss_su.update_graph(*ss_sg, seq);
    for (auto const &n : *ss_sg) {
      // std::cout << n->data().get_sequence() << std::endl;
      auto seg = _rm.get_segment(
          StringStringMap{{"end_id", n->data().get_end_id(0)->get_str()}});
...
private:
  resources::ResourceManager const &_rm;
  secondary_structure::SequenceUpdater _ss_su;
};
