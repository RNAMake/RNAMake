...
//     if(ENOENT == errno) { throw ResourceBuilderException("motif helix path
//     does not exist!"); }
//
//     auto lib_path = base::resources_path() +
//     "/motif_libraries/ideal_helices.db";
//
...
//         auto segs = seg_f_.all_segments_from_pdb(path,
//         util::SegmentType::HELIX, false);
//
...
void ResourceBuilder::build_basic_libraries() {
  auto paths = std::map<String, util::SegmentType>{
      {_motif_dirs_path + "/two_ways/", util::SegmentType::TWOWAY_JUNCTION},
      {_motif_dirs_path + "/hairpins/", util::SegmentType::HAIRPIN},
      {_motif_dirs_path + "/helices/", util::SegmentType::HELIX},
      {_motif_dirs_path + "/junctions/", util::SegmentType::NWAY_JUNCTION},
      {_motif_dirs_path + "/tertiary_contact_hairpin_hairpin/",
       util::SegmentType::TC_HAIRPIN_HAIRPIN},
      {_motif_dirs_path + "/tertiary_contact_junction_hairpin/",
       util::SegmentType::TC_JUNCTION_HAIRPIN},
      {_motif_dirs_path + "/tertiary_contact_junction_junction/",
       util::SegmentType::TC_JUNCTION_JUNCTION},
      {_motif_dirs_path + "/basepair_steps/", util::SegmentType::HELIX}};
  ...
  auto lines = base::path::get_lines_from_file(path);
  for (auto const &l : lines) {
    if (l.size() < 10) {
      continue;
    }
    auto spl = base::string::split(l, ",");
...
/*
void ResourceBuilder::_insert_segment_to_motif_table(
    structure::Segment const &seg, int id, util::sqlite::Connection &conn) {
  ...
  conn.bind(1, blob_);
  insert_str =
      start_insert_str_ + base::quoted_string(std::to_string(id)) + ", ?, ";
  insert_str += base::quoted_string(seg.get_name_str()) + ", ";
  insert_str +=
      base::quoted_string(seg.get_end(0).get_name()->get_str()) + ", ";
...
/*
void ResourceBuilder::_build_basic_library(
    String const &path, util::SegmentType seg_type,
    std::map<String, int> const &excluded_motifs) {

  auto lib_type = base::filename(path);
  auto lib_path =
      base::resources_path() + "/motif_libraries/" + lib_type + ".db";
  ...
  while ((entry = readdir(pDIR)) != NULL) {
    ...
    if (excluded_motifs.find(fname) != excluded_motifs.end()) {
      continue;
    }
