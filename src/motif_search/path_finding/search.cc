...
void Search::setup(motif_search::ProblemOP p) {
...
motif_search::SolutionOP Search::next() {
  ...
  while (!queue_.empty()) {
    ...
    if (score < best) {
      ...
      LOG_VERBOSE << "best_score=" << best
                  << " motifs_in_solution=" << current->level()
                  << " steps=" << steps;
    }
...
motif_data_structure::MotifStateGraphOP Search::_graph_from_node(NodeOP node) {
  auto current = node;
  auto nodes = NodeOPs();
...
bool Search::_steric_clash(motif::MotifState const &ms, Node const &n) {
  ...
  float dist = 0;
  // first node will be start node, so no reason to compute the start node as
  // its a dummy might need to add a check to see if state has beads or this
  // will seg fault ... -- JDY
  while (current->parent() != nullptr) {
    // quick check to see how far these two motifs are from each other. If too
    // far sterics calc is probably not worth, need to benchamark -- JDY
