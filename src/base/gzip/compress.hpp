...
static const unsigned long MAX_SIZE_BEFORE_COMPRESS =
    2000000000;  // 2GB decompressed

// Compress method that takes a pointer an immutable character sequence (aka a
// string in C)
std::string compress(const char *data, std::size_t size,
  ...
  do {
    ...
    // check it here. Why not? Well, it turns out that deflate() can do no wrong
    // here." Basically only possible error is from deflateInit not working
    // properly
