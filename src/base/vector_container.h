...
// wrapper to avoid doing std::shared_ptr<std::vector<T>> since pybind11 wont
// allow this
template <typename T>
class VectorContainer {
 public:
  inline VectorContainer(std::vector<T> const& vec) : vec_(std::move(vec)) {}
 ...
 public:
  inline T const& operator[](Index i) const {
  ...
  inline T const& at(Index i) const {
    expects<std::runtime_error>(
        i < vec_.size(),
        "cannot get element: " + std::to_string(i) + " it is out of range");
    return vec_[i];
 ...
 public:
  inline std::vector<T> const& get_data() { return vec_; }
