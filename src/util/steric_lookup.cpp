...
StericLookup::StericLookup(float grid_size, float cutoff /*, float radius */) {
  if (grid_size < 0) {
  ...
  /*
  if (radius < 0) {
    String msg = "Cannot take a negative checking radius!";
    base::log_and_throw<base::MathException>(msg);
  } else if (cutoff == 0) {
    String msg = "Cannot take zero lookup radius!";
    base::log_and_throw<base::MathException>(msg);
  ...
  */
  ...
  _radius = ceil(cutoff/grid_size); // number of grid points to check in each direction
  _additions = math::Vector3s();
  _setup_additions();
}
...
int StericLookup::total_clash(const math::Vector3s &points) {
  int clash_count = 0;
  for (auto const &p : points) {
    clash_count += clash(p);
  }
...
void StericLookup::to_pdb(String const &pdb_name) {
