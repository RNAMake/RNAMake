...
char *case_strstr(const char *haystack, const char *needle) {
  ...
  if (p != NULL) p = (p - haystack_cp) + (char *)haystack;
...
long get_line_number(char *filename, long skips) {
  ...
  if (!num) fatal("File <%s> contains 0 valid records\n", filename);
...
double get_point2line_perp_distance(double *pnt, double *line_p1,
  ...
  for (i = 1; i <= 3; i++) p1_pnt[i] -= d * p1_p2[i];
...
void get_xml_tag(FILE *fpxml, char *prefix, char *line, char *connector,
  ...
  if (!tag_match(prefix, line, tag)) return;
  ...
  if (!isok) fatal("tag <%s> has end match\n", tag);
}
...
void extract_attribute(char *line, char *attr, char *attr_val, long to_lower) {
  ...
  p0 = strstr(line, str);
  if (!p0) fatal("line <%s> does not contain attribute: \"%s\"\n", line, attr);

  p1 = p0 + strlen(str);

  p2 = strchr(p1, '"');
  if (!p2) fatal("line <%s> has no matching \"\n", line);
  *p2 = '\0';

  strcpy(attr_val, p1);

  if (to_lower) lowerstr(attr_val);
}
...
void base_frame(long num_residue, char *bseq, long **seidx, long *res_type,
  ...
  for (i = 1; i <= num_residue; i++) {
    if (res_type[i] < 0) continue; /* non-bases */
...
void peptide_info(long num_residue, long **seidx, char **AtomName,
  ...
  for (i = 1; i <= num_residue; i++) {
    ...
    for (j = 0; j <= num_saa; j++)
      if (!strcmp(ResName[ib], SAA[j])) break;
...
static void align_block_xyz(double *orien_i, double *org_i, double *xyz_j,
  ...
  for (i = 1; i <= 3; i++) {
    for (j = 1; j <= 3; j++) temp[j] = orien_i[i + (j - 1) * 3];
...
void base_blks(long num_residue, long *res_type, double **orien, double **org,
  ...
  for (i = 1; i <= num_residue; i++)
    if (res_type[i] >= 0) num_nt++; /* number of bases */
  if (!num_nt) {
  ...
  for (i = 1; i <= num_residue; i++) {
    ...
    for (j = 1; j <= nbond; j++)
      for (k = 1; k <= 2; k++) tlinkage[ib + j][k] = ia + linkage[j][k];
...
static void reset_alist_symbol_idx(char *alist, long *aidx) {
  ...
  if (nchar == 0) fatal("empty atom list not allowed\n");
...
static void check_misc_pars(miscPars *misc_pars) {
  ...
  if (misc_pars->max_dv < 0.0) fatal("max_dv (%g) < 0.0\n", misc_pars->max_dv);

  if (misc_pars->min_dv < 0.0) fatal("min_dv (%g) < 0.0\n", misc_pars->min_dv);
...
static void get_3dna_pars(miscPars *misc_pars) {
  ...
  if (Gvars.DEBUG >= DEBUG_LEVEL) output_misc_pars(fp2, misc_pars, '1');
  ...
  if (Gvars.DEBUG < DEBUG_LEVEL) mask_off_options(misc_pars);
}
...
long is_linked_by_gap(long i, long j, double **o3_p) {
  ...
  for (k = i; k < j; k++)
    if (!is_linked(k, k + 1, o3_p)) return 0L;
...
long is_linked(long i, long j, double **o3_p) {
  ...
  if (dval_in_range(d, 0.0, O3P_UPPER)) return 1L; /* 5'--->3' linkage */

  d = distance_ab(o3_p, j, i, 4, 8);                /* O3'[j]-P[i] */
  if (dval_in_range(d, 0.0, O3P_UPPER)) return -1L; /* 3'--->5' linkage */
...
void peptide_blks(long num_residue, char *BDIR, long *cidx, double **orien,
  ...
  for (i = 1; i < num_residue; i++) /* NB: org[i][4] default to -1.0 */
    if (dval_in_range(org[i][4], 0.0, TRSP_RMS)) num_pep++;
  if (!num_pep) {
...
void convert_resNameSpace(char *resName, char replacement, char *newName) {
  ...
  if (replacement != '\0') {
    ...
    for (i = 0; i < k; i++)
      if (resName[i] != ' ') newName[j++] = resName[i];
    newName[j] = '\0';
  }
}
...
/* see also function set_planarStr() in set_planar.c */
{
  ...
  if (nmatch >= 3) {
    ...
    if (Gvars.VERBOSE) fprintf(stderr, "%s ===> %g\n", idmsg, org_i[4]);
  }
...
void planarAA_blks(long num_residue, long **seidx, char **ResName, char *BDIR,
  ...
  for (i = 1; i <= num_residue; i++) /* NB: paa_org[i][4] default to -1.0 */
    if (dval_in_range(paa_org[i][4], 0.0, TRSP_RMS)) num_paa++;
  if (!num_paa) {
...
long number_of_nt(long num_residue, long *res_type) {
  ...
  for (i = 1; i <= num_residue; i++)
    if (res_type[i] == 0 || res_type[i] == 1) num++;
...
void write_snap_par(FILE *fp, char *direction, double dist, double rot_ang,
  ...
  fprintf(fp, "\t%s %10.3f %10.3f %10.3f\n", direction, dist, rot_ang,
          ab_angle);
  for (j = 1; j <= 6; j++) fprintf(fp, "%10.3f", pars[j]);
  fprintf(fp, "\n");

  for (j = 1; j <= 3; j++) fprintf(fp, "%10.3f", orgP[j]);
  fprintf(fp, "\n");

  for (i = 1; i <= 3; i++) {
    for (j = 1; j <= 3; j++) fprintf(fp, "%10.3f", rotP[j][i]);
    fprintf(fp, "\n");
  }
}
...
void write_atom_xyz(FILE *fp, char *fmt, double *xyz, double dft) {
  ...
  for (i = 1; i <= 3; i++) fprintf(fp, fmt, (xyz == NULL) ? dft : xyz[i]);
  fprintf(fp, "\n");
}
...
long get_nextModelNumber(char *pdbfile) {
  ...
  if (exist_file(pdbfile)) {
    ...
    while ((p0 = my_getline(fp)) != NULL) {
      if (str_pmatch(p0, "MODEL ") && (sscanf(p0, "%*s %ld", &k) == 1)) num++;
      if (num != k)
...
void cleanup_files(long renew, long cleanup) {
  ...
  for (i = 0; i <= num_misc; i++) remove_file(MISC[i]);

  if (cleanup) fatal("done with cleaning up snap/poco files.\n");
}
