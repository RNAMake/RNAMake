...
double get_double(const nlohmann::json &, const String &);

int get_int(const nlohmann::json &, const String &);

char get_char(const nlohmann::json &, const String &);

String get_string(const nlohmann::json &, const String &);

math::Vector3 get_point(const nlohmann::json &, const String &);

// math::Quaternion get_quaternion(const nlohmann::json&, const String&);

math::Matrix3x3 get_matrix(const nlohmann::json &);

Reals get_reals(const nlohmann::json &, const String &);

Indexes get_ints(const nlohmann::json &, const String &);
...
struct DssrNt : public DssrElem {
  ...
  bool valid() const override {
    // TODO add in the things this class needs to be used by the rest of the
    // programs
    return false;
  }
};
...
struct DssrPair : public DssrElem {
  ...
  math::Matrix3x3 ref_frame{-1., -1., -1., -1, -1., -1., -1., -1., -1.};
  String hbonds_desc{
      "NA"}; //	"N1(imino)-N3[3.14],N2(amino)-O2(carbonyl)[3.27],O6(carbonyl)-N4(amino)[2.96]"
  int hbonds_num{-1};
  int index{-1};
...
DssrNts get_nts(const nlohmann::json &);

DssrPairs get_pairs(const nlohmann::json &);

DssrHairpins get_hairpins(const nlohmann::json &);

DssrHelices get_helices(const nlohmann::json &);

DssrStems get_stems(const nlohmann::json &);

DssrILoops get_iloops(const nlohmann::json &);

DssrJuncs get_juncs(const nlohmann::json &);

void get_elements(String const &, DssrNts &, DssrPairs &, DssrHairpins &,
                  DssrHelices &, DssrStems &, DssrILoops &
                  //        DrrsJuncs & ;
)
