...
math::Point util::get_point(const nlohmann::json& json, const String& key) {
  auto value = json.find(key);
...
math::Matrix util::get_matrix(const nlohmann::json& json) {
...
Ints util::get_ints(const nlohmann::json& json, const String& key) {
  auto value = json.find(key);
  if (value != json.end() && !value->is_null()) {
    auto values = Ints{};
    for (auto& vv : *value) {
