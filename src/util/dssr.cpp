...
double util::get_double(const nlohmann::json &json, const String &key) {
...
int util::get_int(const nlohmann::json &json, const String &key) {
  auto value = json.find(key);
  if (value != json.end() && !value->is_null()) {
...
char util::get_char(const nlohmann::json &json, const String &key) {
  auto value = json.find(key);
  if (value != json.end() && !value->is_null()) {
...
String util::get_string(const nlohmann::json &json, const String &key) {
  auto value = json.find(key);
  if (value != json.end() && !value->is_null()) {
...
math::Vector3 util::get_point(const nlohmann::json &json, const String &key) {
  auto value = json.find(key);
  if (value != json.end() && !value->at(0).is_null() &&
      !value->at(1).is_null() && !value->at(2).is_null()) {
...
math::Quaternion util::get_quaternion(const nlohmann::json &json,
                                      const String &key) {
  auto value = json.find(key);
  if (value != json.end() && !value->at(0).is_null() &&
      !value->at(1).is_null() && !value->at(2).is_null() &&
      !value->at(3).is_null()) {
    return math::Quaternion(value->at(0), value->at(1), value->at(2),
                            value->at(3));
  } else {
...
math::Matrix3x3 util::get_matrix(const nlohmann::json &json) {
  ...
  if (*x_axis != nullptr && *y_axis != nullptr && *z_axis != nullptr &&
      !x_axis->at(0).is_null() && !x_axis->at(1).is_null() &&
      !x_axis->at(2).is_null() && !y_axis->at(0).is_null() &&
      !y_axis->at(1).is_null() && !y_axis->at(2).is_null() &&
      !z_axis->at(0).is_null() && !z_axis->at(1).is_null() &&
      !z_axis->at(2).is_null()) {
    return math::Matrix(x_axis->at(0), x_axis->at(1), x_axis->at(2),
...
Reals util::get_reals(const nlohmann::json &json, const String &key) {
  auto value = json.find(key);
  if (value != json.end() && !value->is_null()) {
    auto values = Reals{};
    for (auto vv : *value) {
...
Indexes util::get_ints(const nlohmann::json &json, const String &key) {
...
util::DssrNts util::get_nts(const nlohmann::json &data) {
...
util::DssrPairs util::get_pairs(const nlohmann::json &data) {
...
util::DssrHairpins util::get_hairpins(const nlohmann::json &data) {
  auto hairpins = util::DssrHairpins{};

  for (auto &hairpin : *data.find("hairpins")) {

    if (hairpin.is_null())
      continue; // need to add something like this for everywhere
...
util::DssrHelices util::get_helices(const nlohmann::json &data) {
...
util::DssrStems util::get_stems(const nlohmann::json &data) {
...
util::DssrILoops util::get_iloops(const nlohmann::json &data) {
...
void util::get_elements(String const &pdb_path, util::DssrNts &nts,
                        util::DssrPairs &pairs, util::DssrHairpins &hairpins,
                        util::DssrHelices &helices, util::DssrStems &stems,
                        util::DssrILoops &iloops) {
  // auto dssr_json =
  // base::execute_command_json("../../resources/x3dna/osx/bin/x3dna-dssr -i=" +
  // pdb_path + " --json --more 2> /dev/null");
  auto x3dna_path = base::x3dna_path();
  auto dssr_json =
      base::execute_command_json(x3dna_path + "/bin/x3dna-dssr -i=" + pdb_path +
                                 " --json --more 2> /dev/null");
  if (!nts.empty()) {
    std::cout << "Warning, nts vector is NOT empty, but data will be "
                 "compeletely overwritten";
  }
