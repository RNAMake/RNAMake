...
template <class... Ts, class T> struct Append<TypeList<Ts...>, T> {
...
template <typename T, typename... Ts> struct Cdr<TypeList<T, Ts...>> {
  typedef TypeList<Ts...> type;
};

template <class List> using Cdr_t = typename Cdr<List>::type;

// Find metafunction (get the idx'th element)
template <class List, unsigned idx> struct Find : Find<Cdr_t<List>, idx - 1> {};

template <typename T, typename... Ts> struct Find<TypeList<T, Ts...>, 0> {
  typedef T type;
};
...
template <int N> using char_array = const char[N];
 ...
 *
 * The upshot is that `decltype(my_function(Rank<100>{}))` is a single
 * well-formed expression, which, because of C++ overload resolution rules, can
 * be a "mutable" value from the point of view of metaprogramming.
 *
 *
 * Attribution:
...
template <typename S, typename T, T S::*member_ptr> struct member_ptr_helper {
...
template <typename M> struct member_helper {
  ...
  VISIT_STRUCT_CXX14_CONSTEXPR static void apply_visitor(V &&visitor, S1 &&s1,
                                                         S2 &&s2) {
    std::forward<V>(visitor)(M::member_name(),
...
template <typename Mlist> struct structure_helper;

template <typename... Ms> struct structure_helper<TypeList<Ms...>> {
  template <typename V, typename S>
  VISIT_STRUCT_CXX14_CONSTEXPR static void
  apply_visitor(V &&visitor, S &&structure_instance) {
    // Use parameter pack expansion to force evaluation of the member helper for
    // each member in the list. Inside parens, a comma operator is being used to
    // discard the void value and produce an integer, while not being an
    // unevaluated context. The order of evaluation here is enforced by the
    // compiler. Extra zero at the end is to avoid UB for having a zero-size
    // array.
    int dummy[] = {
  ...
  VISIT_STRUCT_CXX14_CONSTEXPR static void visit_pointers(V &&visitor) {
    int dummy[] = {
        (member_helper<Ms>::visit_pointers(std::forward<V>(visitor)), 0)..., 0};
    static_cast<void>(dummy);
    static_cast<void>(visitor);
  }
  ...
  VISIT_STRUCT_CXX14_CONSTEXPR static void visit_accessors(V &&visitor) {
    int dummy[] = {
        (member_helper<Ms>::visit_accessors(std::forward<V>(visitor)), 0)...,
        0};
    static_cast<void>(dummy);
    static_cast<void>(visitor);
  }
  ...
  VISIT_STRUCT_CXX14_CONSTEXPR static void visit_types(V &&visitor) {
    int dummy[] = {
        (member_helper<Ms>::visit_types(std::forward<V>(visitor)), 0)..., 0};
    static_cast<void>(dummy);
    static_cast<void>(visitor);
  }
};
...
struct visitable<T, typename std::enable_if<std::is_same<
  ...
  static VISIT_STRUCT_CXX14_CONSTEXPR void apply(V &&v, S &&s) {
    detail::structure_helper<
        typename T::Visit_Struct_Registered_Members_List__>::
  ...
  template <int idx>
  static VISIT_STRUCT_CONSTEXPR auto
  get_accessor(std::integral_constant<int, idx>) -> typename detail::Find_t<
      typename T::Visit_Struct_Registered_Members_List__, idx>::accessor_t {
    return {};
  }
  ...
  static VISIT_STRUCT_CONSTEXPR decltype(T::Visit_Struct_Get_Name__())
  get_name() {
...
#define BEGIN_VISITABLES(NAME)                                                 \
  ...
  ::visit_struct::detail::                                                     \
      TypeList<> static inline Visit_Struct_Get_Visitables__(                  \
...
#define VISITABLE(TYPE, NAME)                                                  \
  ...
  struct VISIT_STRUCT_MAKE_MEMBER_NAME(NAME)                                   \
      : visit_struct::detail::member_ptr_helper<                               \
            VISIT_STRUCT_CURRENT_TYPE, TYPE,                                   \
            &VISIT_STRUCT_CURRENT_TYPE::NAME> {                                \
    static VISIT_STRUCT_CONSTEXPR const ::visit_struct::detail::char_array<    \
        sizeof(#NAME)> &                                                       \
    member_name() {                                                            \
      return #NAME;                                                            \
    }                                                                          \
  };                                                                           \
  static inline ::visit_struct::detail::Append_t<                              \
      VISIT_STRUCT_GET_REGISTERED_MEMBERS,                                     \
      VISIT_STRUCT_MAKE_MEMBER_NAME(NAME)>                                     \
      Visit_Struct_Get_Visitables__(                                           \
          ::visit_struct::detail::Rank<                                        \
              VISIT_STRUCT_GET_REGISTERED_MEMBERS::size + 1>);                 \
  static_assert(true, "")
...
#define END_VISITABLES                                                         \
  typedef VISIT_STRUCT_GET_REGISTERED_MEMBERS                                  \
      Visit_Struct_Registered_Members_List__;                                  \
  typedef ::visit_struct::detail::intrusive_tag                                \
      Visit_Struct_Visitable_Structure_Tag__;                                  \
  static_assert(true, "")
