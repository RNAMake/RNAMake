...
void PairFinder::_write_fpmst(double* morg, double* morien, FILE* rframe,
  ...
  auto reg = std::regex(
      "#\\s+(?:\\.+\\d+\\>)*(\\w+):\\.*(-*\\d+)\\S:\\[\\.*(\\S+)\\](\\w+)\\s+"
      "\\-\\s+(?:\\.+\\d+\\>)*(\\w+):\\.*(-*\\d+)\\S:\\[\\.*(\\S+)\\](\\w+)");
  auto r = math::Matrix(rs[0].x(), rs[0].y(), rs[0].z(), rs[1].x(), rs[1].y(),
                        rs[1].z(), rs[2].x(), rs[2].y(), rs[2].z());
  ...
  if ((bp_info->res1_name == 'C' && bp_info->res2_name == 'G') ||
    ...
    // std::cout << bp_info->res1_num << " " << bp_info->res1_name << " " <<
    // bp_info->res2_num << " "
 ...
 *col# 1   2    3   4    5     6     7     8-10  11-13 14-16 17-19 20-22 23-25
 *26-28
 */
static void best_pair(long i, long num_residue, long* RY, long** seidx,
                      double** xyz, long* idx, double** NC1xyz,
                      long* matched_idx, double** orien, double** org,
                      long** ring_atom, char** AtomName, char* bseq,
...
static long bp_coplanar(long i, double d, double d2, double* txyz,
  ...
  if (j && fabs(dot(&bp_xyz[i][9], dp)) < OLCRT &&
      ...
      fabs(dot(&bp_xyz[j][9], dp)) < OLCRT &&
      fabs(dot(&bp_xyz[j][18], dp)) < OLCRT) {
    co_planar = 1;
  }
  return co_planar;
}
...
static void bp_context(long num_bp, miscPars* misc_pars, double** bp_xyz,
                       long** bp_order, long** end_list, long* num_ends,
                       FILE* tfp) {
  double helix_break = misc_pars->helix_break;
  double d = EMPTY_NUMBER, d2, d3, ddmin[9], txyz[4], txyz2[4], txyz3[4],
         zave[4];
  ...
  //     end_list[i][2], end_list[i][3]);
}

/* locate all possible helical regions, including isolated base-pairs */
static void locate_helix(long num_bp, long** helix_idx, long num_ends,
  ...
  for (i = 1; i <= num_ends && ip < num_bp; i++) {
    ...
    if (k == k0) {
      continue;
    } /* end point of a processed helix */
...
static void get_ij(long m, long* swapped, long** base_pairs, long* n1,
                   long* n2) {
  if (swapped[m]) {
    *n1 = base_pairs[m][2];
    *n2 = base_pairs[m][1];
...
static long chain1dir(long m, long n, long* swapped, long** base_pairs,
                      double** o3_p) {
...
static double wcbp_zdir(long m, long n, long idxm1, long idxm2, long idxn1,
                        long idxn2, double** bp_xyz) {
  double dm[4], dn[4];
...
static long wc_bporien(long m, long n, long* swapped, long** base_pairs,
                       double** bp_xyz, double** o3_p) {
 ...
 *    |       |
 *    i1 ---- j1 */
static long check_o3dist(long m, long n, long* swapped, long** base_pairs,
                         double** o3_p) {
...
static long check_schain(long m, long n, long* swapped, long** base_pairs,
                         double** o3_p) {
  long i1, i2, j1_osx, j2, irev = 0;
...
/* check for situations where wc_bporien, check_o3dist & check_schain do not
 * apply */
static long check_others(long m, long n, long* swapped, long** base_pairs,
                         double** o3_p, double** bp_xyz) {
...
static void check_direction(long i, long** helix_idx, long* bp_idx,
  ...
  if (direction[1] + direction[2] + direction[4] + direction[5] == 0) {
    return;
  } /* all empty: no direct connection */
...
static void check_strand2(long i, long** helix_idx, long* bp_idx,
                          double** bp_xyz, long* swapped, long** base_pairs,
                          double** o3_p, long* direction, FILE* tfp) {
  ...
  if (!helix_idx[i][7]) {
    ...
    for (j = helix_idx[i][1]; j < helix_idx[i][2]; j++) {
      ...
      if (wc_bporien(m, n, swapped, base_pairs, bp_xyz, o3_p)) {
        continue;
      } /* in good WC-geometry orientation */
      get_ij(m, swapped, base_pairs, &i1, &j1_osx);
      get_ij(n, swapped, base_pairs, &i2, &j2);
      if (!is_linked(i1, i2, o3_p) && !is_linked(j1_osx, j2, o3_p) &&
 ...
 *           col#     1       2       3       4      5       6        7
 */
static void five2three(long num_bp, long* num_helix, long** helix_idx,
                       long* bp_idx, double** bp_xyz, long** base_pairs,
                       double** o3_p, FILE* tfp) {
  double do3_p;
  ...
  for (i = 1; i <= *num_helix; i++) {
    ...
    for (j = helix_idx[i][1]; j < helix_idx[i][2]; j++) {
      ...
      // frprintf(tfp, "          %4ld: %2ld %2ld %2ld %2ld", j, rev_wc,
      // rev_o3d,
      //         rev_oth, rev_csc);
      if (rev_wc) {
        swapped[n] = !swapped[n];
    ...
    check_direction(i, helix_idx, bp_idx, swapped, base_pairs, o3_p, direction);
    check_strand2(i, helix_idx, bp_idx, bp_xyz, swapped, base_pairs, o3_p,
                  direction, tfp);
...
static void re_ordering(long num_bp, long** base_pairs, long* bp_idx,
                        long* helix_marker, long** helix_idx,
                        miscPars* misc_pars, long* num_helix, double** o3_p,
                        char* bseq, long** seidx, char** ResName, char* ChainID,
  ...
  locate_helix(num_bp, helix_idx, num_ends, num_helix, end_list, bp_order,
               bp_idx, helix_marker);
...
void PairFinder::_write_bestpairs(long num_bp, long** base_pairs, long* bp_idx,
  ...
  // -- JDY - fixing single basepairs
  // removes messed up base pair in TTR
  for(auto const & bp : bps) {
    seen[bp.res1.num] = 1;
    seen[bp.res2.num] = 1;
  }
  int neighbors = 0;
  std::vector<int> neighbor_nums;
  std::vector<int> remove;
  int pos = 0;
  for(auto const & bp : bps) {
    neighbors = 0;
    neighbor_nums = {bp.res1.num-1, bp.res1.num+1, bp.res2.num-1, bp.res2.num+1};
    for(auto const & n : neighbor_nums) {
      if(seen.find(n) != seen.end()) {
        neighbors += 1;
      }
    }
    //std::cout << bp.res1.num << " " << bp.res2.num << " " << neighbors <<
    //    std::endl;
    if(neighbors < 2) {
  ...
  while (num1 < num2) {
    ...
    for (i = 1; i <= num_residue; i++) {
      if (RY[i] < 0 || matched_idx[i]) {
        continue;
      } /* non-base or paired base */
      best_pair(i, num_residue, RY, seidx, xyz, idx, NC1xyz, matched_idx, orien,
                org, ring_atom, AtomName, bseq, misc_pars, pair_istat);
      if (pair_istat[1]) {
        /* with paired base */
        best_pair(pair_istat[1], num_residue, RY, seidx, xyz, idx, NC1xyz,
                  matched_idx, orien, org, ring_atom, AtomName, bseq, misc_pars,
                  pair_jstat);
        if (i == pair_jstat[1]) {
...
void PairFinder::_duplex(long num, long num_residue, char* bseq, long** seidx,
                         ...
                         double** xyz, char* parfile, miscPars* misc_pars) {
