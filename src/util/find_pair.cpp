...
PairFinder::_write_fpmst(
  ...
  if((bp_info->res1_name == 'C' && bp_info->res2_name == 'G') ||
    ...
    //std::cout << bp_info->res1_num << " " << bp_info->res1_name << " " << bp_info->res2_num << " "
...
best_pair(
  ...
  for(j = 1; j <= num_residue; j++) {
    ...
    check_pair(
      i, j, bseq, seidx, xyz, NC1xyz, orien, org, idx, AtomName, misc_pars,
...
PairFinder::_write_bestpairs(
  ...
  for(i = 1; i <= num_bp; i++) {
    ...
    ivec[2] = ib;
    pair2mst(
      inum_base, ivec, AtomName, ResName, ChainID, ResSeq, Miscs, xyz,
...
write_helix(
  ...
  for(i = 1; i <= num_helix; i++) {
    ...
    for(j = 1; j <= tnum_res; j++) {
      m = ivect[j];
      pdb_record(
        seidx[m][1], seidx[m][2], &inum, 0, AtomName, ResName,
...
find_bestpair(
  ...
  while(num1 < num2) {
    ...
    for(i = 1; i <= num_residue; i++) {
      ...
      best_pair(
        i, num_residue, RY, seidx, xyz, idx, NC1xyz, matched_idx,
        orien, org, ring_atom, AtomName, bseq, misc_pars, pair_istat);
      if(pair_istat[1]) {
        /* with paired base */
        best_pair(
          pair_istat[1], num_residue, RY, seidx, xyz, idx, NC1xyz,
...
PairFinder::_duplex(
  ...
  identify_htw(
    num_residue, seidx, RY, AtomName, ResName, ChainID, ResSeq,
    Miscs, xyz, htm_water);

  base_info(
    num_residue, bseq, seidx, RY, AtomName, ResName, ChainID,
...
*/
  num_bp = find_bestpair(
    nout, base_pairs, num_residue, bseq, seidx, RY, AtomName,
  ...
  re_ordering(
    num_bp, base_pairs, bp_idx, helix_marker, helix_idx, misc_pars,
    &num_helix, o3_p, bseq, seidx, ResName, ChainID, ResSeq, Miscs);
  PairFinder::_write_bestpairs(
    num_bp, base_pairs, bp_idx, bseq, seidx, AtomName, ResName, ChainID,
    ResSeq, Miscs, xyz, orien, org, htm_water, misc_pars);
  write_helix(
    num_helix, helix_idx, bp_idx, seidx, AtomName, ResName, ChainID, ResSeq,
  ...
  if(args.curves) { /* generate Curves input file */
    if(args.divide && num_helix > 1) {
      ...
      curves_input(
        0, 1, num_bp, num_bp, args.pdbfile, args.outfile, parfile,
    ...
    col_helices(
      num_helix, helix_idx, bp_idx, base_pairs, seidx, args.pdbfile,
      ChainID, ResSeq);
...
static char
get_standard_one_letter_nt(char*rname) {
...
PairFinder::_handle_str() {
  ...
  read_pdb(
    args.pdbfile, NULL, AtomName, ResName, ChainID, ResSeq, xyz, Miscs,
  ...
  PairFinder::_duplex(
    num, num_residue, bseq, seidx, RY, AtomName, ResName, ChainID,
