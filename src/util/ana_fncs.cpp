...
static double idx2torsion(long *idx, double **xyz, long chk_lkg) {
  ...
  if (i > 4) d = chk_lkg ? torsion(xyz4) : torsion2(xyz4);
...
long **read_input(char *inpfile, char *pdbfile, char *outfile, long *ds,
  ...
  if (*num_bp <= 0) fatal("illegal number (%ld <= 0) of base-pairs\n", *num_bp);
  ...
  if (*ip) { /* user input */
    for (i = 1; i <= *num_bp; i++) {
      if (fgets(str, sizeof str, fp) == NULL) fatal("cannot read pair list\n");
      if (*ds == 2) { /* duplex */
        k = sscanf(str, "%ld %ld %ld", &pair_num[1][i], &pair_num[2][i],
                   &pair_num[3][i]);
        if (k != 2 && k != 3) fatal("two serial numbers required\n");
...
void print_header(long ds, long num_bp, long num, char *pdbfile, FILE *fp) {
  ...
  print_sep(fp, '*', 76);
  fprintf(fp,
          "1. The list of the parameters given below correspond to"
...
static void output_atom_xyz(FILE *fp, long idx, double **xyz, char *bstr,
  ...
  if (idx)
    for (i = 1; i <= 3; i++) fprintf(fp, fmt, xyz[idx][i]);
  else
    for (i = 1; i <= 3; i++) fprintf(fp, "%s", bstr);
}
...
void output_Borg_P_C1_C4(long num_residue, double **org, double **xyz,
  ...
  for (i = 1; i <= num_residue; i++) {
    if (nt_list[i][NT_NUM] < 3) continue;

    fprintf(fp, "%4ld %s", i, nt_info[i]);
    for (j = 1; j <= 3; j++) fprintf(fp, fmt, org[i][j]);
...
void atom_list(long ds, long num_bp, long **pair_num, long **seidx, long *RY,
  ...
  for (i = 1; i <= ds; i++) {
    for (j = 1; j <= num_bp; j++) {
      ...
      if (RY[rnum] < 0) fatal("Non-base residue: %s\n", ResName[ib]);
...
void output_nt_torsion(long num_residue, char **nt_info, long **nt_list,
  ...
  for (i = 1; i <= num_residue; i++) {
    ...
    if (nt_torsion[i][TOR_PHASE] < EMPTY_CRITERION) strcat(ostr, "    ");
...
static void print_duplex_torsions(long num_bp, long **pair_num, char **bp_seq,
  ...
  for (i = 1; i <= ds; i++) {
    ...
    for (j = 1; j <= num_bp; j++) {
      ...
      for (m = 1; m <= 6; m++) parcat(str, nt_bb_torsion[idx][m], fmt, bstr);
...
void p_c1_dist(long ds, long num_bp, char **bp_seq, long **phos, long **chi,
  ...
  for (i = 1; i <= ds; i++) {
    for (j = 1; j <= nbpm1; j++) {
      ...
      ia = phos[i][j];
      ib = phos[i][j + 1];
      if (ia && ib) p_dist[i][j] = p1p2_dist(xyz[ia], xyz[ib]);
      ia = chi[i][(j - 1) * 4 + 2];
      ib = chi[i][j * 4 + 2];
      if (ia && ib) c1_dist[i][j] = p1p2_dist(xyz[ia], xyz[ib]);
    }
  }
  ...
  fprintf(fp, "                 Strand I");
  if (ds == 2) fprintf(fp, "                          Strand II");
  fprintf(fp, "\n");

  fprintf(fp, "    step      P--P     C1'--C1'");
  if (ds == 2) fprintf(fp, "       step      P--P     C1'--C1'");
  fprintf(fp, "\n");
...
void lambda_d3(long num_bp, char **bp_seq, long **chi, long **c6_c8,
  ...
  print_sep(fp, '*', 76);
  fprintf(fp,
          "lambda: virtual angle between C1'-YN1 or C1'-RN9"
  ...
  for (i = 1; i <= num_bp; i++) {
    ...
    for (j = 1; j <= 5; j++) parcat(str, lambda_dist[i][j], fmt, bstr);
    fprintf(fp, "%s\n", str);
  }
...
void print_axyz(long num_bp, char **bp_seq, long **aidx, char *aname,
  ...
  for (i = 1; i <= num_bp; i++) {
    ...
    if (aidx[1][i])
      for (j = 1; j <= 3; j++) fprintf(fc, fmt, xyz[aidx[1][i]][j]);
    else
      fprintf(fc, "%s%s%s", bstr, bstr, bstr);

    if (aidx[2][i])
      for (j = 1; j <= 3; j++) fprintf(fc, fmt, xyz[aidx[2][i]][j]);
    else
      fprintf(fc, "%s%s%s", bstr, bstr, bstr);
...
void groove_width(long parallel, long num_bp, char **bp_seq, long **phos,
  ...
  for (i = 1; i <= nbpm1; i++) {
    ...
    for (j = 1; j <= 4; j++) {
      idx[j] = i + iminor[j];
      if (!lval_in_range(idx[j], 1, nbpm1)) break;
    }
    if (j > 4) {
      pp[1] = phos[1][idx[1] + 1];
      pp[3] = phos[1][idx[3] + 1];
      ...
      if (pp[1] && pp[2] && pp[3] && pp[4]) {
        ...
        for (k = 1; k <= 4; k++) {
          idxm1[k] = idx[k] - 1;
          idxp1[k] = idx[k] + 1;
          if (idxm1[k] < 1 || idxp1[k] > nbpm1) break;
        }
        if (k > 4) {
  ...
  for (i = 1; i <= nbpm1; i++) {
    if (bphlx[i]) {
      for (k = i - 3; k <= i + 3; k++) {
        if (!lval_in_range(k, 1, nbpm1)) continue;
  ...
  for (i = 1; i <= nbpm1; i++) {
    ...
    for (j = 1; j <= 4; j++) parcat(str, gwidth[i][j], fmt, bstr);
    fprintf(fp, "%s\n", str);
  }
  ...
  for (nset = 1; nset <= num_dist_sets; nset++) {
    ...
    if (nset != num_dist_sets) fprintf(fc, "\n");
  }
...
void check_wc_wobble_pair(long *bpid, char *bp, double shear, double stretch,
  ...
  if (fabs(stretch) > 2.0 || fabs(opening) > 60) return;

  if (dval_in_range(fabs(shear), 1.8, 2.8)) *bpid = 1; /* with WC geometry */
  if (fabs(shear) <= 1.8 && num_strmatch(bp, WC, 1, 8)) *bpid = 2; /* WC */
}
...
void set_chain_nmarkers019_to_symbols(long num, long *nmarkers,
  ...
  temp = lvector(1, num);
  for (i = 1; i < num; i++) temp[i] = (tcp[i + 1] != tcp[i]) ? 1 : 0;
  temp[num] = 1;
  ...
  n = 0;
  for (i = 1; i <= num; i++)
    if (temp[i]) seidx[++n][2] = i;
  for (i = 2; i <= n; i++) seidx[i][1] = seidx[i - 1][2] + 1;
  seidx[1][1] = 1;

  for (i = 1; i <= n; i++) seidx[i][0] = seidx[i][2] - seidx[i][1] + 1;
...
void ref_frames(long ds, long num_bp, long **pair_num, char **bp_seq,
  ...
  if (irna) RA_NUM++; /* One more C1' atom */
  ...
  if (ds == 2) {
    ...
    ib = 0;
    ik = 0;
    if (num_bp == 1) init_dvector(orgi, 1, 3, 0.0);
    for (i = 1; i <= ds; i++) {
      for (j = 1; j <= num_bp; j++) {
        ioffset3 = (j - 1) * 3;
        if (j < num_bp) ddxyz(org[i] + ioffset3, org[i] + ioffset3 + 3, orgi);
        cpxyz(orien[i] + (j - 1) * 9 + 6, vz);
        if (!pair_num[3][j]) {
          ik++;                                        /* non-breaks */
          if (dot(orgi, vz) < 0.0 && WC_info[j]) ++ib; /* z-axis reversed */
        }
      }
    }
    ...
    if (ib && ib != ik) *str_type = 2;                 /* unusual cases */
    if (ik != ds * num_bp) *str_type = *str_type + 10; /* more than one helix */
  }
  /* end of ds == 2 */
  ...
  fprintf(fp, "\n\n");
  fprintf(fp, "            Strand I");
  if (ds == 2) fprintf(fp, "                    Strand II          Helix");
  fprintf(fp, "\n");
...
void bpstep_par(double **rot1, double *org1, double **rot2, double *org2,
  ...
  for (i = 1; i <= 3; i++) {
    pars[i] = 0.0;
    for (j = 1; j <= 3; j++) pars[i] += t1[j] * mst_orien[j][i];
  }
...
void helical_par(double **rot1, double *org1, double **rot2, double *org2,
  ...
  if (vlen < XEPS) { /* for twist = 0.0 */
    ...
    for (i = 1; i <= 3; i++) axis_h[i] /= vlen;

  temp = dmatrix(1, 3, 1, 3);

  rot1_h = dmatrix(1, 3, 1, 3);
  for (i = 1; i <= 3; i++) t1[i] = rot1[i][3]; /* z1 */
  ...
  for (i = 1; i <= 3; i++) t2[i] = rot2[i][3]; /* z2 */
  ...
  for (i = 1; i <= 3; i++) t1[i] = t2[i] - pars[3] * axis_h[i];
  if (fabs(pars[6]) < HTWIST0) /* twist = 0.0: cf <xhelfunc> */
    for (i = 1; i <= 3; i++) org1_h[i] = org1[i] + 0.5 * t1[i];
  else {
    get_vector(t1, axis_h, 90 - pars[6] / 2, AD_axis);
    AD_mag = 0.5 * veclen(t1) / sin(deg2rad(pars[6] / 2));
    for (i = 1; i <= 3; i++) org1_h[i] = org1[i] + AD_mag * AD_axis[i];
  }

  for (i = 1; i <= 3; i++) org2_h[i] = org1_h[i] + pars[3] * axis_h[i];
  avexyz(org1_h, org2_h, mst_org);
  ddxyz(org1_h, org1, t1);

  for (i = 1; i <= 2; i++) {
    pars[i] = 0.0;
    for (j = 1; j <= 3; j++) pars[i] += t1[j] * rot1_h[j][i];
  }
...
void print_par(char **bp_seq, long num_bp, long ich, long ishel, double **param,
  ...
  if (num_bp > 2) {
    ...
    for (i = 1; i <= 6; i++) fprintf(fp, fmt, temp[i]);
    fprintf(fp, "\n");
    fprintf(fp, "      s.d.");
...
static void single_helix(long num_bp, char **bp_seq, double **step_par,
  ...
  fprintf(fchek,
          "Reference frame: Origins (Ox, Oy, Oz) followed by the"
...
void prt_stepstr(char **step_str, long num_step, long *bphlx, long ishel,
  ...
  for (i = 1; i <= num_step; i++) {
    ...
    if (bphlx[i])
      for (j = 1; j <= 6; j++) fprintf(fp, "%s", bstr);
    else {
      num++;
      for (j = 1; j <= 6; j++) {
...
static void double_helix(long num_bp, char **bp_seq, double **step_par,
  ...
  print_sep(fp, '*', 76);
  fprintf(fp,
          "Origin (Ox, Oy, Oz) and mean normal vector"
  ...
  for (i = 1; i <= num_bp; i++) {
    ...
    fprintf(fp, " %4ld %c%c%c ", i, bp_seq[1][i], bp_seq[0][i], bp_seq[2][i]);
    for (j = 1; j <= 3; j++) fprintf(fp, fmt, mfoi[j]);   /* origin */
    for (j = 1; j <= 3; j++) fprintf(fp, fmt, mfi[j][3]); /* base-pair normal */
    fprintf(fp, "\n");
  ...
  if (istep != 1) {
    ...
    fprintf(fp,
            "    be meaningless. Files <bp_helical.par>, <bp_step.par>, "
            "<hstacking.pdb>,\n");
...
void parvec2mtx(double *parvec, long num, double **parmtx) {
  ...
  for (i = 1; i <= num; i++) {
    ioffset = (i - 1) * nc;
    for (j = 1; j <= nc; j++) parmtx[i][j] = parvec[ioffset + j];
  }
}
...
void print_ss_rebuild_pars(double **pars, long num_bp, char *str, char **bp_seq,
  ...
  for (i = 1; i <= 6; i++) fprintf(fp, fmt, 0.0);
  fprintf(fp, "\n");

  for (i = 2; i <= num_bp; i++) {
    fprintf(fp, "%c ", bp_seq[1][i]);
    for (j = 1; j <= 6; j++) fprintf(fp, fmt, pars[i - 1][j]);
    fprintf(fp, "\n");
  }
}
...
void print_ds_rebuild_pars(double **bp_par, double **step_par, long num_bp,
  ...
  for (i = 1; i <= 6; i++) fprintf(fp, fmt, bp_par[1][i]);
  for (i = 1; i <= 6; i++) fprintf(fp, fmt, 0.0);
  fprintf(fp, "\n");

  for (i = 2; i <= num_bp; i++) {
    fprintf(fp, "%c%c%c ", bp_seq[1][i], bp_seq[0][i], bp_seq[2][i]);
    for (j = 1; j <= 6; j++) fprintf(fp, fmt, bp_par[i][j]);
    for (j = 1; j <= 6; j++) fprintf(fp, fmt, step_par[i - 1][j]);
    fprintf(fp, "\n");
  }
}
...
void print_ref(char **bp_seq, long num_item, long ich, double *org,
  ...
  for (i = 1; i <= num_item; i++) {
    ...
    ioffset3 = (i - 1) * 3;
    for (j = 1; j <= 3; j++) fprintf(fp, "%8.2f", org[ioffset3 + j]);

    ioffset9 = (i - 1) * 9;
    for (j = 1; j <= 9; j++) fprintf(fp, "%6.2f", orien[ioffset9 + j]);
...
void print_xyzP(long parallel, long nbpm1, char **bp_seq, long **phos,
  ...
  for (i = 1; i <= nbpm1; i++) {
    ...
    if (ip1 && ip2) avexyz(P_mst1, P_mst2, aveP[i]);
...
void print_PP(long parallel, double **twist_rise, long num_bp, char **bp_seq,
  ...
  print_sep(fp, '*', 76);
  fprintf(fp,
          "Classification of each dinucleotide step in"
  ...
  for (i = 1; i <= nbpm1; i++) {
    ...
    if (ip && !bphlx[i]) {
      ...
      if (WC_info[i] && WC_info[i + 1] &&                /* WC geometry */
        ...
        if (abi) ABIval[i] = get_ABI(i, aveS[i][3], chi_angle);
      }
    } else
      for (j = 1; j <= 7; j++) strcat(step_info[i], bstr);
  }
  idx[1] = BUF512;
  idx[num_bp] = BUF512;
  for (i = 2; i <= nbpm1; i++) idx[i] = strABT[i] - strABT[i - 1];
  for (i = 1; i <= nbpm1; i++) {
    ...
    if (abi && ABIval[i] > EMPTY_CRITERION) fprintf(fp, " %6.2f", ABIval[i]);
    fprintf(fp, "\n");
  }
...
void str_classify(double twist_p, double twist_n, long str_type, long parallel,
  ...
  if (twist_p && twist_n) {
    ...
    if (str_type < 0) {
      fprintf(fp,
              "Note that 3DNA determines this junction based *purely* on the "
              "geometry of\n"
              "your input structure. The junction is between a right-handed "
              "fragment with\n"
              "a left-handed one, not necessarily just B-DNA and Z-DNA, even "
              "though most\n"
...
double a_hlxdist(long idx, double **xyz, double *hlx_axis, double *hlx_pos) {
  ...
  if (idx) {
    ...
    temp = dot(d, hlx_axis);
    for (i = 1; i <= 3; i++) d[i] -= temp * hlx_axis[i];
    return veclen(d);
  } else
    return EMPTY_NUMBER;
}
...
void helix_radius(long ds, long num_bp, char **bp_seq, double **orien,
  ...
  print_sep(fp, '*', 76);
  fprintf(fp,
          "Helix radius (radial displacement of P, O4', and C1'"
...
void print_shlx(char **bp_seq, long nbpm1, long ich, double *shlx_orien,
  ...
  for (i = 1; i <= nbpm1; i++) {
    ...
    ioffset6 = (i - 1) * 6;
    ioffset18 = (i - 1) * 18;
    for (j = 1; j <= 3; j++) fprintf(fp, "%8.2f", shlx_org[ioffset6 + j]);
    for (j = 1; j <= 9; j++) fprintf(fp, "%6.2f", shlx_orien[ioffset18 + j]);
    fprintf(fp, "\n          ");
...
void get_helix_axis(long ds, long num_bp, char **bp_seq, double **orien,
  ...
  if (ds == 1) {
    ...
    for (i = 1; i <= nbpm1; i++) {
      ...
      for (j = 1; j <= 3; j++) fprintf(fp, fmt, morg[j]);
      for (j = 1; j <= 3; j++) fprintf(fp, fmt, hx[j]);
      fprintf(fp, "\n");
    }
...
void get_axis(long nvec, long **idx, long num, double **xyz, long nb, long *C1b,
  ...
  *hrise = EMPTY_NUMBER;
  ...
  for (i = 1; i <= nvec; i++) ddxyz(xyz[idx[i][1]], xyz[idx[i][2]], vxyz[i]);
...
static void get_poc_radius(long ds, long num_bp, long **phos, long **chi,
  ...
  for (i = 1; i <= ds; i++)
    for (j = 1; j <= num_bp; j++) {
      k = 0; /* P */
      ik = phos[i][j];
      if (ik) poc_radius[k][++poc_num[k]] = a_hlxdist(ik, xyz, haxis, hstart);
      ioffset = (j - 1) * 4;
      k = 1; /* O4 */
      ik = chi[i][ioffset + 1];
      if (ik) poc_radius[k][++poc_num[k]] = a_hlxdist(ik, xyz, haxis, hstart);
...
static void global_c1_c1_par(long num_bp, char **bp_seq, long **chi,
  ...
  for (i = 1; i <= num_bp; i++) { /* displacement and angle */
    ...
    if (ia && ib) {
      ...
      dtmp = dot(dd, haxis);
      for (j = 1; j <= 3; j++) org_xyz[j] = dd[j] - dtmp * haxis[j];
  ...
  for (i = 1; i <= num_bp; i++) fprintf(fp, "%s\n", c1_hpar[i]);
...
void global_analysis(long ds, long num_bp, long num, char **bp_seq, long **chi,
  ...
  if (hrise < EMPTY_CRITERION) return; /* no helix defined */

  print_sep(fp, '*', 76);
  fprintf(fp,
          "Global linear helical axis defined by equivalent C1'"
          " and RN9/YN1 atom pairs\n");
  fprintf(fp, "Deviation from regular linear helix: %.2f(%.2f)\n", hrise,
          std_rise);

  if (std_rise > Gvars.misc_pars.std_curved) return;
...
static long with_stacking(long ds, long istep, long **pair_num, double **orien,
  ...
  if (ds == 1) return with_ss_overlap(istep, i1, i2, orien, ring_atom, xyz);
...
void base_overlap(long ds, long num_bp, long num, long num_residue,
  ...
  for (i = 1; i < num_bp; i++) {
    ...
    for (j = 1; j <= k; j++) fprintf(fp, fmt, olarea[5 + j], olarea[j]);
    if (ds == 2) fprintf(fp, fmt, olarea[10], olarea[5]);
    fprintf(fp, "\n");
  }
...
void ring_oidx(long num, long num_residue, long *RY, long **seidx,
  ...
  for (i = 1; i <= num_residue; i++) /* for each residue */
    if (ring_atom[i][10] > 0) get_cntatom(ring_atom[i], connect, idx);
...
void get_cntatom(long *ringlist, long **connect, long *idx) {
  ...
  for (i = 1; i <= ra_num; i++) {
    ...
    for (j = 1; j <= connect[id][7]; j++) {
      ic = connect[id][j];
      if (idx[ic] == 3) continue;                  /* excluding H atom */
...
static void pia_inness(double *out_s, vertex *P, long cP, vertex *Q, long cQ) {
  ...
  for (j = 0; j < cP; ++j) {
    if (s) pia_cntrib(out_s, P[j].ip, P[j + 1].ip, s);
    s += P[j].inside;
  }
}
...
static double pia_inter(point *a, long na, point *b, long nb) {
  ...
  maxx = maxy = -XBIG;
  for (j = 0; j < na; j++) {
    if (minx > a[j].x) minx = a[j].x;
    if (miny > a[j].y) miny = a[j].y;
    if (maxx < a[j].x) maxx = a[j].x;
    if (maxy < a[j].y) maxy = a[j].y;
  }
  for (j = 0; j < nb; j++) {
    if (minx > b[j].x) minx = b[j].x;
    if (miny > b[j].y) miny = b[j].y;
    if (maxx < b[j].x) maxx = b[j].x;
    if (maxy < b[j].y) maxy = b[j].y;
  }
...
static void check_cehs_pair_frames(double **r1, double **r2) {
  ...
  /* sum = dot(z1, z2) */
  for (i = 1; i <= 3; i++) sum += r1[i][3] * r2[i][3];
...
static void check_cehs_step_frames(double **r1, double *o1, double **r2,
  ...
  sum = 0;
  for (i = 1; i <= 3; i++) sum += dd[i] * r1[i][3];
  if (sum < 0) reverse_y_z_columns(r1);

  sum = 0;
  for (i = 1; i <= 3; i++) sum += dd[i] * r2[i][3];
  if (sum < 0) reverse_y_z_columns(r2);
}
...
void cehs_pars(long num_bp, long istart, long istep, long **pair_num,
  ...
  for (i = 1; i <= ds; i++) {
    for (j = 1; j <= num_bp; j++) {
      ...
      for (k = 1; k <= num_batom; k++) cpxyz(xyz[batom[k]], bxyz[k]);
  ...
  for (i = 1; i <= num_bp; i++) { /* c6_c8 atoms exist for sure from above */
    ...
    ie = seidx[rnum][2];
    cehs_base_atoms(AtomName, ib, ie, &num_batom, batom);
    for (j = 1; j <= num_batom; j++) cpxyz(xyz[batom[j]], bxyz[j]);
    ...
    if (dot(z, &orien[1][ioffset + 6]) < 0) negate_xyz(z);
...
void schnaap_global(long num_bp, long num, char **bp_seq, long **chi,
  ...
  if (hrise < EMPTY_CRITERION || std_rise > Gvars.misc_pars.std_curved) return;
  ...
  for (i = 1; i <= num_bp; i++) {
    ...
    for (j = 1; j <= 3; j++) {
      hpars[i][j] = 0.0;
      for (k = 1; k <= 3; k++) hpars[i][j] += hend[k] * rh[k][j];
    }
  }
...
void sgl_helix(double **rot1, double **rot2, double *rot_ang, double *rot_hlx) {
  ...
  for (i = 1; i <= 3; i++) dsum += R[i][i]; /* trace of R */
...
void rna(double **rot1, double *org1, double *pvt1, double **rot2, double *org2,
  ...
  for (i = 1; i <= 3; i++) pars[i + 3] = ang * haxis[i];
  ddxyz(org1, org2, dorg);
  ...
  for (i = 1; i <= 3; i++) pars[i] = p1[i] + p2[i] + pvt1[i] - pvt2[i];
...
void pvt_dxdy(double **rot1, double *org1, double *pvt1, double *pars,
  ...
  for (i = 1; i <= 3; i++) t2[i] = dot(t1, temp[i]) + org1[i] - org1_h[i];

  for (i = 1; i <= 2; i++) {
    pars[i] = 0.0;
    for (j = 1; j <= 3; j++) pars[i] += t2[j] * rot1_h[j][i];
  }
...
void out_rna(long ds, long num_bp, char **bp_seq, long *bphlx, double **orien,
  ...
  for (i = 1; i <= ds; i++) {
    for (j = 1; j <= nbpm1; j++) {
      ...
      for (k = 1; k <= 6; k++) heli_par[j][k] = hpi[k];
    }
    if (i == 1) {
      print_sep(fp, '*', 76);
