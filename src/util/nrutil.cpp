...
void max_dmatrix(double **d, long nr, long nc, double *maxdm) {
  ...
  for (i = 1; i <= nc; i++) {
    maxdm[i] = -XBIG;
    for (j = 1; j <= nr; j++) maxdm[i] = dval_max(maxdm[i], d[j][i]);
  }
}
...
void min_dmatrix(double **d, long nr, long nc, double *mindm) {
  ...
  for (i = 1; i <= nc; i++) {
    mindm[i] = XBIG;
    for (j = 1; j <= nr; j++) mindm[i] = dval_min(mindm[i], d[j][i]);
  }
}
...
void ave_dmatrix(double **d, long nr, long nc, double *avedm) {
  ...
  if (!nr) nrerror("divided by zero in <ave_dmatrix>");

  for (i = 1; i <= nc; i++) {
    avedm[i] = 0.0;
    for (j = 1; j <= nr; j++) avedm[i] += d[j][i];
    avedm[i] /= nr;
  }
}
...
double max_dvector(double *d, long nl, long nh) {
  ...
  for (i = nl; i <= nh; i++) maxdv = dval_max(maxdv, d[i]);
...
double min_dvector(double *d, long nl, long nh) {
  ...
  for (i = nl; i <= nh; i++) mindv = dval_min(mindv, d[i]);
...
void init_cmatrix(char **cmtx, long nrl, long nrh, long ncl, long nch,
  ...
  for (i = nrl; i <= nrh; i++) init_cvector(cmtx[i], ncl, nch, init_val);
}
...
void init_dmatrix(double **dmtx, long nrl, long nrh, long ncl, long nch,
  ...
  for (i = nrl; i <= nrh; i++) init_dvector(dmtx[i], ncl, nch, init_val);
}
...
void init_lmatrix(long **lmtx, long nrl, long nrh, long ncl, long nch,
  ...
  for (i = nrl; i <= nrh; i++) init_lvector(lmtx[i], ncl, nch, init_val);
}
...
void init_cvector(char *cvec, long ib, long ie, char init_val) {
  ...
  for (i = ib; i < ie; i++) cvec[i] = init_val;
  cvec[ie] = '\0';
}
...
void init_dvector(double *dvec, long ib, long ie, double init_val) {
  ...
  for (i = ib; i <= ie; i++) dvec[i] = init_val;
}
...
void init_lvector(long *lvec, long ib, long ie, long init_val) {
  ...
  for (i = ib; i <= ie; i++) lvec[i] = init_val;
}
...
void p1p2_ave(double *xyz1, double *xyz2, double *ave) {
  ...
  for (i = 1; i <= 3; i++) ave[i] = 0.5 * (xyz1[i] + xyz2[i]);
}
...
long within_limits(double *xyz1, double *xyz2, double dlow, double dhigh) {
  ...
  for (i = 1; i <= 3; i++)
    if (fabs(dxyz[i] = xyz1[i] - xyz2[i]) > dhigh) break;
  if (i > 3) {
    d = veclen(dxyz);
    if (dval_in_range(d, dlow, dhigh)) ltok = 1;
  }
  return ltok;
}
...
void sumxyz(double *xyz1, double *xyz2, double *sxyz) {
  ...
  for (i = 1; i <= 3; i++) sxyz[i] = xyz1[i] + xyz2[i];
}
...
void avexyz(double *xyz1, double *xyz2, double *mxyz) {
  ...
  for (i = 1; i <= 3; i++) mxyz[i] = 0.5 * (xyz1[i] + xyz2[i]);
}
...
void ddxyz(double *xyz1, double *xyz2, double *dxyz) {
  ...
  for (i = 1; i <= 3; i++) dxyz[i] = xyz2[i] - xyz1[i];
}
...
void multi_matrix(double **a, long nra, long nca, double **b, long nrb,
  ...
  if (nca != nrb) nrerror("matrices a and b do not conform");

  for (i = 1; i <= nra; i++) {
    for (j = 1; j <= ncb; j++) {
      o[i][j] = 0.0;
      for (k = 1; k <= nca; k++) o[i][j] += a[i][k] * b[k][j];
    }
  }
}
...
void multi_vec_matrix(double *a, long n, double **b, long nr, long nc,
  ...
  if (n != nr) nrerror("vector and matrix do not conform");

  for (i = 1; i <= nc; i++) {
    o[i] = 0.0;
    for (j = 1; j <= n; j++) o[i] += a[j] * b[j][i];
  }
}
