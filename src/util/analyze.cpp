...
static void get_ring_center_normal(long ds, long num_bp, long **pair_num,
  ...
  for (i = 1; i <= ds; i++) {
    ...
    for (j = 1; j <= num_bp; j++) {
      ...
      for (k = 1; k <= 3; k++) fprintf(fp, fmt, cxyz[k]); /* center */
      for (k = 1; k <= 3; k++) fprintf(fp, fmt, nvec[k]); /* base normal */
      fprintf(fp, "\n");
    }
  }
...
static void simple_bp_pars(double **orien, double **org, long NNy, long **c6_c8,
  ...
  fprintf(fp, "Simple base-pair parameters based on %s vectors\n",
          NNy ? "RN9--YN1" : "RC8--YC6");
  fprintf(fp,
          "      bp        Shear    Stretch   Stagger    Buckle  Propeller  "
          "Opening\n");
...
static void simple_step_heli_pars(double **orien, double **org, long **chi,
  ...
  if (hel_pars) {
    fprintf(fp,
            "Simple base-pair helical parameters based on consecutive C1'-C1' "
            "vectors\n");
    fprintf(fp,
    ...
    fprintf(fp,
            "Simple base-pair step parameters based on consecutive C1'-C1' "
            "vectors\n");
    fprintf(fp,
            "      step       Shift     Slide      Rise      Tilt      Roll    "
            " Twist\n");
  }
  ...
  for (i = 1; i < num_bp; i++) {
    ...
    ddxyz(p1 + 9, p2 + 9, dd);
    if (dot(dd, p1 + 6) < 0) reverse_y_z_columns(r1);
    if (dot(dd, p2 + 6) < 0) reverse_y_z_columns(r2);
...
static void check_simple_parameters(long simple, double **orien, double **org,
  ...
  print_sep(fp, '*', 76);
  fprintf(fp,
          "The 'simple' parameters are intuitive for non-Watson-Crick base "
          "pairs\n");
  fprintf(
      fp,
  ...
  fprintf(fp,
          "*description* only, not to be fed into the 'rebuild' program. See "
          "URL\n");
  fprintf(fp,
          "http://x3dna.org/highlights/"
...
static void process_str(char *inpfile, struct_args *args) {
  ...
  for (i = 1; i <= num_bp; i++) bphlx[i] = args->icnt ? 0 : pair_num[ds + 1][i];
...
static long derive_simple_pars_settings(char *option) {
  ...
  if (lux_ncmatch(option, "no|false|off")) return k;

  k = true; /* default: RC8--YC6, no helical pars */

  if (lux_ncmatch(option, "n1|n9")) k |= SIMPLE_BP_LONG_AXIS_RN9_YN1;

  if (lux_ncmatch(option, "heli")) k |= SIMPLE_STEP_HELICAL_PARS;
...
static void analyze_cmdline(int argc, char *argv[], struct_args *args) {
  ...
  for (i = 1; i < argc; i++) {
    ...
    if (check_global_options(argv[i])) continue;
