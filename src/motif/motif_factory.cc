...
MotifOP MotifFactory::motif_from_file(String const& path, bool rebuild_x3dna,
                                      bool include_protein,
                                      int force_num_chains) {
  if (!base::file_exists(path)) {
    throw MotifFactoryException("cannot generate motif from file " + path +
                                " does not exist");
  }
  ...
  if (base::is_dir(path)) {
    ...
    if (!base::file_exists(pdb_path)) {
      throw MotifFactoryException("cannot generate motif from directory " +
                                  path +
                                  " it exists but there is no pdb "
  ...
  if (force_num_chains != -1 &&
      structure->chains().size() != force_num_chains) {
  ...
  if (include_protein) {
    ...
    for (auto const& r : res) {
      try {
        auto bead = structure::Bead(r->get_atom("CA")->coords(),
                                    structure::BeadType::BASE);
...
MotifOP MotifFactory::get_oriented_motif(MotifOP const& m, int end_pos) {
  ...
  auto m_added_2_1 =
      get_aligned_motif(m->ends()[end_pos], m_added->ends()[0], m_added);
...
MotifOP MotifFactory::motif_from_res(structure::ResidueOPs& res,
  ...
  if (ends.size() == 0 && bps.size() >= 2) {
    std::cout << ends.size() << " " << bps.size() << " " << chains.size()
              << std::endl;
...
MotifOP MotifFactory::motif_from_bps(structure::BasepairOPs const& bps) {
...
MotifOP MotifFactory::can_align_motif_to_end(MotifOP const& m, int ei) {
  ...
  for (auto& end : m_added->ends()) {
    i++;
    if (end->name() == m_end->name()) {
      continue;
    }
    auto added_helix_1 =
        get_aligned_motif(end, added_helix_->ends()[0], added_helix_);
    end->flip();
    auto added_helix_2 =
        get_aligned_motif(end, added_helix_->ends()[0], added_helix_);
    end->flip();

    clash_count_1 = _steric_clash_count(m_added, added_helix_1) +
                    _steric_clash_count(base_motif_, added_helix_1);
    clash_count_2 = _steric_clash_count(m_added, added_helix_2) +
                    _steric_clash_count(base_motif_, added_helix_2);
...
void MotifFactory::standardize_rna_structure_ends(MotifOP& m) {
  m->get_beads(m->ends());
  for (auto const& end : m->ends()) {
    auto m_added =
        get_aligned_motif(end, added_helix_->ends()[0], added_helix_);
    if (_steric_clash(m, m_added) == 0) {
      continue;
    } else {
      end->flip();
    }
...
MotifOP MotifFactory::align_motif_to_common_frame(MotifOP const& m, int ei) {
  auto m_added = get_aligned_motif(ref_motif_->ends()[0], m->ends()[ei], m);
  return m_added;
}

void MotifFactory::_standardize_motif(MotifOP& m) {
  _align_chains(m);
  _align_ends(m);
  _setup_secondary_structure(m);
}

structure::BasepairOPs MotifFactory::_setup_basepairs(
    String const& path, structure::StructureOP const& s, bool rebuild_x3dna) {
  ...
  for (auto const& xbp : x_basepairs) {
    ...
    if (res1 == nullptr || res2 == nullptr) {
      ...
      throw MotifFactoryException(
          "cannot find residues in basepair during setup");
    }

    // Emplacement constructs the object in-place
    basepairs.emplace_back(new structure::Basepair(
        res1, res2, xbp.r, util::get_str_from_x3dna_type(xbp.bp_type)));
  }
  ...
  for (auto const& xbp : x_basepairs) {
    ...
    res1 = s->get_residue(xbp.res1.num, String(1, xbp.res1.chain_id), i_code_1);
    res2 = s->get_residue(xbp.res2.num, String(1, xbp.res2.chain_id), i_code_2);
    std::cout << res1->num() << " " << res1->name() << " " << res2->num() << " "
              << res2->name() << std::endl;
    if (res1 == nullptr || res2 == nullptr) {
      throw MotifFactoryException(
          "cannot find residues in basepair during setup");
...
structure::BasepairOPs MotifFactory::_setup_basepair_ends(
  ...
  for (auto const& bp : basepairs) {
    for (auto const& ce1 : chain_ends) {
      for (auto const& ce2 : chain_ends) {
        if (bp->bp_type() == "cW-W" && bp->res1()->uuid() == ce1->uuid() &&
...
void MotifFactory::_setup_secondary_structure(MotifOP& m) {
  ...
  ss->end_ids(end_ids);
  m->end_ids(end_ids);
  m->secondary_structure(
      std::static_pointer_cast<secondary_structure::Motif>(ss));
}

void MotifFactory::_align_chains(MotifOP& m) {
...
void MotifFactory::_align_ends(MotifOP& m) {
...
int MotifFactory::_steric_clash(MotifOP const& m1, MotifOP const& m2) {
...
int MotifFactory::_steric_clash_count(MotifOP const& m1, MotifOP const& m2) {
  auto count = 0;
  for (auto const& c1 : m1->beads()) {
    for (auto const& c2 : m2->beads()) {
      if (c1.btype() == structure::BeadType::PHOS ||
...
int MotifFactory::_bead_overlap(MotifOP const& m1, MotifOP const& m2) {
  auto count = 0;
  auto dist = 0.0;
...
structure::StructureOP MotifFactory::_get_reduced_chain_num_structure(
    structure::Structure const& start, int chain_num) {
...
MotifOPs get_standardize_motifs(MotifFactory& mf, String const& path) {
  ...
  if (motifs.size() == 0) {
    throw MotifFactoryException("attempted to add motif from path " + path +
                                " unforunately it has no viable "
