...
Motif::Motif(String const &s, structure::ResidueTypeSet const &rts)
    : structure::RNAStructure(), id_(util::Uuid()) {

  if (s.length() < 10) {
    throw base::RNAMakeException(
        "tried to construct Motif object from string, with a string too short");
  }
  ...
  for (auto const &bp_str : basepair_str) {
    ...
    auto bp = std::make_shared<structure::Basepair>(res1, res2, bpstate.r(),
                                                    bp_spl[2]);
...
Motif::Motif(Motif const &m) {
  ...
  secondary_structure_ =
      std::make_shared<secondary_structure::Motif>(*m.secondary_structure_);
  ...
  for (auto const &bp : m.basepairs_) {
    ...
    auto new_bp = std::make_shared<structure::Basepair>(res1, res2, bp->r(),
                                                        bp->bp_type());
...
String const Motif::to_str() {
  std::stringstream ss;
  ss << path_ << "&" << name_ << "&" << int(score_) << "&" << block_end_add_
     << "&" << (int)mtype_;
...
void Motif::copy_uuids_from_motif(Motif const &m) {
  id_ = m.id_;
...
MotifStateOP Motif::get_state() {
  ...
  auto ms = MotifState(name_, end_names, end_ids_, end_states, bead_centers,
                       score_, (int)residues().size(), block_end_add_, id_);
  return std::make_shared<MotifState>(ms);
}

void align_motif(structure::BasepairStateOP const &ref_bp_state,
                 structure::BasepairOP const &motif_end, MotifOP &motif) {
  ...
  auto dist1 = motif_end->res1()->get_atom("C1'")->get_coords().distance(
      ref_bp_state->sugars()[0]);
  auto dist2 = motif_end->res2()->get_atom("C1'")->get_coords().distance(
      ref_bp_state->sugars()[0]);
  ...
  if (dist1 < dist2) {
    sugar_diff_1 = ref_bp_state->sugars()[0] -
                   motif_end->res1()->get_atom("C1'")->get_coords();
    sugar_diff_2 = ref_bp_state->sugars()[1] -
                   motif_end->res2()->get_atom("C1'")->get_coords();
  } else {
    sugar_diff_1 = ref_bp_state->sugars()[0] -
                   motif_end->res2()->get_atom("C1'")->get_coords();
    sugar_diff_2 = ref_bp_state->sugars()[1] -
...
MotifOP get_aligned_motif(structure::BasepairOP const &ref_bp,
...
MotifOP file_to_motif(String const &path) {
  auto lines = base::get_lines_from_file(path);
  return std::make_shared<Motif>(
      lines[0],
...
Motif ref_motif() {
  ...
  auto m = Motif(
      line, structure::ResidueTypeSetManager::getInstance().residue_type_set());
  return m;
}
