...
class MotifEnsemble {
 ...
 public:
  ...
  MotifEnsemble(MotifOPs const &motifs, Floats const &energies)
    ...
    if (motifs.size() > 0) {
      ...
      for (auto const &m : motifs) {
        if (block_end_add_ != m->block_end_add()) {
          throw MotifEnsembleException(
              "all motifs included in the ensemble must have the same "
              "block_end_add_");
        }
      }
    }
  ...
  MotifEnsemble(  // extra ctor added by cj 10/20
      String const &id, MotifOPs const &motifs, Floats const &energies)
...
void motif_ensemble_from_csv_file(String const &,
