...
public:
   ...
   */
  MotifEnsembleException(String const &message) : std::runtime_error(message) {}
};

class MotifEnsemble {
private:
  struct MotifEnsembleMember {
    MotifEnsembleMember(MotifOP const &nmotif, float const &nenergy)
        : motif(nmotif), energy(nenergy) {}

    String to_str() { return motif->to_str() + "#" + std::to_string(energy); }
...
public:
  MotifEnsemble()
      : id_(""), block_end_add_(0), members_(MotifEnsembleMemberOPs()) {}

  MotifEnsemble(MotifOPs const &motifs, Reals const &energies)
      : members_(MotifEnsembleMemberOPs()), block_end_add_(0) {
    ...
    if (motifs.size() > 0) {
      ...
      for (auto const &m : motifs) {
        if (block_end_add_ != m->block_end_add()) {
          throw MotifEnsembleException("all motifs included in the ensemble "
                                       "must have the same block_end_add_");
        }
      }
    }
  ...
  MotifEnsemble( // extra ctor added by cj 10/20
      String const &id, MotifOPs const &motifs, Reals const &energies)
      : MotifEnsemble(motifs, energies) {
    id_ = id;
  }

  MotifEnsemble(MotifEnsemble const &me)
      : id_(me.id_), block_end_add_(me.block_end_add_),
  ...
  MotifEnsemble(String const &s, structure::ResidueTypeSet const &rts) {
...
public:
  ...
  MotifStateEnsembleOP get_state();

  MotifEnsembleMemberOPs const &members() { return members_; }

  void id(String const &id) { id_ = id; }
