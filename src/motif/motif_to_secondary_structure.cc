  ...
  while (!open_chains_.empty()) {
    ...
    for (auto const& r : c->residues()) {
      ...
      for (auto const& bp : bps) {
        ...
        if (wc_bp(bp) && bp->bp_type() == "cW-W") {
          passes = 1;
        }
        if (gu_bp(bp) && bp->bp_type() == "cW-W") {
          passes = 1;
        }
      ...
      if (saved_bp != nullptr) {
        seen_bp_[saved_bp->uuid()] = saved_bp;
      }
      if (r != nullptr) {
        ss_res.push_back(std::make_shared<secondary_structure::Residue>(
            r->short_name(), ss, r->num(), r->chain_id(), r->uuid(),
            r->i_code()));
      }
    }
    ...
    chains_.erase(std::remove(chains_.begin(), chains_.end(), best_chain),
                  chains_.end());
...
structure::ChainOP MotiftoSecondaryStructure::_get_next_chain(
    structure::RNAStructureOP const& motif) {
  structure::BasepairOPs bps;
  int best_score = -1;
  int score = 0;
...
MotiftoSecondaryStructure::_setup_basepairs_and_ends(
  ...
  for (auto const& kv : seen_bp_) {
    ...
    if (res1 == nullptr || res2 == nullptr) {
      throw std::runtime_error(
          "did not properly find residues for basepairs in ss");
    }
    ss_bps.push_back(std::make_shared<secondary_structure::Basepair>(
        res1, res2, bp->uuid()));
  }
  ...
  return std::make_shared<secondary_structure::RNAStructure>(struc, ss_bps,
                                                             ss_ends);
}
