...
MotifStateEnsemble::MotifStateEnsemble(String const& s)
    : id_(""), block_end_add_(0), members_(MotifStateEnsembleMemberOPs()) {
...
MotifStateEnsemble::MotifStateEnsemble(MotifStateOPs const& motif_states,
                                       Floats const& energies) {
  if (motif_states.size() != energies.size()) {
    throw MotifStateEnsembleException(
        "must supply the same number of motif_states and energies");
  }

  if (motif_states.size() == 0) {
    throw MotifStateEnsembleException(
        "must supply at least one motif state to ensemble");
  }

  for (int i = 0; i < motif_states.size(); i++) {
    auto member = std::make_shared<MotifStateEnsembleMember>(motif_states[i],
                                                             energies[i]);
    members_.push_back(member);
  }

  std::sort(members_.begin(), members_.end(),
            MotifStateEnsembleMember_LessThanKey());
  block_end_add_ = members_[0]->motif_state->block_end_add();
  id_ = members_[0]->motif_state->end_ids()[0];
}

String MotifStateEnsemble::to_str() {
