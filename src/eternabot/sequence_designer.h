...
struct SequenceDesignerResult {
  inline SequenceDesignerResult(String const &n_sequence, float n_score)
      : sequence(n_sequence), score(n_score) {}
...
struct sequence_designer_result_less_than_key {
  inline bool operator()(SequenceDesignerResultOP const &r1,
...
public:
  ...
  SequenceDesignerResultOPs const &design(secondary_structure::PoseOP const &);
...
public: // option wrappers
  ...
  inline bool has_option(String const &name) {
    return options_.has_option(name);
  }
...
private:
  Strings const &_get_random_pair();

  void _set_bp_sequence(Strings const &, secondary_structure::BasepairOP);

  void _find_designable_bps(secondary_structure::PoseOP);

  void _generate_inital_sequence(secondary_structure::PoseOP);

  bool _new_sequence_violations();
