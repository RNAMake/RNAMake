...
FeaturesOP
FeatureGenerator::get_features(secondary_structure::PoseOP const &p) {
...
void FeatureGenerator::update_features(FeaturesOP &features,
  ...
  for (auto const &bp : p->basepairs()) {
    if (secondary_structure::is_gc_pair(bp)) {
      features->gc += 1;
    } else if (secondary_structure::is_au_pair(bp)) {
      features->ua += 1;
    } else if (secondary_structure::is_gu_pair(bp)) {
      features->gu += 1;
    }
  }

  for (auto const &r : p->residues()) {
    if (r->res_type() == secondary_structure::ResType::ADE) {
      features->a_count += 1;
    } else if (r->res_type() == secondary_structure::ResType::CYT) {
      features->c_count += 1;
    } else if (r->res_type() == secondary_structure::ResType::GUA) {
      features->g_count += 1;
    } else if (r->res_type() == secondary_structure::ResType::URA) {
      features->u_count += 1;
    }
  }
