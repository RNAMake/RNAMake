...
SequenceDesigner::SequenceDesigner()
    : scorer_(Scorer()), results_(SequenceDesignerResultOPs()),
  ...
  for (auto const &seq : disallowed_sequences) {
    seq_constraints_.add_disallowed_sequence(seq);
  }
...
void SequenceDesigner::setup_options() {
...
void SequenceDesigner::update_var_options() {
...
SequenceDesigner::design(secondary_structure::PoseOP const &p) {
  ...
  _generate_inital_sequence(
      p); // generate initial sequence, get rid as many violations as possible
  scorer_.setup(p);
  ...
  results_.push_back(
      std::make_shared<SequenceDesignerResult>(best_sequence, best_score));
  return results_;
}
...
Strings const &SequenceDesigner::_get_random_pair() {
  return possible_bps_[rng_.randrange(possible_bps_.size())];
}

void SequenceDesigner::_set_bp_sequence(Strings const &pair,
                                        secondary_structure::BasepairOP bp) {
  bp->res1()->name(pair[0]);
  bp->res2()->name(pair[1]);
}

void SequenceDesigner::_find_designable_bps(secondary_structure::PoseOP p) {
  ...
  if (designable_bps_.size() == 0) {
    LOG_ERROR << "no basepairs are designable! ";
    exit(0);
  }
}

bool SequenceDesigner::_new_sequence_violations() {
...
void SequenceDesigner::_generate_inital_sequence(
  ...
  while (_new_sequence_violations()) {
    ...
    _set_bp_sequence(pair, designable_bps_[pos]);
    if (count > 1000000) {
      LOG_WARNING << "cannot find initial sequence that does not have sequence "
                     "violations! ";
