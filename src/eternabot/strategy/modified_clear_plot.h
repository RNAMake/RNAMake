...
#ifndef __RNAMake_mod_clear_plot__
#define __RNAMake_mod_clear_plot__
...
class ModifiedCleanPlotStackCapsandSafeGC : public Strategy {
public:
    ModifiedCleanPlotStackCapsandSafeGC() {
...
public:
    ...
    score(FeaturesOP const & features) {
        ...
        float plotscore = (1.0 - (penalty / (npairs+1)));
        float gc_penalty = 0;
        if(npairs > 0) {
        ...
        float score =  (2.0 + cap_score * params_[1] + plotscore * params_[0] - gc_penalty * params_[2]) * 50;
