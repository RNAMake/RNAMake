...
public:
  float score(FeaturesOP const &features) {
    ...
    float score = 100;
    score -=
        fabsf(features->g_count / features->length - params_[0]) * params_[1];
    score -=
        fabsf(features->u_count / features->length - params_[2]) * params_[3];
    score -=
        fabsf(features->c_count / features->length - params_[4]) * params_[5];
