...
public:
  MotifMergerException(String const &message) : std::runtime_error(message) {}
};

struct ChainNodeData {
  inline ChainNodeData() {}

  inline ChainNodeData(structure::ChainOP const &nc, util::Uuid const &nm_id,
                       int nprime5_override = 0, int nprime3_override = 0)
      : c(nc), m_id(nm_id), prime5_override(nprime5_override),
        prime3_override(nprime3_override) {}

  inline ChainNodeData(ChainNodeData const &cnd)
      : c(cnd.c), m_id(cnd.m_id), prime5_override(cnd.prime5_override),
        prime3_override(cnd.prime3_override) {}

  structure::ResidueOPs included_res() {
    auto res = c->residues();
    if (prime5_override == 1) {
      res.erase(res.begin());
    }
    if (prime3_override == 1) {
      res.pop_back();
    }
    return res;
  }
...
public:
  MotifMerger()
      : all_bps_(
            std::map<util::Uuid, structure::BasepairOP, util::UuidCompare>()),
  ...
  MotifMerger(MotifMerger const &mm, motif::MotifOPs const &motifs)
      : all_bps_(
            std::map<util::Uuid, structure::BasepairOP, util::UuidCompare>()),
        motifs_(std::map<util::Uuid, motif::MotifOP, util::UuidCompare>()),
        res_overrides_(mm.res_overrides_), bp_overrides_(mm.bp_overrides_),
        rna_structure_(std::make_shared<structure::RNAStructure>()),
...
public:
  void add_motif(motif::MotifOP const &);

  void add_motif(motif::MotifOP const &, structure::BasepairOP const &,
                 motif::MotifOP const &, structure::BasepairOP const &);

  structure::RNAStructureOP const &get_structure();

  void remove_motif(motif::MotifOP const &);

  void update_motif(motif::MotifOP const &);

  void connect_motifs(motif::MotifOP const &m1, motif::MotifOP const &m2,
...
public:
  inline structure::ResidueOP get_residue(util::Uuid const &uuid) {
  ...
  inline structure::BasepairOP get_basepair(util::Uuid const &uuid) {
  ...
  inline void to_pdb(String const &fname, int renumber = -1,
                     int close_chains = 0, int conect_statements = 0) {
    return get_structure()->to_pdb(fname, renumber, close_chains,
                                   conect_statements);
  }

private:
  void _link_motifs(motif::MotifOP const &, structure::BasepairOP const &,
                    motif::MotifOP const &, structure::BasepairOP const &);

  ChainNodes _get_end_nodes(ChainNodes const &, structure::BasepairOP const &);

  void _link_chains(ChainNodes &, ChainNodes &, MotifMergerType const &,
                    MotifMergerType const &);

  void _connect_chains(ChainNode &, ChainNode &, int, int,
                       MotifMergerType const &, MotifMergerType const &);

  void _build_structure();

  MotifMergerType _assign_merger_type(motif::MotifOP const &);

private:
  std::map<util::Uuid, structure::BasepairOP, util::UuidCompare> all_bps_;
  std::map<util::Uuid, motif::MotifOP, util::UuidCompare> motifs_;
  std::map<util::Uuid, util::Uuid, util::UuidCompare> res_overrides_,
      bp_overrides_;
