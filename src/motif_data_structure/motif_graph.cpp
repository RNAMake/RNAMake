...
MotifGraph::MotifGraph(String const &s, MotifGraphStringType const &s_type)
    : MotifGraph() {

  if (s_type == MotifGraphStringType::MG) {
    _setup_from_str(s);
  } else if (s_type == MotifGraphStringType::TOP) {
    _setup_from_top_str(s);
  } else {
    throw MotifGraphException(
        "cannot destringify motif graph wrong string type");
  }
}
...
void MotifGraph::update_indexes(std::map<int, int> const &index_hash) {
...
Indexes MotifGraph::_get_available_parent_end_pos(
  ...
  if (parent_end_index != -1) {
    ...
    if (!avail) {
      throw MotifGraphException(
          ...
          std::to_string(parent_end_index) +
          " you are trying to "
...
int MotifGraph::_add_motif_to_graph(
    motif::MotifOP &m,
    data_structure::graph::GraphNodeOP<motif::MotifOP> const &parent,
    int parent_end_index) {
...
int MotifGraph::_steric_clash(motif::MotifOP const &m) {
  float dist = 0;
  for (auto const &n : graph_) {
    for (auto const &c1 : n->data()->beads()) {
      if (c1.get_type() == util::BeadType::PHOS) {
        continue;
      }
      for (auto const &c2 : m->beads()) {
        if (c2.get_type() == util::BeadType::PHOS) {
          continue;
        }
...
int MotifGraph::_get_connection_end(
  ...
  if (bp_name != "") {
    ...
    if (!node->available_pos(ei)) {
      throw MotifGraphException("cannot add connection with " +
                                std::to_string(node->index()) +
                                " and "
                                "end name " +
                                bp_name + " as the end is blocked");
    }
    ...
    if (node_indexes.size() > 1) {
      throw MotifGraphException(
          "cannot connect nodes " + std::to_string(node->index()) +
          " its unclear "
          " which ends to attach as there is more then one possibility");
    }

    if (node_indexes.size() == 0) {
      throw MotifGraphException("cannot connect nodes " +
                                std::to_string(node->index()) +
                                " there are "
...
void MotifGraph::_align_motifs_all_motifs() {
  _update_align_list();
  auto non_aligned_nodes = unaligned_nodes();
  for (auto &n : align_list_) {
    if (std::find(non_aligned_nodes.begin(), non_aligned_nodes.end(), n) !=
        non_aligned_nodes.end()) {
      continue;
    }
...
int MotifGraph::add_motif(motif::MotifOP const &m, int parent_index,
  ...
  if (parent == nullptr) {
    throw MotifGraphException("cannot add motif: " + m->name() +
                              " as there are no open parents to add too");
  }

  try {
    parent_end_index = parent->data()->get_end_index(p_end_name);
  } catch (structure::RNAStructureException const &e) {
    throw MotifGraphException("cannot find parent_end_name: " + p_end_name +
                              " in "
                              "parent motif: " +
                              parent->data()->name());
  }
...
int MotifGraph::add_motif(motif::MotifOP const &m, int parent_index,
                          int parent_end_index, int orphan) {

  for (auto const &n : graph_.nodes()) {
    if (n->data()->id() == m->id()) {
      throw MotifGraphException("cannot add motif there is already a motif in "
                                "this graph with its unique "
                                " indentifier");
    }
  }
  ...
  for (auto const &p : avail_pos) {
    auto m_added =
        get_aligned_motif(parent->data()->ends()[p], m->ends()[0], m);
    if (sterics_ && _steric_clash(m_added)) {
      continue;
    }
...
void MotifGraph::add_motif_tree(MotifTreeOP const &mt, int parent_index,
...
void MotifGraph::_add_motif_tree(MotifTreeOP const &mt, int parent_index,
                                 int parent_end_index) {
...
void MotifGraph::add_motif_graph(MotifGraph &mg, int parent_index,
                                 int parent_end_index) {
...
void MotifGraph::add_connection(int i, int j, String const &i_bp_name,
  ...
  try {
    node_i = graph_.get_node(i);
  } catch (data_structure::graph::GraphException) {
    throw MotifTreeException("cannot connect: " + std::to_string(i) + " " +
                             std::to_string(j) + " as node " +
                             std::to_string(i) + " does not exist");
  }

  try {
    node_j = graph_.get_node(j);
  } catch (data_structure::graph::GraphException) {
    throw MotifTreeException("cannot connect: " + std::to_string(i) + " " +
                             std::to_string(j) + " as node " +
...
void MotifGraph::remove_motif(int pos) {
...
void MotifGraph::remove_level(int level) {
  int pos = 0;
...
void MotifGraph::replace_ideal_helices() {
  int found = 1;
  while (found) {
    found = 0;
    for (auto const &n : graph_) {
      ...
      if (parent == nullptr) {
        ...
        if (old_n_aligned == 0) {
          ...
          auto m_added =
              get_aligned_motif(old_n->data()->ends()[0],
                                new_n->data()->ends()[0], new_n->data());
          n->data() = m_added;
        }

      } else {
        auto m_added = get_aligned_motif(
            parent->data()->ends()[parent_end_index], h->ends()[0], h);
      ...
      for (int j = 0; j < count; j++) {
        ...
        auto m_added =
            get_aligned_motif(parent->data()->ends()[1], h->ends()[0], h);
...
void MotifGraph::replace_helical_sequence(
  ...
  for (auto &n : graph_.nodes()) {
    ...
    if (ss_m == nullptr) {
      throw MotifGraphException(
          "could not find ss motif, cannot update helical sequence");
    }

    // std::cout << ss_m->end_ids()[0] << " " << ss_m->sequence() << " " <<
    // new_name << std::endl;
    if (n->data()->end_ids()[0] == ss_m->end_ids()[0] &&
        n->data()->name() != "HELIX.IDEAL") {
      continue;
    }
...
void MotifGraph::write_pdbs(String const &fname) {
  std::stringstream ss;
  for (auto const &n : graph_.nodes()) {
...
String MotifGraph::topology_to_str() {
  String s = "", con_str = "";
  String key1 = "", key2 = "";
  ...
  for (auto const &n : graph_.nodes()) {
    ...
    s += std::to_string(n->index()) + "," +
         std::to_string(aligned_[n->index()]) + "|";
    for (auto const &c : n->connections()) {
      if (c == nullptr) {
        continue;
      }
      key1 = std::to_string(c->node_1()->index()) + " " +
             std::to_string(c->node_2()->index());
      key2 = std::to_string(c->node_2()->index()) + " " +
             std::to_string(c->node_1()->index());
      if (seen_connections.find(key1) != seen_connections.end() ||
          seen_connections.find(key2) != seen_connections.end()) {
        continue;
      }
      seen_connections[key1] = 1;
      con_str += std::to_string(c->node_1()->index()) + "," +
                 std::to_string(c->node_2()->index()) + ",";
      con_str += std::to_string(c->end_index_1()) + "," +
                 std::to_string(c->end_index_2()) + "|";
    }
  }
  s += "&";
  s += con_str;
...
void MotifGraph::_update_align_list() {
  if (!update_align_list_) {
    return;
  }
  ...
  auto used_nodes =
      std::map<data_structure::graph::GraphNodeOP<motif::MotifOP>, int>();
  ...
  for (auto const &start : non_aligned_nodes) {
    ...
    while (!open.empty()) {
      ...
      for (auto const &c : n->connections()) {
        i++;
        if (i == n->data()->block_end_add() || c == nullptr) {
          continue;
        }
        auto partner_n = c->partner(n->index());
        if (seen_nodes.find(partner_n) != seen_nodes.end() ||
            used_nodes.find(partner_n) != used_nodes.end()) {
          continue;
        }
        if (c->end_index(partner_n->index()) ==
            partner_n->data()->block_end_add()) {
          open.push(partner_n);
...
void MotifGraph::_update_merger() {
  if (!update_merger_) {
    return;
  }
  ...
  auto non_aligned_nodes = unaligned_nodes();
  auto seen_connections =
      std::map<data_structure::graph::GraphConnectionOP<motif::MotifOP>, int>();
  ...
  for (auto const &n : align_list_) {
    ...
    merger_->add_motif(n->data(), n->data()->ends()[0], parent->data(),
                       parent->data()->ends()[pei]);
  }
  ...
  for (auto const &n : align_list_) {
    for (auto const &c : n->connections()) {
      ...
      if (seen_connections.find(c) != seen_connections.end()) {
        continue;
      }
...
structure::BasepairOP const &MotifGraph::get_available_end(int pos) {
  auto n = graph_.get_node(pos);
  auto avail_pos = n->available_children_pos();

  if (avail_pos.size() == 1) {
    throw MotifGraphException("attempting to get end with pos " +
                              std::to_string(pos) +
...
MotifGraph::get_available_end(int pos, String const &end_name) {
...
structure::BasepairOP MotifGraph::get_available_end(String const &m_name,
  ...
  for (auto const &n : graph_.nodes()) {
    if (n->data()->name() == m_name) {
      if (node != nullptr) {
        throw MotifGraphException("attempting to get end with m_name: " +
                                  m_name + " and end name " + end_name +
                                  " but there are MULTIPLE nodes with this "
                                  "name, dont know which to pick");
      }
      node = n;
    }
  }
  ...
  if (node == nullptr) {
    throw MotifGraphException("attempting to get end with m_name: " + m_name +
                              " and end name " + end_name +
  ...
  for (auto const &e : node->data()->ends()) {
    if (e->get_name_str() == end_name) {
      if (end != nullptr) {
        throw MotifGraphException(
            "attempting to get end with m_name: " + m_name + " and end name " +
            end_name +
  ...
  if (!node->available_pos(end_index)) {
    throw MotifGraphException(
        "attempting to get end with m_name " + m_name + "and end_name " +
        end_name +
        " but the end with this end name is not available to build from");
  }
...
MotifGraph::unaligned_nodes() const {
  ...
  for (auto const &kv : aligned_) {
    if (kv.second == 0) {
      nodes.push_back(get_node(kv.first));
    }
  }
  return nodes;
}
