...
MotifTree::MotifTree()
    : tree_(data_structure::tree::TreeStatic<motif::MotifOP>()),
      merger_(nullptr), update_merger_(1), connections_(MotifConnections()),
      options_(base::Options()) {
  setup_options();
}

MotifTree::MotifTree(String const &s)
    : tree_(data_structure::tree::TreeStatic<motif::MotifOP>()),
      merger_(nullptr), update_merger_(1), connections_(MotifConnections()),
      options_(base::Options()) {
  setup_options();
  ...
  for (auto const &e : node_spl) {
    ...
    try {
      ...
      throw MotifTreeException(String("could not get motif did you forget to "
    ...
    if (pos == -1) {
      throw MotifTreeException("failed to add " + m->name() + " pos " +
                               std::to_string(i) +
                               " in the tree "
...
MotifTree::MotifTree(String const &s, MotifTreeStringType type)
    : tree_(data_structure::tree::TreeStatic<motif::MotifOP>()),
      merger_(nullptr), update_merger_(1), connections_(MotifConnections()),
      options_(base::Options()) {
...
MotifTree::MotifTree(MotifTree const &mt)
...
void MotifTree::_setup_from_str(String const &mt_str) {
  set_option_value("sterics", false);
  ...
  for (auto const &n_str : node_spl) {
    ...
    tree_.add_data(m, (int)m->ends().size(), std::stoi(n_spl[2]),
                   std::stoi(n_spl[3]));
...
MotifTree::_get_parent(int parent_index) {
  ...
  try {
    ...
    throw MotifTreeException("could not add motif with parent index: " +
...
Indexes MotifTree::_get_available_parent_end_pos(
  ...
  if (parent_end_index != -1) {
    ...
    auto name = parent->data()->ends()[parent_end_index]->get_name_str();
...
int MotifTree::_steric_clash(motif::MotifOP const &m) {
  float dist = 0;
...
int MotifTree::_get_connection_end(
  ...
  if (bp_name != "") {
    ...
    if (connections_.in_connection(node->index(), bp_name)) {
      throw MotifTreeException("cannot add connection with " +
    ...
    if (node_indexes.size() == 0) {
      throw MotifTreeException("cannot connect nodes " +
                               std::to_string(node->index()) +
                               " there are "
...
int MotifTree::add_motif(motif::MotifOP const &m, int parent_index,
  ...
  try {
    ...
    throw MotifTreeException("cannot find parent_end_name: " + parent_end_name +
...
int MotifTree::add_motif(motif::MotifOP const &m, int parent_index,
  ...
  for (auto const &n : tree_) {
    if (n->data()->id() == m->id()) {
      throw MotifTreeException("cannot add motif there is already a motif in "
                               "this tree with its unique "
                               " indentifier");
    }
  }
  ...
  for (auto const &p : avail_pos) {
    ...
    int pos = tree_.add_data(m_added, (int)m_added->ends().size(),
                             parent->index(), p);
    return pos;
  }
...
void MotifTree::add_connection(int i, int j, String const &i_bp_name,
...
void MotifTree::remove_node(int i) {
  ...
  try {
    ...
    throw MotifTreeException("cannot remove node with index: " +
                             std::to_string(i) + " as it does not exist");
  }
}

void MotifTree::remove_node_level(int level) {
...
void MotifTree::write_pdbs(String const &fname) {
  for (auto const &n : tree_) {
    n->data()->to_pdb(fname + "." + std::to_string(n->index()) + ".pdb");
  }
}

String MotifTree::topology_to_str() {
  String s;

  for (auto const &n : tree_) {
    s += n->data()->name() + "," + n->data()->ends()[0]->get_name_str() + ",";
    s +=
        n->data()->end_ids()[0] + "," + std::to_string(n->parent_index()) + ",";
...
void MotifTree::_update_merger() {
  ...
  for (auto const &n : tree_) {
    ...
    merger_->add_motif(n->data(), n->data()->ends()[0], n->parent()->data(),
                       n->parent()->data()->ends()[pei]);
  }
...
void MotifTree::update_var_options() {
