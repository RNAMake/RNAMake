...
MotifStateEnsembleTree::MotifStateEnsembleTree(MotifTreeOP const &mt)
  ...
  for (auto const &n : *mt) {
    ...
    auto found_supplied =
        resources::Manager::instance().has_supplied_motif_ensemble(
    ...
    // extra motif ensemble supplied by user
      ...
      std::cout
          << "MOTIF STATE ENSEMBLE TREE: found supplied ensemble for name=";
      std::cout << n->data()->name() << " endname=" << n->data()->end_name(0)
                << std::endl;
      mse = resources::Manager::instance()
                .get_supplied_motif_ensemble(n->data()->name(),
                                             n->data()->end_name(0))
                ->get_state();
    ...
    if (i == 0) {
      ...
      if (j == -1) {
        throw MotifStateTreeEnsembleException(
            "failed to add ensemble in setup_from_mt");
      }
    }
  }
...
MotifStateEnsembleTree::MotifStateEnsembleTree(MotifStateTreeOP const &mst)
    : MotifStateEnsembleTree(mst->to_motif_tree()) {}

// add functions
// ////////////////////////////////////////////////////////////////////////////////////

int MotifStateEnsembleTree::add_ensemble(
    motif::MotifStateEnsembleOP const &ensemble, int parent_index,
  ...
  if (parent == nullptr) {
    return tree_.add_data(
        std::make_shared<motif::MotifStateEnsemble>(*ensemble),
  ...
  for (auto const &p : avail_pos) {
    ...
    return tree_.add_data(
        std::make_shared<motif::MotifStateEnsemble>(*ensemble),
        ensemble->num_end_states(), parent->index(), p);
  }

  return -1;
}

MotifStateTreeOP MotifStateEnsembleTree::to_mst() {
...
void MotifStateEnsembleTreeEnumerator::record(String fname) {
  ...
  while (!iterator.end()) {
    ...
    for (int i = 0; i < c.size(); i++) {
      if (c[i] == last_combo[i]) {
        ...
        mst->replace_state(
            i, mtst_->get_node(i)->data()->members()[c[i]]->motif_state);
      }
    }
    ...
    out << vector_to_str(d) << "," << matrix_to_str(r) << "," << mag << ","
        << r_diff << "," << r_best << "," << euler[0] << "," << euler[1] << ","
        << euler[2] << " "
