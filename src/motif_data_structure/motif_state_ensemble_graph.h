...
public:
  MotifStateEnsembleGraphException(String const &message)
...
public:
  MotifStateEnsembleGraph()
      : graph_(data_structure::FixedEdgeDirectedGraph<
               motif::MotifStateEnsemble>()) {}

  MotifStateEnsembleGraph(MotifStateEnsembleGraph const &mseg) {
    graph_ = mseg.graph_;
...
public: // iterator
  typedef typename data_structure::FixedEdgeDirectedGraph<
      motif::MotifStateEnsemble>::const_iterator const_iterator;
  typedef typename data_structure::FixedEdgeDirectedGraph<
      motif::MotifStateEnsemble>::iterator iterator;
...
public:
  size_t size() { return graph_.get_num_nodes(); }

public: // add functions
  int add_ensemble(motif::MotifStateEnsemble const &ensemble) {
  ...
  int add_ensemble(motif::MotifStateEnsemble const &ensemble,
    ...
    auto ni =
        graph_.add_node(ensemble, ensemble.num_end_states(), 0, parent_nie);
    _update_default_transveral();
    return ni;
  }
...
public: // get
  inline motif::MotifStateEnsemble const &get_ensemble(Index ni) {
...
public: // getters for connections
  ...
  get_ensemble_connections(Index ni) const {
    return graph_.get_node_edges(ni);
  }

  inline bool are_ensembles_connected(Index n1, Index n2) {
...
public: // add functions
  int add_ensemble(motif::MotifStateEnsembleOP ensemble) {
    auto ni = graph_.add_node(ensemble, ensemble->num_end_states());
    _update_default_transveral();
    return ni;
  }
...
public: // get functions
  inline motif::MotifStateEnsembleOP get_ensemble(Index ni) {
    return graph_.get_node_data(ni);
  }
...
public:
  ...
  inline std::vector<data_structure::NodeIndexandEdge> get_leafs() {
...
void motif_state_ensemble_graph_from_motif_graph(
    MotifGraph &, resources::Manager &, MotifStateEnsembleGraph & /* return */,
