...
MotifStateGraph::MotifStateGraph()
    : graph_(data_structure::graph::GraphStatic<MSNodeDataOP>()),
      ...
      clash_radius_(2.5), sterics_(1), options_(base::Options()),
      update_align_list_(1), aligned_(std::map<int, int>()) {
  setup_options();
}
...
MotifStateGraph::MotifStateGraph(MotifStateGraph const &msg)
    :
  ...
  for (auto const &n : msg.graph_.nodes()) {
    ...
    graph_.add_data(n_data, -1, -1, -1,
                    n->data()->cur_state->end_states().size(), 1, n->index());
  ...
  for (auto const &c : msg.graph_.connections()) {
    graph_.connect(c->node_1()->index(), c->node_2()->index(), c->end_index_1(),
                   c->end_index_2());
  }
...
void MotifStateGraph::_setup_from_mg(MotifGraphOP const &mg) {
  aligned_ = mg->aligned();
  auto max_index = 0;
  for (auto const &n : *mg) {
    auto n_data = std::make_shared<MSNodeData>(n->data()->get_state());
    graph_.add_data(n_data, -1, -1, -1, n->data()->ends().size(), 1,
                    n->index());
    if (n->index() > max_index) {
      max_index = n->index();
    }
...
MotifStateGraph::_get_parent(String const &m_name, int parent_index) {
...
Indexes MotifStateGraph::_get_available_parent_end_pos(
...
int MotifStateGraph::_get_parent_index_from_name(
    data_structure::graph::GraphNodeOP<MSNodeDataOP> const &parent,
  ...
  if (!avail) {
    throw MotifStateGraphException(
        "cannot add state to graph as the end with name: " + parent_end_name +
        " you are trying to "
        "add it to is already filled or does not exist");
  }
...
int MotifStateGraph::_get_connection_end(
  ...
  if (bp_name != "") {
    ...
    if (!node->available_pos(ei)) {
      throw MotifStateGraphException("cannot add connection with " +
    ...
    if (node_indexes.size() == 0) {
      throw MotifStateGraphException("cannot connect nodes " +
                                     std::to_string(node->index()) +
                                     " there are "
...
int MotifStateGraph::add_state(motif::MotifStateOP const &state,
                               int parent_index, int parent_end_index,
                               int orphan) {

  for (auto const &n : graph_.nodes()) {
    if (n->data()->uuid() == state->uuid()) {
      throw MotifStateGraphException("cannot add state: " + state->name() +
  ...
  if (parent == nullptr || orphan) {
    ...
    auto pos =
        graph_.add_data(n_data, -1, -1, -1, (int)state->end_states().size(), 1);
    if (pos != -1) {
      update_align_list_ = 1;
      aligned_[pos] = 0;
    }
  ...
  for (auto const &p : avail_pos) {
    ...
    auto pos = graph_.add_data(n_data, parent->index(), p, 0,
                               (int)state->end_states().size());
    if (pos != -1) {
      update_align_list_ = 1;
      aligned_[pos] = 1;
    }
...
void MotifStateGraph::add_connection(int i, int j, String const &i_bp_name,
...
void MotifStateGraph::replace_state(int i,
                                    motif::MotifStateOP const &new_state) {

  auto n = graph_.get_node(i);
  /*if(new_state->end_states().size() !=
  n->data()->ref_state->end_states().size()) { throw MotifStateGraphException(
...
void MotifStateGraph::remove_state(int pos) {
  if (pos == -1) {
...
void MotifStateGraph::remove_level(int level) {
  int pos = 0;
...
MotifGraphOP MotifStateGraph::to_motif_graph() {
  ...
  auto seen_connections =
      std::map<data_structure::graph::GraphConnectionOP<MSNodeDataOP>, int>();
  ...
  for (auto const &n : align_list_) {
    auto m = resources::Manager::instance().motif(n->data()->name(), "",
                                                  n->data()->end_name(0));
...
void MotifStateGraph::_update_align_list() {
  ...
  auto used_nodes =
      std::map<data_structure::graph::GraphNodeOP<MSNodeDataOP>, int>();
...
void MotifStateGraph::_align_states(int pos) {
  auto non_aligned_nodes = unaligned_nodes();
  ...
  for (auto const &n : align_list_) {
    ...
    if (element_in_vector(n, non_aligned_nodes)) {
      continue;
    }

    auto parent = n->connections()[0]->partner(n->index());
    auto pei = n->connections()[0]->end_index(parent->index());
    aligner_.get_aligned_motif_state(parent->data()->get_end_state(pei),
                                     n->data()->cur_state,
                                     n->data()->ref_state);
  }
}
...
void MotifStateGraph::setup_options() {
...
void MotifStateGraph::update_var_options() {
