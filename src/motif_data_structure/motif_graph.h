...
enum MotifGraphStringType { OLD, MG, TOP };

class MotifGraphException : public std::runtime_error {
public:
  MotifGraphException(String const &message) : std::runtime_error(message) {}
};
...
public: // construction
  ...
  MotifGraph(String const &, MotifGraphStringType const &);

  MotifGraph(MotifGraph const &);

  ~MotifGraph() {}

public: // setup helpers
  void update_indexes(std::map<int, int> const &);

public:
  typedef typename data_structure::graph::GraphNodeOPs<
      motif::MotifOP>::const_iterator const_iterator;
  typedef typename data_structure::graph::GraphNodeOPs<motif::MotifOP>::iterator
      iterator;
...
private: // add function helpers
  ...
  int _add_motif_to_graph(
      motif::MotifOP &,
      data_structure::graph::GraphNodeOP<motif::MotifOP> const &, int);

  Indexes _get_available_parent_end_pos(
      data_structure::graph::GraphNodeOP<motif::MotifOP> const &, int);

  void _add_motif_tree(MotifTreeOP const &, int, int);
  ...
  void _align_motifs_all_motifs();

public: // add functions
  int add_motif(motif::MotifOP const &m, int parent_index = -1,
                int parent_end_index = -1, int orphan = 0);

  int add_motif(motif::MotifOP const &, int, String const &);

  void add_motif_tree(MotifTreeOP const &mt, int parent_index = -1,
                      int parent_end_index = -1);

  void add_motif_tree(MotifTreeOP const &, int, String const &);

  void add_motif_graph(MotifGraph &mg, int parent_index = -1,
                       int parent_end_index = -1);
...
public: // graph wrappers
  ...
  get_node(util::Uuid const &uuid) const {
    ...
    throw MotifGraphException(
        "cannot get node with uuid no motif has it in this graph");
  }
...
public: // designing functions
  ...
  void replace_ideal_helices(MotifGraph & /* return */,
                             std::map<int, int> & /* return */
  );

  void replace_helical_sequence(secondary_structure::PoseOP const &);

  inline void replace_helical_sequence(String const &seq) {
  ...
  inline String designable_sequence() {
...
public: // getters
  ...
  structure::BasepairOP const &get_available_end(int);

  structure::BasepairOP const &get_available_end(int, String const &);

  structure::BasepairOP get_available_end(String const &, String const &);
...
public: // Motif Merger Wrappers
  ...
  inline void to_pdb(String const fname = "test.pdb", int renumber = -1,
                     int close_chains = 0, int conect_statement = 0) {
  ...
  String pdb_str(int renumber = -1, int close_chains = 0,
                 int conect_statement = 0) {
    auto s = get_structure();
    return s->to_pdb_str(1, 1);
  }

  inline String sequence() { return secondary_structure()->sequence(); }

  inline String dot_bracket() { return secondary_structure()->dot_bracket(); }
...
private:
  void _setup_from_top_str(String const &);
