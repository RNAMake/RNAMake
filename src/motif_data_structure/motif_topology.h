...
public:
  MotifTopologyException(String const &message) : std::runtime_error(message) {}
};
...
private:
  struct _GraphtoTreeNode {
    inline _GraphtoTreeNode(
        ...
        : parent(nparent), parent_end_index(nparent_end_index), node(nnode),
          motif(nmotif) {}
...
private:
  _GraphtoTreeNodeOP
  _get_start_node(MotifGraphOP const &,
                  data_structure::graph::GraphNodeOP<motif::MotifOP> const &,
                  int);

  motif::MotifOP _get_reoriented_motif(motif::MotifOP const &, int);

  _GraphtoTreeNodeOPs _get_new_nodes(_GraphtoTreeNodeOP const &);

  int _get_new_parent_end_index(
      data_structure::graph::GraphNodeOP<motif::MotifOP> const &,
