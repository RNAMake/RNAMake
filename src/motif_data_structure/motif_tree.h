...
enum MotifTreeStringType { MT_STR, TOP_STR };

class MotifTreeException : public std::runtime_error {
public:
  MotifTreeException(String const &message) : std::runtime_error(message) {}
};
...
private:
  ...
  public:
    inline MotifTreePrinter(MotifTree const &mt)
        : branch_length_(25), levels_(std::map<int, int>()),
          node_pos_(std::map<int, int>()),
          nodes_per_level_(std::map<int, int>()), start_pos_(100) {
  ...
  public:
    String print_tree(MotifTree const &mt) {
      auto nodes_per_level =
          std::map<int, data_structure::tree::TreeNodeOPs<motif::MotifOP>>();
      for (auto const &n : mt) {
  ...
  private:
    String _print_level(
        data_structure::tree::TreeNodeOPs<motif::MotifOP> const &nodes) {
      auto s = String("");
      ...
      auto pairs = std::vector<
          std::pair<data_structure::tree::TreeNodeOP<motif::MotifOP>, int>>();
      ...
      std::sort(pairs.begin(), pairs.end(),
                [](std::pair<data_structure::tree::TreeNodeOP<motif::MotifOP>,
                             int> const &p1,
                   std::pair<data_structure::tree::TreeNodeOP<motif::MotifOP>,
                             int> const &p2) { return p1.second < p2.second; });
      ...
      for (auto const &n : sorted_nodes) {
        ...
        if (n->parent() != nullptr) {
          ...
          auto parent_end_name =
              n->parent()->data()->ends()[parent_end_index]->get_name_str();
          strs.push_back("|");
          strs.push_back("E" + std::to_string(parent_end_index) + " - " +
                         parent_end_name);
          strs.push_back("|");
        }

        strs.push_back("N" + std::to_string(n->index()) + " - " +
                       n->data()->name());
    ...
    void _setup_node_positions(MotifTree const &mt) {
      ...
      for (auto const &n : mt) {
        ...
        if (children.size() == 1) {
          ...
          if (extra == 0) {
            ...
            node_pos_[children[0]->index()] =
                parent_pos - branch_length_ / extra;
            node_pos_[children[1]->index()] =
                parent_pos + branch_length_ / extra;
          }
        } else if (children.size() > 2) {
    ...
    void _assign_node_levels(MotifTree const &mt) {
...
public: // constructors
  ...
  MotifTree(String const &);

  MotifTree(String const &, MotifTreeStringType);

  MotifTree(MotifTree const &);
...
public: // iterators
  typedef typename data_structure::tree::TreeStatic<motif::MotifOP>::iterator
      iterator;
  typedef
      typename data_structure::tree::TreeStatic<motif::MotifOP>::const_iterator
          const_iterator;
...
private: // adding functions helpers
  data_structure::tree::TreeNodeOP<motif::MotifOP> _get_parent(int);

  Indexes _get_available_parent_end_pos(
      data_structure::tree::TreeNodeOP<motif::MotifOP> const &, int);

  int _steric_clash(motif::MotifOP const &);

  int _get_connection_end(
      data_structure::tree::TreeNodeOP<motif::MotifOP> const &, String const &);

public: // adding functions
  int add_motif(motif::MotifOP const &m, int parent_index = -1,
                int parent_end_index = -1);

  int add_motif(motif::MotifOP const &m, int parent_index,
...
public: // outputing functions
  inline void to_pdb(String const fname = "test.pdb", int renumber = -1,
                     int close_chains = 0, int conect_statements = 0) {

    try {
      _update_merger();
      return merger_->to_pdb(fname, renumber, close_chains, conect_statements);
    } catch (MotifMergerException) {
      throw MotifTreeException("cannot produce merged structure for a pdb it "
                               "is likely you have created a ring "
                               "with no start, call write_pdbs() to see what "
...
public: // merger wrappers
  ...
  secondary_structure::PoseOP secondary_structure() {
    try {
      ...
      throw MotifTreeException("cannot produce merged secondary structure it "
                               "is likely you have created a ring "
                               "with no start, call write_pdbs() to see what "
                               "the topology would look like");
    }
