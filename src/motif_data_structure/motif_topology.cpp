...
MotifTreeOP GraphtoTree::convert(
  ...
  while (open_nodes.size() > 0) {
    ...
    if (current->parent == nullptr) {
      ...
      // and trying to add somethign on the 0th end of a HELIX this stops it
  ...
  // get all connections that need to added as extra connections to preserve
  // topology
  for (auto const &n : *mg) {
    for (auto const c : n->connections()) {
      ...
      if (partner->data()->mtype() == util::MotifType::HELIX) {
        n2_ei = 1;
      }
  ...
  if (start == nullptr) {
    ...
    if (not_aligned.size() == 0) {
      throw MotifTopologyException(
          "cannot convert graph to tree no starting point");
    }
    auto start = not_aligned[0];
    start_n =
        std::make_shared<_GraphtoTreeNode>(nullptr, 0, start, start->data());
  } else {
    start_n =
        std::make_shared<_GraphtoTreeNode>(nullptr, 0, start, start->data());
...
motif::MotifOP GraphtoTree::_get_reoriented_motif(motif::MotifOP const &m,
                                                  int end_index) {
...
GraphtoTree::_get_new_nodes(GraphtoTree::_GraphtoTreeNodeOP const &current) {
  ...
  for (auto const &c : current->node->connections()) {
    ...
    auto new_n = std::make_shared<_GraphtoTreeNode>(
        current->node, parent_end_index, partner, partner->data());
...
int GraphtoTree::_get_new_parent_end_index(
  ...
  if (parent->data()->mtype() != util::MotifType::HELIX) {
    ...
    throw std::runtime_error(
        "did not find original end something went really wrong");
  }
...
MotifTreeOP
graph_to_tree(MotifGraphOP const &mg,
              data_structure::graph::GraphNodeOP<motif::MotifOP> start,
  ...
  while (open.size() > 0) {
    ...
    if (index == 0) {
      ...
      if (name[2] == '=') {
        ...
        m = resources::Manager::instance().motif(
            name, "", current->data()->ends()[0]->get_name_str());
      }
      mt->add_motif(m);
      ...
      for (auto const &c : current->connections()) {
        ...
        if (seen_nodes.find(p_new) == seen_nodes.end()) {
          continue;
        }
      ...
      if (current->data()->name()[2] == '=' && c_end_index == 1) {
        ...
        m = resources::Manager::instance().motif(current->data()->name(), "",
                                                 c_end_name);
      }

      seen_connections[std::to_string(p_index) + " " + std::to_string(index)] =
          1;
      pos = mt->add_motif(m, p_index, p_end_name);
      mt->get_node(pos)->data()->id(current->data()->id());
      assert(pos != -1 &&
             "did not sucessfully add motif to tree during conversion");
    }
    ...
    for (auto const &c : current->connections()) {
      ...
      if (seen_nodes.find(p) != seen_nodes.end()) {
        continue;
      }
      if (current_nodes.find(p) != current_nodes.end()) {
        continue;
      }
      open.push(p);
      current_nodes[p] = 1;
  ...
  for (auto const &n : *mg) {
    for (auto const &c : n->connections()) {
      ...
      key1 = std::to_string(seen_nodes[c->node_1()]) + " " +
             std::to_string(seen_nodes[c->node_2()]);
      key2 = std::to_string(seen_nodes[c->node_2()]) + " " +
             std::to_string(seen_nodes[c->node_1()]);
