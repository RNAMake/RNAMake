...
MotifStateTree::MotifStateTree()
    : tree_(data_structure::tree::TreeStatic<MSNodeDataOP>()),
      ...
      connections_(MotifConnections()), options_(base::Options()) {
  setup_options();
}

MotifStateTree::MotifStateTree(MotifTreeOP const &mt) : MotifStateTree() {
  ...
  for (auto const &n : *mt) {
    ...
    auto ms = resources::Manager::instance().motif_state(
        n->data()->name(), n->data()->end_ids()[0],
    ...
    if (i == 0) {
      ...
      // std::cout << ms->name() << " " << n->parent_index() << " " <<
      // n->parent_end_index() << std::endl;
      if (j == -1) {
...
MotifStateTree::MotifStateTree(MotifStateTree const &mst)
    :
  ...
  for (auto const &n : mst) {
    tree_.get_node(n->index())->data() =
        std::make_shared<MSNodeData>(*n->data());
  }

  update_var_options();
}

MotifStateTree::MotifStateTree(String const &s) : MotifStateTree() {
  ...
  for (auto const &e : node_spl) {
    ...
    try {
      ms = resources::Manager::instance().motif_state(n_spl[0], n_spl[2],
                                                      n_spl[1]);
    } catch (resources::ResourceManagerException const &e) {
      throw MotifStateTreeException(
          String("could not get state did you forget to add it to the resource "
                 "manager: ") +
          e.what());
    }
    ...
    if (pos == -1) {
      throw MotifStateTreeException("failed to add " + ms->name() + " pos " +
                                    std::to_string(i) +
                                    " in the tree "
...
MotifStateTreeNodeOP MotifStateTree::_get_parent(int parent_index) {
  ...
  try {
    ...
    throw MotifStateTreeException(
        "could not add motif state with parent index: " +
...
Indexes MotifStateTree::_get_available_parent_end_pos(
    MotifStateTreeNodeOP const &parent, int parent_end_index) {
...
int MotifStateTree::_get_parent_index_from_name(
    MotifStateTreeNodeOP const &parent, String const &parent_end_name) {
...
int MotifStateTree::_get_connection_end(MotifStateTreeNodeOP const &node,
  ...
  if (bp_name != "") {
    ...
    if (!node->available_pos(ei)) {
      throw MotifStateTreeException("cannot add connection with " +
    ...
    if (node_indexes.size() == 0) {
      throw MotifStateTreeException("cannot connect nodes " +
                                    std::to_string(node->index()) +
                                    " there are "
...
int MotifStateTree::add_state(motif::MotifStateOP const &state,
                              int parent_index, int parent_end_index) {

  for (auto const &n : tree_) {
    if (n->data()->uuid() == state->uuid()) {
      throw MotifStateTreeException("cannot add state: " + state->name() +
  ...
  for (auto const &p : avail_pos) {
    ...
    if (sterics_ && _steric_clash(n_data)) {
      continue;
    }

    return tree_.add_data(n_data, (int)state->end_states().size(),
                          parent->index(), p);
  }
...
int MotifStateTree::add_state(motif::MotifStateOP const &state,
                              int parent_index, String const &parent_end_name) {
...
int MotifStateTree::add_mst(MotifStateTreeOP const &mst, int parent_index,
  ...
  for (auto const &n : *mst) {
    ...
    if (j == -1) {
      throw MotifStateTreeException(
          "could not add motif state tree to this tree");
    }
  }
...
void MotifStateTree::add_connection(int i, int j, String const &i_bp_name,
...
void MotifStateTree::replace_state(int i,
                                   motif::MotifStateOP const &new_state) {

  auto n = tree_.get_node(i);
  if (new_state->end_states().size() !=
      n->data()->ref_state->end_states().size()) {
    throw MotifStateTreeException(
  ...
  for (int i = 0; i < new_state->end_states().size(); i++) {
    if (connections_.in_connection(n->index(), old_state->end_names()[i])) {
      connections_.update_connection_name(n->index(), old_state->end_names()[i],
  ...
  while (!queue_.empty()) {
    ...
    aligner_.get_aligned_motif_state(
        parent->data()->cur_state->end_states()[pei],
        current->data()->cur_state, current->data()->ref_state);
...
void MotifStateTree::remove_node(int i) {
  ...
  try {
    ...
    throw MotifStateTreeException("cannot remove node with index: " +
                                  std::to_string(i) + " as it does not exist");
  }
}

void MotifStateTree::remove_node_level(int level) {
...
String MotifStateTree::topology_to_str() {
  String s;

  for (auto const &n : tree_) {
    s += n->data()->ref_state->name() + "," +
         n->data()->ref_state->end_names()[0] + ",";
    s += n->data()->ref_state->end_ids()[0] + "," +
         std::to_string(n->parent_index()) + ",";
...
MotifTreeOP MotifStateTree::to_motif_tree() {
  ...
  for (auto const &n : tree_) {
    ...
    /*if(n->data()->ref_state->name() != "") {
        m = resources::Manager::instance().motif(n->data()->ref_state->name(),
    n->data()->ref_state->end_ids()[0]);
    }
    else {
        m = resources::Manager::instance().motif("",
    n->data()->ref_state->end_ids()[0]);
    }*/
    ...
    if (i == 0) {
      align_motif(n->data()->cur_state->end_states()[0], m->ends()[0], m);
      mt->add_motif(m);
      continue;
    }
...
void MotifStateTree::setup_options() {
...
void MotifStateTree::update_var_options() {
