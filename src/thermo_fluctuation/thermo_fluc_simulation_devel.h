...
public:
  ThermoFlucSimulationException(String const &message)
      : std::runtime_error(message) {}
};

class ThermoFlucSimulationLogger {
public:
  ThermoFlucSimulationLogger(String const &fname = "test.out")
      : outputed_header_(false) {
    out_.open(fname);
  }

  ~ThermoFlucSimulationLogger() { out_.close(); }

public:
  virtual void log(motif_data_structure::MotifStateTreeOP const &mst,
                   float score) {

    auto end_state_1 = mst->get_node(ni1_)->data()->get_end_state(ei1_);
    auto end_state_2 = mst->get_node(ni2_)->data()->get_end_state(ei2_);

    out_ << vector_to_str(end_state_1->d()) << ","
         << matrix_to_str(end_state_1->r()) << ",";
    out_ << vector_to_str(end_state_2->d()) << ","
         << matrix_to_str(end_state_2->r()) << "," << score;
    out_ << std::endl;
  }

  virtual void setup(motif_data_structure::MotifStateTreeOP const &mst, int ni1,
...
protected:
  virtual void
  _output_header(motif_data_structure::MotifStateTreeOP const &mst) {
...
typedef std::shared_ptr<ThermoFlucSimulationLogger>
...
private:
  inline int _check_sterics() {
    clash_ = 0;
    for (auto const &i : check_nodes_1_) {
      for (auto const &j : check_nodes_2_) {
        for (auto const &b2 :
             sampler_.mst()->get_node(i)->data()->cur_state->beads()) {
          for (auto const &b1 :
               sampler_.mst()->get_node(j)->data()->cur_state->beads()) {
...
public:
  void setup(motif_data_structure::MotifStateEnsembleTreeOP const &, int, int,
             int, int);
...
public:
  inline void set_logger(ThermoFlucSimulationLoggerOP logger) {
    logger_ = logger;
  }

  inline void set_scorer(ThermoFlucScorerOP scorer) { scorer_ = scorer; }
