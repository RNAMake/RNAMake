...
void ThermoFlucSampler::setup(
  ...
  if (randomized_start_) {
    for (int i = 0; i < mset_->size(); i++) {
      ...
      if (mem_pos_ == mset_node_->data()->size()) {
        mem_pos_--;
      }
...
void ThermoFlucSampler::sample(int steps) {
  for (int i = 0; i < steps; i++) {
    next();
  }
}

int ThermoFlucSampler::next() {

  node_num_ = rng_.randrange((int)mst_->size());
  if (node_num_ == mst_->size()) {
    node_num_--;
  }
...
void ThermoFlucSampler::update(
    int node_num, motif::MotifStateEnsembleMemberOP const &new_mem) {
...
void ThermoFlucSampler::to_pdb(String fname, int renumber) {
