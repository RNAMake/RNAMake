...
public:
  ThermoFlucSamplerException(String const &message)
...
public:
  void sample(int steps = 1000);

  void setup(motif_data_structure::MotifStateEnsembleTreeOP const &);

  int next();

  void undo();

  void to_pdb(String fname = "test.pdb", int renumber = -1);

private:
  void update(int, motif::MotifStateEnsembleMemberOP const &);

public: // getters
  inline float temperature() { return temperature_; }

  inline motif_data_structure::MotifStateTreeOP mst() { return mst_; }

public: // setters
  inline void temperature(float const &temp) {
    if (temp < 0) {
      throw ThermoFlucSamplerException("cannot set temperature lower then 0");
    }
  ...
  inline void randomized_start(bool random_start) {
