...
class Simulation {
public:
  Simulation(ScorerOP scorer, sterics::StericsOP sterics_)
      : scorer_(scorer->clone()), sterics_(sterics_->clone()) {
...
public:
  void setup(motif_data_structure::MotifStateEnsembleGraph const &,
...
public: // setters
public: // outputs
  void write_pdbs(String const &name = "nodes") {
    msg_->to_motif_graph()->write_pdbs(name);
  }

  String get_pdb_str() { return msg_->to_motif_graph()->pdb_str(); }

  motif_data_structure::MotifGraphOP get_motif_graph() {
