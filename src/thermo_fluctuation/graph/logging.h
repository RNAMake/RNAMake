...
public:
  virtual void setup(motif_data_structure::MotifStateGraphOP,
                     data_structure::NodeIndexandEdge const &,
                     data_structure::NodeIndexandEdge const &) = 0;

  virtual void log(motif_data_structure::MotifStateGraphOP, float) = 0;
...
/**
 * Logs the rotation and translation of the two target basepairs to will overlap
 * to generate a hit Log order: target bp_1 origin, target bp_1 rotation, target
 * bp_2 origin, target bp_2 rotation, score
 *
 */
class TargetBPInfoLogger : public Logger {
public:
  TargetBPInfoLogger(String const &log_file) : Logger(log_file) {}

  ~TargetBPInfoLogger() {}

  Logger *clone() const { return new TargetBPInfoLogger(*this); };

public:
  void setup(motif_data_structure::MotifStateGraphOP msg,
  ...
  void log(motif_data_structure::MotifStateGraphOP msg, float score) {}
