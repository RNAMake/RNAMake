...
void ThermoFlucSimulationDevel::setup(
    motif_data_structure::MotifStateEnsembleTreeOP const &mset, int ni1,
    int ni2, int ei1, int ei2) {
  ...
  if (sampler_.mst()->get_node(ni1_)->data()->cur_state->end_states().size() <=
      ei1_) {
    throw ThermoFlucSimulationException(
  ...
  if (sampler_.mst()->get_node(ni2_)->data()->cur_state->end_states().size() <=
      ei2_) {
    throw ThermoFlucSimulationException(
  ...
  if (get_string_option("steric_nodes") == "") {
    return;
  }
  auto steric_node_str = get_string_option("steric_nodes");
  auto spl = base::string::split(steric_node_str, ":");
  if (spl.size() != 2) {
    throw ThermoFlucSimulationException(
        "incorrect format for steric_nodes option, must be in the form "
        "NodeSet1:NodeSet2,"
        " example: '22,21:1' which check sterics of nodes 22 and 21 against "
        "node 1");
  }

  auto node_set_1 = base::string::split(spl[0], ",");
  for (auto const &n_num : node_set_1) {
    ...
    try {
      ...
      throw ThermoFlucSimulationException(
          "cannot setup sterics for simulation, specified node: " +
          std::to_string(n_index) + " does not exist");
    }

    check_nodes_1_.push_back(n_index);
  }

  auto node_set_2 = base::string::split(spl[1], ",");
...
String ThermoFlucSimulationDevel::static_run() {
  ...
  String results;
  end_state_1_ =
  sampler_.mst()->get_node(ni1_)->data()->cur_state->end_states()[ei1_];
  end_state_2_ =
  sampler_.mst()->get_node(ni2_)->data()->cur_state->end_states()[ei2_];
  ...
  results += std::to_string(frame_score_) + " " + std::to_string(r_diff_) + " "
  + std::to_string(frame_score_ + r_diff_);*/

  // return results;
  return String("");
}

int ThermoFlucSimulationDevel::run() {
  ...
  auto r1_trans = math::Matrix3x3();
  auto r2_trans = math::Matrix3x3();
  auto r_result = math::Matrix3x3();
  ...
  if (record_) {
    // default setup
    if (logger_ == nullptr) {
      logger_ = std::make_shared<ThermoFlucSimulationLogger>();
    }
  ...
  while (steps < steps_) {
    ...
    if (record_) {
      if (record_only_bound_ && score_ <= cutoff_) {
        logger_->log(sampler_.mst(), score_);
    ...
    if (dump_state_) {
      if (record_only_bound_ && score_ <= cutoff_) {
        out_dump_state << sampler_.mst()->to_motif_tree()->to_str()
                       << std::endl;
      } else if (record_only_unbound_ && score_ > cutoff_) {
        out_dump_state << sampler_.mst()->to_motif_tree()->to_str()
                       << std::endl;
    ...
    if (dump_pdbs_) {
      if (record_only_bound_ && score_ <= cutoff_) {
        sampler_.mst()->to_motif_tree()->to_pdb(
            "test." + std::to_string(pdb_count) + ".pdb", 1, 1);
        pdb_count += 1;
      } else if (record_only_unbound_ && score_ > cutoff_) {
        sampler_.mst()->to_motif_tree()->to_pdb(
            "test." + std::to_string(pdb_count) + ".pdb", 1, 1);
...
void ThermoFlucSimulationDevel::setup_options() {
  ...
  options_.add_option("record_all_file", "test_all.out",
...
void ThermoFlucSimulationDevel::update_var_options() {
