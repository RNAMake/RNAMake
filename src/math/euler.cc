...
void axis_angle_from_matrix(Matrix& m, AxisAngle& aa) {
  auto epsilon = 0.01;  // margin to allow for rounding errors
  auto epsilon2 = 0.1;  // margin to distinguish between 0 and 180 degrees
  if (std::abs(m.xy() - m.yx()) < epsilon &&
      std::abs(m.xz() - m.zx()) < epsilon &&
      std::abs(m.yz() - m.zy()) < epsilon) {
    // singularity found
    // first check for identity matrix which must have +1 for all terms
    //  in leading diagonaland zero in other terms
    if (std::abs(m.xy() + m.yx()) < epsilon2 &&
        std::abs(m.xz() + m.zx()) < epsilon2 &&
        std::abs(m.yz() + m.zy()) < epsilon2 &&
        std::abs(m.xx() + m.yy() + m.zz() - 3) < epsilon2) {
    ...
    if (xx > yy && xx > zz) {
      if (xx < epsilon) {
        ...
        aa.axis.x(sqrt(xx));
        aa.axis.y(xy / aa.axis.x());
        aa.axis.z(yz / aa.axis.x());
      }
    } else if (yy > zz) {
      if (yy < epsilon) {
        aa.axis.x(0.7071);
        ...
        aa.axis.y(sqrt(yy));
        aa.axis.x(xy / aa.axis.y());
        aa.axis.z(yz / aa.axis.y());
      }
    } else {
      if (zz < epsilon) {
        aa.axis.x(0.7071);
        ...
        aa.axis.z(sqrt(zz));
        aa.axis.x(xz / aa.axis.z());
        aa.axis.y(yz / aa.axis.z());
      }
    }
    return;
  }

  auto s = sqrt((m.zy() - m.yz()) * (m.zy() - m.yz()) +
                (m.xz() - m.zx()) * (m.xz() - m.zx()) +
                (m.yx() - m.xy()) * (m.yx() - m.xy()));
  if (std::abs(s) < 0.001) {
    s = 1;
