...
class xyzMatrix {
 ...
 public:
  ...
  friend inline void dot(xyzMatrix<T> const& a, xyzMatrix<T> const& b,
                         xyzMatrix<T>& c) {
 ...
 public:
  inline void row(const int i, Vector const& v) {
    if (i == 0) {
      xx_ = v.x();
      xy_ = v.y();
      xz_ = v.z();
  ...
  inline void row(const int i, std::vector<T> const& v) {
    if (i == 0) {
      xx_ = v[0];
      xy_ = v[1];
      xz_ = v[2];
 ...
 public:
  ...
  inline float const difference(xyzMatrix<T> const& b) const {
    float dist = 0.0f;
    dist += fabs(xx_ - b.xx_);
    dist += fabs(xy_ - b.xy_);
    dist += fabs(xz_ - b.xz_);
  ...
  inline const xyzMatrix get_flip_orientation() const {
    return xyzMatrix(xx_, xy_, xz_, -yx_, -yy_, -yz_, -zx_, -zy_, -zz_);
  }

  inline const xyzMatrix get_unitarize() const {
 ...
 public:  // Properties: scalars
  ...
  inline Value const& xx() const { return xx_; }

  /// @brief Value xy const
  inline Value const& xy() const { return xy_; }

  /// @brief Value xz const
  inline Value const& xz() const { return xz_; }

  /// @brief Value yx const
  inline Value const& yx() const { return yx_; }

  /// @brief Value yy const
  inline Value const& yy() const { return yy_; }

  /// @brief Value yz const
  inline Value const& yz() const { return yz_; }

  /// @brief Value zx const
  inline Value const& zx() const { return zx_; }

  /// @brief Value zy const
  inline Value const& zy() const { return zy_; }

  /// @brief Value zz const
  inline Value const& zz() const { return zz_; }

 public:  // Properties: value assignment
  /// @brief xx assignment
  inline void xx(Value const& xx_a) { xx_ = xx_a; }

  /// @brief xy assignment
  inline void xy(Value const& xy_a) { xy_ = xy_a; }

  /// @brief xz assignment
  inline void xz(Value const& xz_a) { xz_ = xz_a; }

  /// @brief yx assignment
  inline void yx(Value const& yx_a) { yx_ = yx_a; }

  /// @brief yy assignment
  inline void yy(Value const& yy_a) { yy_ = yy_a; }

  /// @brief yz assignment
  inline void yz(Value const& yz_a) { yz_ = yz_a; }

  /// @brief zx assignment
  inline void zx(Value const& zx_a) { zx_ = zx_a; }

  /// @brief zy assignment
  inline void zy(Value const& zy_a) { zy_ = zy_a; }

  /// @brief zz assignment
  inline void zz(Value const& zz_a) { zz_ = zz_a; }

  inline xyzMatrix<T> transposed() const {
...
inline const Matrix matrix_from_str(std::string const& s) {
...
template <typename T>
inline xyzVector<T> operator*(xyzMatrix<T> const& m, xyzVector<T> const& v) {
  return xyzVector<T>(m.xx() * v.x() + m.xy() * v.y() + m.xz() * v.z(),
...
inline xyzMatrix<T> transform_1(xyzMatrix<T> const& m) {
  return xyzMatrix<T>(m.xx(), m.xy(), m.xz(), -m.yx(), -m.yy(), -m.yz(),
                      -m.zx(), -m.zy(), -m.zz());
}

template <typename T>
std::ostream& operator<<(std::ostream& stream, xyzMatrix<T> const& v) {
  stream << "(" << v.xx() << ", " << v.xy() << ", " << v.xz() << ")"
         << std::endl;
  stream << "(" << v.yx() << ", " << v.yy() << ", " << v.yz() << ")"
         << std::endl;
  stream << "(" << v.zx() << ", " << v.zy() << ", " << v.zz() << ")"
         << std::endl;
  return stream;
}
