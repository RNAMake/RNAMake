...
std::ostream& operator<<(std::ostream& stream, Quaternion const& q) {
  stream << q.get_a() << " " << q.get_b() << " " << q.get_c() << " "
         << q.get_d();
  return stream;
}
...
Matrix Quaternion::get_rotation_matrix() {
  ...
  return Matrix(1.0 - q[2][2] - q[3][3], q[1][2] - q[3][0], q[1][3] + q[2][0],
                q[1][2] + q[3][0], 1.0 - q[1][1] - q[3][3], q[2][3] - q[1][0],
                q[1][3] - q[2][0], q[2][3] + q[1][0], 1.0 - q[1][1] - q[2][2]);
}

Quaternion get_quaternion_from_matrix(Matrix const& m) {
  double qx, qy, qz, qw;
...
void power_iteration(std::vector<std::vector<double>> const& A,
                     std::vector<double>& eigen_values, int num_simulations) {
  std::random_device rd;
  std::mt19937 gen(rd());  // Standard mersenne_twister_engine seeded with rd()
  std::uniform_real_distribution<> dis(0.0, 1.0);
