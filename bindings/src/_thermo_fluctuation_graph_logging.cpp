...
using namespace thermo_fluctuation::graph::logging;

// Bindings start

namespace py = pybind11;

PYBIND11_MODULE(_thermo_fluctuation_graph_logging,m) {
...
/*
		...
		// methods
		.def("clone",[] (Logger const & ptr) -> Logger * {
		 return ptr.clone(); } )
		.def("setup",[] (Logger  & ptr, motif_data_structure:: MotifStateGraphOP, data_structure::NodeIndexandEdge const &, data_structure::NodeIndexandEdge const &) {
		ptr.setup(MotifStateGraphOP, &, &); } )
		.def("log",[] (Logger  & ptr, motif_data_structure:: MotifStateGraphOP, float ) {
		ptr.log(MotifStateGraphOP, ); } )
		;
*/
        py::class_<TargetBPInfoLogger, std::shared_ptr<TargetBPInfoLogger>>(m, "TargetBPInfoLogger")
		// ctors
		.def(py::init<String const &>())
		// methods
		.def("clone",[] (TargetBPInfoLogger const & ptr) -> Logger * {
		 return ptr.clone(); } )
		.def("setup",[] (TargetBPInfoLogger  & ptr, motif_data_structure::MotifStateGraphOP msg, data_structure::NodeIndexandEdge const & start, data_structure::NodeIndexandEdge const & end) {
		ptr.setup(msg, start, end); } )
		.def("log",[] (TargetBPInfoLogger  & ptr, motif_data_structure::MotifStateGraphOP msg, float score) {
		ptr.log(msg, score); } )
		// inherited methods
		.def("clone",[] (Logger const & ptr) -> Logger * {
		 return ptr.clone(); } )
		.def("setup",[] (Logger  & ptr, motif_data_structure:: MotifStateGraphOP graph, data_structure::NodeIndexandEdge const & nandi1, data_structure::NodeIndexandEdge const &nandi2) {
		ptr.setup(graph, nandi1, nandi2); } )
		.def("log",[] (Logger  & ptr, motif_data_structure:: MotifStateGraphOP graph, float  value) {
		ptr.log(graph, value); } )
		;
