...
using namespace thermo_fluctuation::graph::sterics;

// Bindings start

namespace py = pybind11;

PYBIND11_MODULE(_thermo_fluctuation_graph_sterics,m) {

	// classes

        py::class_<NoSterics, std::shared_ptr<NoSterics>>(m, "NoSterics")
		// ctors
		.def(py::init<>())
		// methods
		.def("clone",[] (NoSterics const & ptr) -> Sterics * {
		 return ptr.clone(); } )
		.def("clash",[] (NoSterics  & ptr, motif_data_structure::MotifStateGraphOP msg) -> bool {
        ...
        py::class_<SelectiveSterics, std::shared_ptr<SelectiveSterics>>(m, "SelectiveSterics")
		// ctors
		.def(py::init<Ints const,Ints const,float>())
		// methods
		.def("clone",[] (SelectiveSterics const & ptr) -> Sterics * {
		 return ptr.clone(); } )
		.def("clash",[] (SelectiveSterics  & ptr, motif_data_structure::MotifStateGraphOP msg) -> bool {
		 return ptr.clash(msg); } )
		// inherited methods
		.def("clone",[] (Sterics const & ptr) -> Sterics * {
		 return ptr.clone(); } )
