...
PYBIND11_MODULE(_motif_search_exhaustive,m) {
	...
	// classes
        ...
        py::class_<MotifStateEnumerator, std::shared_ptr<MotifStateEnumerator>>(m, "MotifStateEnumerator")
		// ctors
		.def(py::init<motif_search::SolutionToplogy>())
		// methods
		.def("start",[] (MotifStateEnumerator  & ptr, structure:: BasepairStateOP bp) {
		ptr.start(bp); } )
		.def("finished",[] (MotifStateEnumerator  & ptr) -> bool {
		 return ptr.finished(); } )
		.def("next",[] (MotifStateEnumerator  & ptr) {
		ptr.next(); } )
		.def("top_state",[] (MotifStateEnumerator  & ptr) -> motif::MotifStateOP {
		 return ptr.top_state(); } )
		.def("all_states",[] (MotifStateEnumerator  & ptr) -> motif::MotifStateOPs const & {
		 return ptr.all_states(); } )
		.def("set_size_limit",[] (MotifStateEnumerator  & ptr, int size_limit) {
