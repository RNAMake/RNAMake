...
def build_libraries(lib_list, dependencies, base_dir):
    ...
    for library in lib_list:
        ...
        library_declarations += "add_library({LIB}_lib  ${{{LIB}_files}})\n".format(LIB=library)
        ...
        library_declarations += "target_link_libraries({LIB}_lib {DEPENDS} )\n".format(
            LIB=library, DEPENDS=' '.join([lib + "_lib" for lib in dependencies[library]] + ["${SQLITE3_LIBRARY} \"-ldl\"" if library == "util" else ""])
        )
    #library_declarations += "add_library(all_lib STATIC {DIR}/main.cpp)\ntarget_link_libraries(all_lib {LIBS} -static)\n".format(
    library_declarations += "add_library(all_lib {DIR}/main.cpp)\ntarget_link_libraries(all_lib {LIBS} )\n".format(
        LIBS=' '.join([lib+"_lib" for lib in dependencies["all"]]),DIR=base_dir
    )
    library_declarations += "#"*100 + '\n'
    return library_declarations

def build_unittests(lib_list, base_dir):
    ...
    for library in lib_list:
        ...
        for source_file in Utils.make_file_list(glob.glob(base_dir + "/" + library + "/*")):
            ...
            unittest_declarations += "\ttarget_link_libraries({TEST} {LIB}_lib )\n".format(
                TEST=unittest_name,LIB=library
            )
    unittest_declarations += "#"*100 + '\n'
...
def build_header(base_dir):
    ...
    #header_contents+= "set(CMAKE_EXE_LINKER_FLAGS \" -lstdc++ -Wl,--no-as-needed -ldl -lgsl\")\n"
    ...
    header_contents+= "find_library(SQLITE3_LIBRARY sqlite3 VARIANT )\n"
...
#    }
#
#    libs = "base math data_structure util vienna secondary_structure eternabot structure motif motif_tools resources motif_data_structures thermo_fluctuation motif_state_search sequence_optimizer".split()
#
    write_CML_file(
        [
        build_header(os.getcwd().replace("/cmake/build","")),
        build_libraries(libs, depends,os.getcwd().replace("/cmake/build","/src") ),
        build_unittests(libs,os.getcwd().replace("/cmake/build","/unittests")),
        build_apps(os.getcwd().split("/cmake/build")[0])
        ]
        )
