...
class MotifTree(object):
    ...
    def _get_parent_available_ends(self, parent, parent_end_index,
        ...
        if parent_end_index != -1 and parent_end_name is not None:
            ...
            final_avail_pos = []
            for p in avail_pos:
                parent_end_name = parent.data.get_end(p).name
                if self._connections.in_connection(parent.index, parent_end_name):
                    continue
    ...
    def add_motif(self, m=None, parent_index=-1, parent_end_index=-1,
        ...
        if parent is None:
            m_copy = motif.Motif.copy(m, new_uuid=1)
            self._update_merger = 1
            return self._tree.add_data(m_copy, m_copy.num_ends(), -1, -1)
        ...
        for p in avail_pos:
            ...
            pos =  self._tree.add_data(m_added, m_added.num_ends(), parent.index, p)
            if pos != -1:
                 self._update_merger = 1
            return pos
    ...
    def replace_motif(self, pos, new_motif):
        ...
        if new_motif.num_ends() != node.data.num_ends():
            raise exceptions.MotifTreeException(
                "attmpted to replace a motif with a different number of ends")

        node.data = motif.Motif.copy(new_motif, new_uuid=1)

        for n in tree.transverse_tree(self._tree, pos):
            parent = n.parent
            if parent is None:
                continue
    ...
    def remove_node_level(self, level=None):
        ...
        while 1:
            found = 0
            r = range(0, len(self._tree.nodes))
            for i in r[::-1]:
                n = self._tree.nodes[i]
                if n.level >= level:
    ...
    def to_pdb_str(self, renumber=-1, close_chain=0):
        self.__update_merger()
        return self._merger.to_pdb_str(renumber=renumber, close_chain=close_chain)

    def to_pretty_str(self):
        printer = MotifTreePrinter(self)
    ...
    def __update_merger(self):
        ...
        for n in self._tree:
            ...
            self._merger.add_motif(n.data, n.data.get_end(0),
                                   n.parent.data, n.parent.data.get_end(pei))
        ...
        for c in self._connections:
            ...
            self._merger.connect_motifs(node_i.data, node_j.data,
                                        node_i.data.get_end(node_i_ei),
                                        node_j.data.get_end(node_j_ei))

        self._update_merger = 0

#TODO FIX!!!
class MotifTreePrinter(object):
        """
        ...
        def __init__(self, mt):
            ...
            raise ValueError("not supported atm")
