...
struct model_detailsT {
	int dangles;	/**<  \brief  Specifies the dangle model used in any energy
					   evaluation (0,1,2 or 3)
					   \note   Some function do not implement all dangle model but
					   only
					   a subset of
					   (0,1,2,3). Read the documentaion of the particular
					   recurrences
					   or
					   energy evaluation function for information about the
					   provided
					   dangle
					   model.
					   */
	int special_hp; /**<  \brief  Include special hairpin contributions for tri,
					   tetra and hexaloops */
	int noLP; /**<  \brief  Only consider canonical structures, i.e. no 'lonely'
				 base pairs */
	...
	int canonicalBPonly; /**<  \brief  remove non-canonical bp's from constraint
							structures  */
};
...
struct paramT {
	...
	int MultipleCA;
	int MultipleCB;
	double temperature; /**<  \brief  Temperature used for loop contribution
						   scaling */

	model_detailsT model_details; /**<  \brief  Model details to be used in the
									 recursions */
};
...
struct pf_paramT {
	...
	double temperature; /**<  \brief    Temperature used for loop contribution
						   scaling */
	double
		alpha; /**<  \brief    Scaling factor for the thermodynamic temperature
...
class Vienna {
  public:
	...
	float fold( String const & );

	plists const &bp_probabilities( String const & );

  public:
	inline String get_structure() {
		return structure.substr( 0, actual_size_ );
	}
  ...
  private:
	...
	inline void get_arrays( int size ) {
		if ( size > size_ ) {
	...
	inline void get_arrays_part_func( int size ) {
	...
	inline void get_iindx( Ints &c_idx, int length ) {
		...
		for ( i = 1; i <= length; i++ ) {
			c_idx[ i ] =
				( ( ( length + 1 - i ) * ( length - i ) ) >> 1 ) + length + 1;
		}
	}
	...
	void update_fold_params_par() {
		...
		for ( i = 0; i < 31; i++ ) {
			params.hairpin[ i ] =
				hairpindH[ i ] - ( hairpindH[ i ] - hairpin37[ i ] ) * tempf;
		}

		for ( i = 0; i <= MIN2( 30, MAXLOOP ); i++ ) {
			params.bulge[ i ] =
				bulgedH[ i ] - ( bulgedH[ i ] - bulge37[ i ] ) * tempf;
			params.internal_loop[ i ] =
				internal_loopdH[ i ] -
				( internal_loopdH[ i ] - internal_loop37[ i ] ) * tempf;
		}
		...
		params.MultipleCA =
			MultipleCAdH - ( MultipleCAdH - MultipleCA37 ) * tempf;
		params.MultipleCB =
			MultipleCBdH - ( MultipleCBdH - MultipleCB37 ) * tempf;

		for ( i = 0; ( i * 7 ) < strlen( Tetraloops ); i++ ) {
			params.Tetraloop_E[ i ] =
				TetraloopdH[ i ] -
				( TetraloopdH[ i ] - Tetraloop37[ i ] ) * tempf;
		}
		for ( i = 0; ( i * 5 ) < strlen( Triloops ); i++ ) {
			params.Triloop_E[ i ] =
				TriloopdH[ i ] - ( TriloopdH[ i ] - Triloop37[ i ] ) * tempf;
		}
		...
		params.TerminalAU =
			TerminalAUdH - ( TerminalAUdH - TerminalAU37 ) * tempf;
		params.DuplexInit =
			DuplexInitdH - ( DuplexInitdH - DuplexInit37 ) * tempf;
		params.MLbase = ML_BASEdH - ( ML_BASEdH - ML_BASE37 ) * tempf;

		for ( i = 0; i <= NBPAIRS; i++ )
			params.MLintern[ i ] =
				ML_interndH - ( ML_interndH - ML_intern37 ) * tempf;

		params.MLclosing =
			ML_closingdH - ( ML_closingdH - ML_closing37 ) * tempf;
		...
		for ( i = 0; i <= NBPAIRS; i++ ) {
			for ( j = 0; j < 5; j++ ) {
				int dd;
				dd = dangle5_dH[ i ][ j ] -
					 ( dangle5_dH[ i ][ j ] - dangle5_37[ i ][ j ] ) * tempf;
				params.dangle5[ i ][ j ] =
					( dd > 0 ) ? 0 : dd; /* must be <= 0 */
				dd = dangle3_dH[ i ][ j ] -
					 ( dangle3_dH[ i ][ j ] - dangle3_37[ i ][ j ] ) * tempf;
				params.dangle3[ i ][ j ] =
					( dd > 0 ) ? 0 : dd; /* must be <= 0 */
			}
		}
	...
	void get_boltzmann_factors( float temperature, float betaScale,
								model_detailsT const &md, float pf_scale );

	int fill_arrays( String const & );

	void backtrack( String const &, int );

	void parenthesis_structure( int );

	void make_ptypes( Shorts const &, String const & );

	void make_ptypes_2( Shorts const &, String const & );

	void pf_linear( String const & );
  ...
  private: // Energy calculations
		...
		if ( params.model_details.special_hp ) {
			if ( size == 4 ) { /* check for tetraloop bonus */
				...
				if ( ( ts = strstr( params.Tetraloops, tl ) ) )
					return (
						params.Tetraloop_E[ ( ts - params.Tetraloops ) / 7 ] );
			} else if ( size == 6 ) {
				char tl[ 9 ] = {0}, *ts;
				strncpy( tl, string, 8 );
				if ( ( ts = strstr( params.Hexaloops, tl ) ) )
					return ( energy =
								 params.Hexaloop_E[ ( ts - params.Hexaloops ) /
													9 ] );
			} else if ( size == 3 ) {
	...
	inline int E_IntLoop( int n1, int n2, int type, int type_2, int si1,
		...
		if ( ns == 0 ) { /* bulge */
			...
			if ( ns == 1 ) {
				...
				if ( nl == 2 ) { /* 2x1 loop */
					if ( n1 == 1 )
						energy =
							params.int21[ type ][ type_2 ][ si1 ][ sq1 ][ sj1 ];
					else
						energy =
							params.int21[ type_2 ][ type ][ sq1 ][ si1 ][ sp1 ];
					return energy;
				} else { /* 1xn loop */
					energy =
						( nl + 1 <= MAXLOOP )
							? ( params.internal_loop[ nl + 1 ] )
							: ( params.internal_loop[ 30 ] +
								(int)( params.lxc * log( ( nl + 1 ) / 30. ) ) );
					energy +=
						MIN2( MAX_NINIO, ( nl - ns ) * params.ninio[ 2 ] );
					energy += params.mismatch1nI[ type ][ si1 ][ sj1 ] +
							  params.mismatch1nI[ type_2 ][ sq1 ][ sp1 ];
					return energy;
				}
			} else if ( ns == 2 ) {
				if ( nl == 2 ) { /* 2x2 loop */
					return params
						.int22[ type ][ type_2 ][ si1 ][ sp1 ][ sq1 ][ sj1 ];
				} else if ( nl == 3 ) { /* 2x3 loop */
					energy = params.internal_loop[ 5 ] + params.ninio[ 2 ];
					energy += params.mismatch23I[ type ][ si1 ][ sj1 ] +
							  params.mismatch23I[ type_2 ][ sq1 ][ sp1 ];
					return energy;
				}
			}
			{ /* generic interior loop (no else here!)*/
				energy =
					( n1 + n2 <= MAXLOOP )
						? ( params.internal_loop[ n1 + n2 ] )
						: ( params.internal_loop[ 30 ] +
							(int)( params.lxc * log( ( n1 + n2 ) / 30. ) ) );

				energy += MIN2( MAX_NINIO, ( nl - ns ) * params.ninio[ 2 ] );

				energy += params.mismatchI[ type ][ si1 ][ sj1 ] +
						  params.mismatchI[ type_2 ][ sq1 ][ sp1 ];
			}
		}
		return energy;
	}
	...
	inline double exp_E_IntLoop( int u1, int u2, int type, int type2, short si1,
		...
		if ( ( no_closingGU ) && ( ( type2 == 3 ) || ( type2 == 4 ) ||
								   ( type == 3 ) || ( type == 4 ) ) )
			no_close = 1;
		...
		if ( ul == 0 ) { /* stack */
			...
			if ( us == 0 ) { /* bulge */
				...
				if ( ul == 2 ) { /* 2x1 loop */
					if ( u1 == 1 )
						return pf
							.expint21[ type ][ type2 ][ si1 ][ sq1 ][ sj1 ];
					else
						return pf
							.expint21[ type2 ][ type ][ sq1 ][ si1 ][ sp1 ];
				} else { /* 1xn loop */
					z = pf.expinternal[ ul + us ] *
						pf.expmismatch1nI[ type ][ si1 ][ sj1 ] *
						pf.expmismatch1nI[ type2 ][ sq1 ][ sp1 ];
					return z * pf.expninio[ 2 ][ ul - us ];
				}
			} else if ( us == 2 ) {
				if ( ul == 2 ) /* 2x2 loop */
					return pf
						.expint22[ type ][ type2 ][ si1 ][ sp1 ][ sq1 ][ sj1 ];
				else if ( ul == 3 ) { /* 2x3 loop */
					z = pf.expinternal[ 5 ] *
						pf.expmismatch23I[ type ][ si1 ][ sj1 ] *
						pf.expmismatch23I[ type2 ][ sq1 ][ sp1 ];
	...
	inline double exp_E_MLstem( int type, int si1, int sj1 ) {
		double energy = 1.0;
		if ( si1 >= 0 && sj1 >= 0 ) {
	...
	inline double exp_E_ExtLoop( int type, int si1, int sj1 ) {
		double energy = 1.0;
		if ( si1 >= 0 && sj1 >= 0 ) {
