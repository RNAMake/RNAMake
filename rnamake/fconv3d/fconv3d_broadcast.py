...
def car2sph(phi):
    ...
    sph_grid = np.mgrid[0:rang_r, 0:rang_phi, 0:np.pi:rang_theta * 1j].astype('float')
    ...
    res = np.zeros([rang_r, rang_phi, rang_theta, rang_al, rang_be, rang_ga]).astype('complex128')
...
def so3int(phi):
    ...
    p = np.mgrid[0:rang_lp, 0:rang_al, 0:rang_ga, 0:rang_be].astype('float')
    # p1(lp,n,mp,beta)
    p[3, :] *= np.pi / rang_be
    p[3, :] = np.cos(p[3, :])
    ...
    sinbeta = np.sin((np.mgrid[0:rang_be].astype('float') + 0.5) * np.pi / rang_be)[np.newaxis, np.newaxis, np.newaxis,
    ...
    res = np.zeros([rang_ap, rang_phi, rang_theta, rang_lp, rang_n, rang_mp]).astype('complex128')
...
def usphint(phi):
    ...
    f3 = ft.ifft(f, axis=1)  # f3(a_p,m-n,theta,lp,mp,n)
    rang_mn = rang_phi - 1
    ...
    if a.y_axis:
        q1 = np.mgrid[0:rang_lp, 0:rang_s, 0:rang_n, 0:np.pi:rang_theta * 1j].astype('float')
    else:
        q1 = np.mgrid[0:rang_lp, 0:rang_s, 0:rang_n, 0:rang_theta].astype('float')
    ...
    if a.y_axis:
        q2 = np.mgrid[0:rang_l, 0:rang_s, 0:rang_m, 0:np.pi:rang_theta * 1j].astype('float')
    else:
        q2 = np.mgrid[0:rang_l, 0:rang_s, 0:rang_m, 0:rang_theta].astype('float')
    ...
    '''sin_thetac = np.broadcast_to(sin_theta, (
        f3m.shape[0], f3m.shape[1], f3m.shape[3], f3m.shape[4], f3m.shape[5], rang_s, rang_l) + sin_theta.shape)
    # sin_thetac(a_p, m, lp, mp, n, s, l, theta)
    sin_thetab = sin_thetac.transpose([0, 1, 7, 2, 3, 4, 5, 6])
    f4 = np.sum(f3b * q1b * q2b * sin_thetab, axis=2) * np.pi / f3m.shape[2]  # f4(a_p,m,lp,mp,n,s,l)
    '''
    # stage1 = f3b * q1b*q2b[:, :, :, :, :, 0, s, l]
    f_hat = np.zeros([rang_ap, rang_m, rang_lp, rang_mp, rang_s, rang_l]).astype('complex128')
    ...
    # stage2 = stage1*q2b
    # f4 = np.sum(stage2, axis=2) * np.pi / rang_theta
...
def usphinti(phi):
    ...
    if a.y_axis:
        q1 = np.mgrid[0:rang_l, 0:rang_s, 0:rang_m, 0:np.pi:rang_theta * 1j].astype('float')
    else:
        q1 = np.mgrid[0:rang_l, 0:rang_s, 0:rang_m, 0:rang_theta].astype('float')
    ...
    if a.y_axis:
        q2 = np.mgrid[0:rang_lp, 0:rang_s, 0:rang_n, 0:np.pi:rang_theta * 1j].astype('float')
    else:
        q2 = np.mgrid[0:rang_lp, 0:rang_s, 0:rang_n, 0:rang_theta].astype('float')
    ...
    ept = np.mgrid[0:rang_phi, 0:rang_n].astype('float')
    # ept(phi,n)
...
def sph2cart(phi):
    ...
    cart_grid = (np.mgrid[0:rang_p, 0:rang_p, 0:rang_p] + p_min).astype('float')
    cart_grid += 0.
...
def so3inti(phi):  # phi(x1,x2,x3,lp,mp,s,n)
    ...
    # p = np.zeros(f.shape + (rang_beta,)) + 0j
    pa = np.mgrid[0:rang_lp, 0:rang_n, 0:rang_mp, 0:rang_beta].astype('float')
