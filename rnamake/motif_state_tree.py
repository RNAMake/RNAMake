...
class MotifStateTree(object):
    ...
    #SETUP FUNCTIONS ##########################################################
    def __init__(self, rm, mt=None, **options):
        self._rm = rm
        self._sterics = 1
        self._tree = tree.TreeStatic()
        self._clash_radius = settings.CLASH_RADIUS
        self._connections = motif_connection.MotifConnections()

        if mt is not None:
            self._setup_from_mt(mt)

    @classmethod
    def from_str(cls, s, rm):
        pass

    @classmethod
    def copy(cls, mst):
        """
        generates a deep copy of the current motif state tree

        :return: deep of copy of current tree
        :rtype: MotifStateTree
        """
        new_mst = cls(mst._rm)
        new_mst._tree = mst._tree.copy()
        new_mst._connections = mst._connections.copy()
        return new_mst
    ...
    def _setup_from_mt(self, mt):
        ...
        self._connections = mt._connections.copy()
    ...
    def _get_parent_node(self, parent_index):
        ...
        parent = self._tree.last_node

        if parent_index != -1:
            try:
                parent = self._tree.get_node(parent_index)
    ...
    def _get_parent_available_ends(self, parent, parent_end_index,
        ...
        if parent_end_index != -1 and parent_end_name is not None:
            ...
            if self._connections.in_connection(parent.index, parent_end_name):
                raise exceptions.MotifStateTreeException(
            ...
            parent_end_name = parent.data.get_end(parent_end_index).name
            ...
            final_avail_pos = []
            for p in avail_pos:
                pen = parent.data.get_end(p).name
                if self._connections.in_connection(parent.index, pen):
                    continue
    ...
    def _get_state_from_manager(self, m_name, m_end_name):
        ...
        try:
            if m_end_name is not None:
                state = self._rm.get_state(name=m_name, end_name=m_end_name)
            else:
                state = self._rm.get_state(name=m_name)
    ...
    def add_state(self, state=None, parent_index=-1, parent_end_index=-1,
        ...
        if parent is None:
            ms_copy = motif_state.Motif.copy(state, new_uuid=1)
            return self._tree.add_data(ms_copy, ms_copy.num_ends(), -1, -1)

        avail_pos = self._get_parent_available_ends(parent, parent_end_index,
                                                    parent_end_name)

        for p in avail_pos:
            ms_copy = motif_state.get_aligned_motif_state(
                        parent.data.get_end(p), state)

            if self._sterics:
                if self._steric_clash(ms_copy):
                    continue

            return self._tree.add_data(ms_copy, ms_copy.num_ends(), parent.index, p)
    ...
    def add_mst(self, mst,  parent_index=-1, parent_end_index=-1,
        ...
        for i, n in enumerate(mst):
            if i == 0:
                j = self.add_state(n.data, parent_index, parent_end_index)
            else:
                ind = index_dict[n.parent_index()]
                pei = n.parent_end_index()
                j = self.add_state(n.data,
                                   parent_index=ind,
    ...
    def _get_connection_end(self, node, bp_name):
        ...
        if bp_name != "":
            ...
            if self._connections.in_connection(node.index, bp_name):
                raise exceptions.MotifStateTreeException(
            ...
            node_index_name = node.data.get_end(node_indexes[0]).name
            if self._connections.in_connection(node.index, node_index_name):
                raise exceptions.MotifStateTreeException(
    ...
    def add_connection(self, i, j, i_bp_name="", j_bp_name=""):
        ...
        node_i_end_name = node_i.data.get_end(node_i_ei).name
        node_j_end_name = node_j.data.get_end(node_j_ei).name

        self._connections.add_connection(i, j, node_i_end_name, node_j_end_name)

    def replace_state(self, i, new_state, keep_uuid=0):
        n = self.get_node(i)
        if new_state.num_ends() !=  n.data.num_ends():
            raise ValueError(
        ...
        n.data = motif_state.Motif.copy(new_state)

        if keep_uuid:
            n.data.uuid = uuid

        for i in range(old_state.num_ends()):
            if self._connections.in_connection(n.index, old_state.get_end(i).name):
                self._connections.update_connection_name(n.index,
                                                        old_state.get_end(i).name,
                                                        new_state.get_end(i).name)


        for n in tree.transverse_tree(self._tree, i):
            parent = n.parent
            if parent is None:
                continue
            pei = n.parent_end_index()

            motif_state.align_motif_state(parent.data.get_end(pei), n.data)

    #REMOVE FUNCTIONS   #######################################################
    def remove_node(self, i):
        self._tree.remove_node(index=i)
    ...
    def remove_node_level(self, level=None):
        self._tree.remove_node_level(level)

        for c in self._connections:
            if len(self._tree) > c.i or len(self._tree) > c.j:
                self._connections.remove(c)

    #TREE WRAPPER      ########################################################
    def get_node(self, i=None, uuid=None):
        if i is not None:
            return self._tree.get_node(i)
    ...
    def decrease_level(self):
        self._tree.decrease_level()

    def next(self):
        return self._tree.next()
    ...
    def to_motif_tree(self):
        ...
        for i, n in enumerate(self._tree.nodes):
            ...
            if i == 0:
                motif.align_motif(n.data.get_end(0), m.get_end(0), m)
                j = mt.add_motif(m)
            else:
                parent_index = n.parent_index()
                parent_end_index = n.parent_end_index()
...
class MotifStateTreePrinter(object):
        """
